...
begin
     ...
     ComboBoxProcessname.Items.Add('FUNC TEST');
     ComboBoxProcessname.Items.Add('SMT_VI_T');
     ComboBoxProcessname.Items.Add('SMT_VI_B');
     {
      with QryTemp do
        begin
          Close;
          commandtext:='select * FROM SAJET.SYS_process WHERE ENABLED=''Y'' AND STAGE_ID=10001 ORDER BY process_NAME ASC ';
          open;
...
begin
    ...
    QryTemp1.CommandText:=QryTemp1.CommandText + '  AND E2.PROCESS_CODE IS NOT NULL AND A2.MODEL_ID =B2.PART_ID AND B2.MODEL_ID=C2.MODEL_ID AND A2.DEFECT_ID =D2.DEFECT_ID '+
                           '  AND A2.STAGE_ID=10001 GROUP BY C2.MODEL_NAME,D2.DEFECT_CODE ) ' +           //STAGE_ID=10001  SMT段
