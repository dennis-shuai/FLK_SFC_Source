...
type
  TfStation = class(TForm)
    ...
    procedure editVICountKeyPress(Sender: TObject; var Key: Char);
  private
...
begin
  ...
  //Read VICount add by phoenix 2012-5-13
  GetVICount;
...
begin
  ...
  begin
     ...
     WriteString('CM','Terminal',sTerminalID);
     WriteString('CM','TypeID',trim(cmbOpid.Text));
     if  trim(editVICount.Text) <> '' then
       WriteString('CM','CarrierSNCount',trim(editVICount.Text))
     else
       WriteString('CM','CarrierSNCount','0');
     Free;
  end;
...
procedure TfStation.editVICountKeyPress(Sender: TObject; var Key: Char);
begin
  if (key < '0') or (Key > '9') then
  begin
    MessageDlg('Please Input Number',mtError,[mbOK],0);
    Abort;
  end;
end;

procedure TfStation.GetVICount;
var sIniFile : String;
begin
  ...
  With TIniFile.Create('SAJET.ini') do
  begin
    editVICount.Text := ReadString('CM','CarrierSNCount','0');
    free;
  end;
end;
