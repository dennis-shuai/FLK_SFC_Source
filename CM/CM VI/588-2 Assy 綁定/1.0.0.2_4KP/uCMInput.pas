...
type
  TfCMInput = class(TForm)
    ...
    procedure edt3DKeyPress(Sender: TObject; var Key: Char);
    procedure edtRGBKeyPress(Sender: TObject; var Key: Char);
    procedure edtLEDKeyPress(Sender: TObject; var Key: Char);
  ...
  public
    ...
    mCode,mCodeNo,mCodeDef,gsCheckSum,mDateCode,Carry16,mCarry:string;
...
var CSN,sNewCSN:string;
begin
   if  Key <>#13 then exit;
   if  edtLDM.Text = '' then exit;
   iResult := CheckItemSN(edtLDM.Text,'3');
   ...
   begin
       Close;
       DataRequest('SAJET.CCM_SN_4KP_ITEM_GO');
       FetchParams;
       Params.ParamByName('TWO').AsString :=sWO;
       Params.ParamByName('TSN').AsString :=edtCT.Text;
       Params.ParamByName('TCSN').AsString :=sNewCSN;
       Params.ParamByName('TITEM_PART_SN1').AsString := edt3D.Text;
       Params.ParamByName('TITEM_PART_SN2').AsString := edtRGB.Text;
       Params.ParamByName('TITEM_PART_SN3').AsString := edtLED.Text;
       Params.ParamByName('TITEM_PART_SN4').AsString := edtLDM.Text;
   ...
   if iResult ='OK' then
      ShowMsg('Input Status:',iResult,'Ω–±Ω¥y§U§@≠”')
   else
      ShowMsg('Input Error °G',iResult,'Ω–±Ω¥y§U§@≠”');
...
procedure TfCMInput.edt3DKeyPress(Sender: TObject; var Key: Char);
begin
   if  Key <>#13 then exit;
   if  edt3D.Text = '' then exit;
   iResult := CheckItemSN(edt3D.Text,'0');
   ShowMSG('3D Label Input',iResult,'');
   if iResult = 'OK' then
   begin
       edt3D.Enabled := False;
       edtRGB.Enabled := True;
...
begin
   ...
   ShowMSG('588 Module Label Input',iResult,'');
   if iResult = 'OK' then
   begin
       edtRGB.Enabled := False;
       edtLED.Enabled := True;
...
procedure TfCMInput.edtLEDKeyPress(Sender: TObject; var Key: Char);
begin
   if  Key <>#13 then exit;
   if  edtLED.Text = '' then exit;
   iResult := CheckItemSN(edtLED.Text,'2');
   ShowMSG('LED Board Label Input',iResult,'');
   if iResult = 'OK' then
   begin
       edtLED.Enabled := False;
       edtLDM.Enabled := True;
       edtLDM.SetFocus;
   end else begin
       edtLED.Clear;
...
begin
  Result := False;
  SNUdf.Clear;
   // ?? Carton or Pallet or Customer SN?ìö??
  ...
  begin
    ...
    begin
        ...
        begin
          Close;
          Params.Clear;
          CommandText := 'select sajet.packing_label(''' + NoType + ''',''Work Order'',''' + sWO + ''') SNID from dual';
          Open;
          Result := True;
