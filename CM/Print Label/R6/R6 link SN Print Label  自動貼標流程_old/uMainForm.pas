...
var sRes,sSN,sPrefix,sPART_NO:string;
begin
    if key <> #13 then exit;
    ...
    qrytemp.CommandText:='SELECT A.CUSTOMER_SN,A.SERIAL_NUMBER,B.PART_NO FROM SAJET.G_SN_STATUS A,SAJET.SYS_PART B '+
    ...
    begin
        ...
        if QryTemp.IsEmpty then begin
              Edtsn.SelectAll;
              edtsn.SetFocus;
              msgPanel.Caption := '¸Ó±ø½X¨S¦³³Q¸j©w' ;
              msgPanel.Color :=clRed;
              Exit;
        end;
        ...
        begin
           ...
           BarDoc.Variables.Item('CSN').Value :=   edtsn.Text;
           Bardoc.PrintLabel(1);
           Bardoc.FormFeed;
        end;
     ...
     if sRes ='OK' then begin
           ...
           begin
               ...
               if sRes = 'OK' then begin
                   ...
                   if (IsStart) and (IsOpen) then
                   begin
                       BarDoc.Variables.Item('SN').Value := trim(edtsn.TEXT);
                       BarDoc.Variables.Item('CSN').Value := trim(sPrefix+edtSN.Text);
                       Bardoc.PrintLabel(1);
                       Bardoc.FormFeed;
                   end;
                   edtsn.SelectAll;
