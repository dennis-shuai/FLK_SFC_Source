...
begin
   ...
   if qrytemp.FieldByName('CUSTOMER_SN').AsString =EdtSN.Text then
   begin
        if (IsStart) and (IsOpen) then
        begin
           sSN := Copy(edtsn.TEXT,9,7);
           BarDoc.Variables.Item('SN').Value :=  sSN;
           Bardoc.PrintLabel(1);
           Bardoc.FormFeed;
        end;
    ...
    if  qrytemp.FieldByName('CUSTOMER_SN').AsString <> 'N/A' then
    begin
        if (IsStart) and (IsOpen) then
        begin
