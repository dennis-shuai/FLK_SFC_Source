...
var iResult,sMatchWO,wo_LOT,sn_LOT,SWO,sNewNo,sPartNo,SWO1:string;
begin

  if  Key <>#13 then exit;
  if  editCustomer.Text ='' then exit;
  editDefect.Text :=UpperCase(Trim(editDefect.Text));
   ...
   begin
        ...
        CommandText := 'SELECT WORK_ORDER FROM SAJET.G_SN_STATUS WHERE CUSTOMER_SN= :SN OR SERIAL_NUMBER =:SN ';
        Params.ParamByName('SN').AsString := Trim(editCustomer.Text);
        Open;
   end;
        SWO1 := QryTemp.FieldByName('WORK_ORDER').AsString;

   if   Copy(SWO1,1,3) = 'NMS' then
   BEGIN
        editCustomer.Text :='';
        editCustomer.Setfocus;
        msgpanel.Caption :='½Ð¥ý§ë¤JµêÀÀ¤u³æ';
   ...
   begin
        ...
        CommandText := 'Select B.WORK_ORDER,B.CUSTOMER_SN ,C.PART_NO FROM SAJET.G_SN_KEYPARTS A,SAJET.G_SN_STATUS B,SAJET.SYS_PART C   '
                     + '  WHERE A.WORK_ORDER = B.WORK_ORDER AND A.PROCESS_ID =:PROCESS  '
                     + '        AND (B.Customer_SN =:SN or B.serial_number =:SN or A.Item_PART_SN =:SN ) AND A.SERIAL_NUMBER =B.SERIAL_NUMBER '
   ...
   if QryTemp1.IsEmpty then begin
   ...
   Sproc.Close;
   Sproc.DataRequest('SAJET.CCM_SN_2WO_INPUT_KP');
   Sproc.FetchParams;
   ...
   if UpperCase(Trim(editDefect.Text)) <>'' then
   ...
   begin
       ...
       begin
            sPartNo :=  QryTemp1.fieldByName('PART_NO').AsString;

            if sPartNo <> labPartNo.Caption then
            begin
                msgpanel.Caption :=' WO ERROR';
                msgPanel.Color :=clRed;
                editCustomer.Text :='';
                editCustomer.Setfocus;
                exit;
            ...
            begin
                 try
                     BarDoc.Variables.Item('SN').Value :=   sNewNo;
                     Bardoc.PrintLabel(1);
                     Bardoc.FormFeed;
