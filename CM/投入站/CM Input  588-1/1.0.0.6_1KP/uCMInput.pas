...
type
  TfCMInput = class(TForm)
    ...
    procedure edtPCBKeyPress(Sender: TObject; var Key: Char);
...
begin
  ...
  begin
    ...
    ShowMSG('Work Order input : ',iResult,'Please input SN!');
...
procedure TfCMInput.edtMainKeyPress(Sender: TObject; var Key: Char);
var csn:string;
begin
    if  Key <>#13 then exit;

    if StrToIntDef(labInputQty.Caption,0) >=  StrToIntDef(labTargetQty.Caption,0) then
    begin
    ...
    begin
        ...
        CommandText := ' select serial_number,customer_sn from Sajet.g_sn_status  where serial_number  =:SN or customer_sn=:sn  ';
        Params.ParamByName('SN').AsString := edtMain.Text ;
        Open;

        If not IsEmpty then
        begin
            ShowMsg('Input Error :','Error','±ø½X­«½Æ');
            edtMain.Clear;
            edtMain.SetFocus;
            Exit;
        end;
...
procedure TfCMInput.edtPCBKeyPress(Sender: TObject; var Key: Char);
begin
    if  Key <>#13 then exit;
    if  edtPCB.Text = '' then exit;
    iResult := CheckItemSN(edtPCB.Text,'0');
    ShowMSG('PCB Label Input',iResult,'');
    if iResult <> 'OK' then
    begin
         edtPCB.Clear;
         edtPCB.SetFocus;
    ...
    begin
         Close;
         DataRequest('SAJET.CCM_SN_KP_ITEM_INPUT');
         FetchParams;
         Params.ParamByName('TWO').AsString := editWO.Text;
         Params.ParamByName('TSN').AsString :=edtMain.Text;
         Params.ParamByName('TITEM_PART_SN').AsString := edtPCB.Text;
         Params.ParamByName('TPROCESSID').AsString :=sProcessID;
