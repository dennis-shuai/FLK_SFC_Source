...
type
  ...
  public
    UpdateUserID ,sProcessID,sPartID,PrintFile: String;
...
begin
  ...
  KillTask('lppa.exe');
   try
      ...
      Application.MessageBox('?›]ÓÐ°²ÑbcodesoftÜ›ów','åeÕ`',MB_OK+MB_ICONERROR);
      isStart:=false;
      Exit;
   end;
   IsStart :=true;
...
begin
   ...
   iResult := CheckItemSN(edtIR.Text,'2');
   ...
   if (SN ='N/A') or (SN ='') then begin
       ShowMsg('Input Error :','Error','¦Û°Ê²£¥Íªº±ø½X¿ù»~');
       edtIR.Clear;
       edtIR.SetFocus;
       Exit;
   end;

   if (CSN ='N/A') or (CSN ='') then begin
       ShowMsg('Input Error :','Error','¦Û°Ê²£¥Íªº«È¤á±ø½X¿ù»~');
       edtIR.Clear;
       edtIR.SetFocus;
       Exit;
   end;
   ...
   begin
      ...
      begin
           try
               ...
               BarDoc.Variables.Item('CSN').Value :=csn;
               Bardoc.PrintLabel(1);
               Bardoc.FormFeed;
               ShowMsg('Input Status:',iResult,'±ø½X¦C¦L§¹²¦,½Ð±½´y¤U¤@­Ó');
               edtIR.Clear;
               edtIR.SetFocus;
           except
   ...
   begin
       ...
       Params.Clear;
       Params.CreateParam(ftString,'WO',ptInput);
       CommandText := 'Select a.TARGET_QTY,a.INPUT_QTY FROM  SAJET.G_WO_BASE a'
...
begin
   ...
   iResult := CheckItemSN(edtLDM.Text,'0');
   ...
   begin
       ...
       begin
           ...
           CommandText := 'select a.serial_number,a.customer_sn from Sajet.g_sn_status a,sajet.g_sn_keyparts b where a.serial_number '+
                          ' = b.serial_number and (b.item_part_sn =:KPSN or b.serial_number =:KPSN) and b.enabled=''Y''  ';
           Params.ParamByName('KPSN').AsString := edtLDM.Text ;
           Open;
       end;

       if qryTemp.IsEmpty  then begin
           ShowMSG('LDM Label Input',iResult,'=> CT Label not found and print');
           edtLDM.Clear;
           edtLDM.SetFocus;
       end;

       sn :=QryTemp.fieldByName('Serial_number').AsString ;
       CSN :=QryTemp.fieldByName('customer_SN').AsString ;
       if (SN ='N/A') or (SN ='') then begin
           ShowMsg('Input Error :','Error','¦Û°Ê²£¥Íªº±ø½X¿ù»~');
           edtLDM.Clear;
           edtLDM.SetFocus;
       ...
       if UpdateUserID = '10000001' then begin
            if (IsStart) and (IsOpen) then
            begin
                 try
                     BarDoc.Variables.Item('SN').Value :=sn;
                     ...
                     msgpanel.Caption :=iResult+'======>>'+'—l±ø½X­«·s¦C¦L§¹²¦';
                     msgPanel.Color :=clGreen;
                     edtLDM.Text :='';
                     edtLDM.Setfocus;
                 except
...
begin
  Result := False;
  SNUdf.Clear;
   // ×xÈ¡ Carton or Pallet or Customer SN¾Ž´aÒŽ„t
...
begin
     ...
     if isStart then BarApp := Unassigned;
