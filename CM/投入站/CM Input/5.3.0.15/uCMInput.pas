...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ShowMSG('Work Order input : ',iResult,'叫块家マ!');
...
begin
  ...
  begin
    ...
    begin
          ...
          IF NOT QryTemp.IsEmpty THEN begin
            ...
            if smatchWO <> '' then
                if editWO.Text <> smatchWO then begin
                     msgPanel.Caption :=' CARRIER 莱赣で皌虫('+ smatchWO +')';
                     msgPanel.Color :=clRed ;
      ...
      //从状态表带出Carrier的工单  add by phoenix  2013-05-09
      with QryTemp do
      begin
        Close;
        Params.Clear;
...
begin
  ...
  begin
    ...
    //客户序号规则检查， add by pheonix 2013-5-9
    ...
    if  iResult <> 'OK' then
    begin
      ShowMSG('Customer ID input : ','Error','め兵絏岿粇');
      editCustomer.SetFocus;
      editCustomer.SelectAll;
      Abort;
    end;
    ...
    begin
      ShowMSG('Customer ID input : ','Error','虫щ计虫羆计!');
      editWO.Enabled := True;
      editWO.SetFocus;
      editWO.SelectAll;
      Abort;
    ...
    begin
      ...
      //重新取得工单的投入量  add by phoenix    2013-05-09
      with QryTemp do
      begin
        Close;
        Params.Clear;
...
begin
    ...
    if Key <> #13 then exit;
    ShowMSG('Work Order input : ','OK','叫块Carrier 腹絏!');
    editCarrier.Enabled := True;
    editCarrier.SetFocus;
    editCarrier.Text := '';
    editCustomer.Text := '';
