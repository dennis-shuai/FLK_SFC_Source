...
begin
    Result:=ExtractFilePath(Application.ExeName) + ExtractFileName('PickDetail.xlt')
end;
...
begin
   ...
   MsExcel.ActiveSheet.range['A2']:= 'W/O: '+fmain.QryDetail.FieldByName('work_order').AsString+'      P/N: '+fmain.QryDetail.FieldByName('part_no').asstring;
   row:=1;
   fmain.QryDetail.First;
   while not fmain.QryDetail.Eof do              
   begin
     MsExcel.ActiveSheet.range['A'+inttostr(3+row)]:=inttostr(row);
     MsExcel.ActiveSheet.range['B'+inttostr(3+row)]:=fmain.QryDetail.FieldByName('Work_order').asstring;
     MsExcel.ActiveSheet.range['C'+inttostr(3+row)]:=fmain.QryDetail.FieldByName('part_no').asstring;
     MsExcel.ActiveSheet.range['D'+inttostr(3+row)]:=fmain.QryDetail.FieldByName('material_no').asstring;
     MsExcel.ActiveSheet.range['E'+inttostr(3+row)]:=fmain.QryDetail.FieldByName('qty').asstring;
     MsExcel.ActiveSheet.range['F'+inttostr(3+row)]:=fmain.QryDetail.FieldByName('commit_user').asstring;
     MsExcel.ActiveSheet.range['G'+inttostr(3+row)]:=fmain.QryDetail.FieldByName('update_time').asstring;
     MsExcel.ActiveSheet.range['H'+inttostr(3+row)]:=fmain.QryDetail.FieldByName('confirm_user').asstring;
     inc(row);
