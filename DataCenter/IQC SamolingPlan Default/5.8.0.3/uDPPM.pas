...
begin
  ...
  begin
    try
       ...
       Params.ParamByName('FUN').AsString :='IQC Sampling Plan Default';
       Execute;
...
begin
  ...
  begin
     ...
     CommandText := 'Select B.PART_ID,C.Sampling_Type Lower_Plan ,E.Sampling_Type Default_Plan,F.Sampling_Type Upper_Plan,'+
                    'B.PART_NO, C.Sampling_TYPE,D.EMP_NAME,A.Update_Time ,A.enabled '+
                    'From SAJET.SYS_IQC_SAMPLING_DEFAULT A '+
                    '    ,SAJET.SYS_PART B '+
                    '    ,SAJET.SYS_QC_SAMPLING_PLAN C ,SAJET.SYS_EMP D '+
                    '    ,SAJET.SYS_QC_SAMPLING_PLAN E,SAJET.SYS_QC_SAMPLING_PLAN F  '+
                    'Where B.PART_NO like :part_no '+
                    'and A.PART_ID = B.PART_ID and A.UPDATE_USERID = D.EMP_ID '+
                    'and A.Lower_Sampling_ID =C.Sampling_id '+
                    'and A.Default_Sampling_ID = E.Sampling_id '+
                    'and A.Upper_Sampling_ID =  F.Sampling_id '+
                    'and C.ENABLED = ''Y'' '+
                    'and B.ENABLED = ''Y'' ' ;
...
begin
  ...
  begin
     ...
     cmbSamplingPlan.ItemIndex := cmbSamplingPlan.Items.IndexOf(QryData.Fieldbyname('Lower_Plan').aSString);
     cmbDefaultAQL.ItemIndex := cmbSamplingPlan.Items.IndexOf(QryData.Fieldbyname('Default_Plan').aSString);
     cmbUpperAQL.ItemIndex := cmbSamplingPlan.Items.IndexOf(QryData.Fieldbyname('Upper_Plan').aSString);
     LabType1.Caption := LabType1.Caption + ' Modify';
     LabType2.Caption := LabType2.Caption + ' Modify';
   //  PARTID := QryData.Fieldbyname('PART_NO').aSString;
    // editpartno.Enabled:=False;
     //cmbSamplingPlan.Enabled:=False;
     If Showmodal = mrOK Then
       ShowData;
     Free;
  end;
...
begin
  ...
  If MessageDlg('Do you want to delete this data ?' + #13#10+
                'PART NO : ' + QryData.Fieldbyname('PART_NO').AsString +#13#10,mtWarning, mbOKCancel,0) = mrOK Then
  begin
    With QryTemp do
    begin
      Close;
      Params.Clear;
      Params.CreateParam(ftString	,'PART_ID', ptInput);
      CommandText := 'Delete SAJET.SYS_IQC_Sampling_default  '+
                     'Where PART_ID = :PART_ID ';
...
begin
  ...
  begin
    Close;
    Params.Clear;
    Params.CreateParam(ftString	,'sSTATUS', ptInput);
    Params.CreateParam(ftString	,'UPDATEUSERID', ptInput);
    Params.CreateParam(ftString	,'PARTID', ptInput);
    CommandText := ' UPDATE SAJET.SYS_IQC_SAMPLING_DEFAULT SET ENABLED=:sSTATUS,Update_userID =:UPDATEUSERID,'+
                   ' Update_Time =SYSDATE  WHERE PART_ID=:PARTID';
    Params.ParamByName('sSTATUS').AsString := sStatus;
