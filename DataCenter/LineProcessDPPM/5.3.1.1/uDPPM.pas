...
type
  ...
  public
    ...
    Procedure CopyToHistory(SModelID,sPdlineID,sProcessID : String);
    procedure showData;
...
begin
  ...
  begin
    try
       ...
       Params.ParamByName('FUN').AsString :='Pdline Process DPPM Define';
       Execute;
...
Procedure TfDPPM.CopyToHistory(SModelID,sPdlineID,sProcessID : String);
begin
  With QryTemp do
  begin
     Close;
     ...
     CommandText := 'Insert Into SAJET.SYS_HT_PDLINE_PROCESS_RATE '+
                      'Select * from SAJET.SYS_PDLINE_PROCESS_RATE '+
...
begin
  ...
  begin
     ...
     CommandText := 'Select A.*, '+
                    ' B.PROCESS_NAME,C.PDLINE_NAME,D.MODEL_NAME '+
                    'From SAJET.SYS_PDLINE_PROCESS_RATE A '+
                    '    ,SAJET.SYS_PROCESS B '+
                    '    ,SAJET.SYS_PDLINE C '+
                    '    ,SAJET.SYS_MODEL D '+
...
begin
  ...
  begin
     ...
     editUPH.Text := QryData.Fieldbyname('UPH').aSString;
     editYield.Text := QryData.Fieldbyname('Target_Yield').aSString;
     editLower.Text := QryData.Fieldbyname('LOWER_LIMIT').aSString;
     editalarm.Text := QryData.Fieldbyname('alarm_LIMIT').aSString;
     edittarget.Text := QryData.Fieldbyname('target_LIMIT').aSString;
...
begin
  Orderidx := 'MODEL_NAME,PDLINE_NAME,PROCESS_NAME';
  cmbShowChange(Self);
...
begin
  ...
  If MessageDlg('Do you want to delete this data ?' + #13#10+
                ...
                'Line : ' + QryData.Fieldbyname('PDLINE_NAME').AsString +#13#10+
                'Process : ' + QryData.Fieldbyname('PROCESS_NAME').AsString +#13#10+
                'Target   : ' + QryData.Fieldbyname('Target_LIMIT').AsString +#13#10+
                'Alarm   : ' + QryData.Fieldbyname('alarm_LIMIT').AsString  +#13#10+
  ...
  begin
    ...
    begin
      ...
      Execute;
      CopyToHistory(QryData.Fieldbyname('MODEL_ID').AsString,QryData.Fieldbyname('PDLINE_ID').AsString, QryData.Fieldbyname('PROCESS_ID').AsString);
      ...
      Params.CreateParam(ftString	,'PDLINE_ID', ptInput);
      Params.CreateParam(ftString	,'MODEL_ID', ptInput);
      CommandText := 'Delete SAJET.SYS_PDLINE_PROCESS_RATE '+
                     'Where PROCESS_ID = :PROCESS_ID '+
                     'and MODEL_ID = :MODEL_ID ' +
                     'and pdline_id = :pdline_id ';
      Params.ParamByName('PROCESS_ID').AsString := QryData.Fieldbyname('PROCESS_ID').AsString;
...
begin
  ...
  S2 := QryData.Fieldbyname('MODEL_ID').AsString;
