...
type
  ...
  public
    { Public declarations }
    Procedure ShowHistory(ProcessID,Pdlineid,modelID : String) ;
  end;
...
Procedure TfHistory.ShowHistory(ProcessID,Pdlineid,modelID : String) ;
begin
  With QryData1 do
  begin
    Close;
    ...
    CommandText := 'Select model_name "Model Name",PDLINE_NAME "PDLINE NAME",Process_Name "Process",'+
                          'TARGET_LIMIT "TARGET Limit",ALARM_LIMIT "ALARM LIMIT", LOWER_LIMIT "LOWER LIMIT", '+
                          'EMP_NAME "Employee", A.UPDATE_TIME "Update Time",'+
                          'A.ENABLED "Status" '+
                   'From SAJET.SYS_HT_PDLINE_PROCESS_RATE A, '+
                        'SAJET.SYS_EMP B, '+
                        'SAJET.SYS_PROCESS C, '+
                        'SAJET.SYS_model D, '+
                        'SAJET.SYS_PDLINE E '+
                   'Where A.PROCESS_ID = :PROCESS_ID and A.model_ID = :model_ID  AND A.PDLINE_ID = :PDLINE_ID '+
