...
type
  ...
  public
    ...
    Procedure CopyToHistory(SPartMFGER_ID : String);
    procedure showData;
...
begin
  ...
  begin
    try
       ...
       Params.ParamByName('FUN').AsString :='Part MFGER Define';
       Execute;
...
Procedure TfDPPM.CopyToHistory(SPartMFGER_ID: String);
begin
  With QryTemp do
  begin
     Close;
     Params.Clear;
     Params.CreateParam(ftString	,'SPartMFGER_ID', ptInput);
     CommandText := 'Insert Into SAJET.SYS_HT_PART_MFGER '+
                      'Select * from SAJET.SYS_PART_MFGER '+
                      'Where part_mfger_ID = :SPartMFGER_ID ';
     Params.ParamByName('SPartMFGER_ID').AsString := SPartMFGER_ID;
     Execute;
  end;
end;
...
begin
  ...
  begin
     ...
     PARTMFGERID := QryData.Fieldbyname('PART_MFGER_ID').aSString;
...
begin
  ...
  if uppercase(orderidx)='MFGER_PART_NO' then
      orderidx:='A.MFGER_PART_NO';
  ShowData;
end;
...
begin
  ...
  If MessageDlg('Do you want to delete this data ?' + #13#10+
                ...
                'mfger_name : ' + QryData.Fieldbyname('mfger_name').AsString +#13#10+
                'mfger_part_no : ' + QryData.Fieldbyname('mfger_part_no').AsString +#13#10
  ...
  begin
    ...
    begin
      ...
      Params.ParamByName('part_mfger_ID').AsString := QryData.Fieldbyname('part_mfger_ID').AsString;
      Execute;
      CopyToHistory(QryData.Fieldbyname('PART_MFGER_ID').AsString);

      Close;
      Params.Clear;
      Params.CreateParam(ftString	,'part_mfger_ID', ptInput);
      CommandText := 'Delete SAJET.SYS_PART_mfger '+
                     'Where part_mfger_ID = :part_mfger_ID ';
      Params.ParamByName('part_mfger_ID').AsString := QryData.Fieldbyname('part_mfger_ID').AsString;
      Execute;
      QryData.Delete;
...
begin
  ...
  S1 := QryData.Fieldbyname('PART_MFGER_ID').AsString;
