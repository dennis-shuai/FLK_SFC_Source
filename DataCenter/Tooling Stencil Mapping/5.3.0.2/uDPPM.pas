...
type
  ...
  public
    ...
    Procedure CopyToHistory(SPartid,STOOLINGSNID : String);
    procedure showData;
...
begin
  ...
  begin
    try
       ...
       Params.ParamByName('FUN').AsString :='ToolingStencilMapping';
       Execute;
...
Procedure TfDPPM.CopyToHistory(SPartid,sTOOLINGSNID : String);
begin
  With QryTemp do
  begin
     Close;
     ...
     CommandText := 'Insert Into SAJET.SYS_HT_PART_STENCIL '+
                      'Select * from SAJET.SYS_PART_STENCIL '+
                      'Where TOOLING_SN_ID = :TOOLING_SN_ID '+
                      'and PART_ID = :PART_ID ';
     Params.ParamByName('TOOLING_SN_ID').AsString := STOOLINGSNID;
...
begin
  ...
  begin
     ...
     CommandText := 'Select A.*, '+
                    ' B.PART_NO,C.TOOLING_SN '+
                    'From SAJET.SYS_PART_STENCIL A '+
                    '    ,SAJET.SYS_PART B '+
                    '    ,SAJET.sys_tooling_sn C '+
                    'Where A.TOOLING_SN_ID = C.TOOLING_SN_ID '+
...
begin
  ...
  begin
    ...
    // STOOLINGSNID := QryData.Fieldbyname('TOOLING_SN_ID').aSString;
    // SPARTID := QryData.Fieldbyname('PART_ID').aSString;
     editTOOLINGSN.Enabled:=False;
...
begin
  ...
  If MessageDlg('Do you want to delete this data ?' + #13#10+
                'TOOLING SN : ' + QryData.Fieldbyname('TOOLING_SN').AsString +#13#10+
  ...
  begin
    ...
    begin
      ...
      Execute;
      CopyToHistory(QryData.Fieldbyname('PART_ID').AsString,QryData.Fieldbyname('TOOLING_SN_ID').AsString);

      Close;
      Params.Clear;
      Params.CreateParam(ftString	,'TOOLING_SN_ID', ptInput);
      Params.CreateParam(ftString	,'PART_ID', ptInput);
      CommandText := 'Delete SAJET.SYS_PART_STENCIL '+
                     'Where TOOLING_SN_ID = :TOOLING_SN_ID '+
                     'and PART_ID = :PART_ID ';
      Params.ParamByName('TOOLING_SN_ID').AsString := QryData.Fieldbyname('TOOLING_SN_ID').AsString;
      Params.ParamByName('PART_ID').AsString := QryData.Fieldbyname('PART_ID').AsString;
...
begin
  ...
  S := QryData.Fieldbyname('TOOLING_SN_ID').AsString;
