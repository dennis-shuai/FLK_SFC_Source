...
var i, j,iStartRow,iDiv,iMod: integer;
    vRange1:Variant;
begin
  MsExcel.Worksheets['Sheet1'].Range['A3']:= combWeek.Text+'Week';
...
begin
  ...
  combWeek.Clear;
  for i:= 1 to 53 do
    combWeek.Items.Add(FormatFloat('00',i));
  with QryTemp do
  begin
    Close;
    Params.Clear;
    CommandText := 'Select to_char(sysdate,''iw'') sWeek from dual ';
    Open;
    combWeek.ItemIndex:= combWeek.Items.IndexOf(Fieldbyname('sWeek').AsString);
    Close;
  end;
...
begin
  ...
  My_FileName := ExtractFilePath(Application.ExeName)+'FQCWeeklyReport.xlt';
...
begin
  ...
  sStartDate:=combYear.Text+combWeek.Text;
    ...
    sSQL:=' select a.*,nvl(c.model_name,''N/A'') model_name '
       ...
       +' where to_char(a.end_time - '+combHour.Text+'/24 - '+combMinute.Text+'/24/60,''yyyyiw'') = '''+sStartDate+''' '
       +' and a.qc_result <> ''N/A'' '
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        strgridData.Cells[iIndex,13]:=strgridData.Cells[iIndex,13]+ FieldbyName('LOT_MEMO').AsString +';';    // у癶磞瓃
      end else
      begin
        ...
        strgridData.Cells[iIndex,12]:=IntToStr(StrToIntDef(strgridData.Cells[iIndex,12],0)+FieldbyName('Lot_Size').asinteger);    //passs lot_size
