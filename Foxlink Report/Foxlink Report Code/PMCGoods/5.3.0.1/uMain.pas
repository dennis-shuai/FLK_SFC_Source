...
begin
  ...
  begin
   close;
   params.Clear;
   commandtext:=' select c1.part_no,b1.work_order,a1.material_no,a1.material_qty,d1.warehouse_name,e1.locate_name,f1.emp_name,b1.update_time  '
               +' from sajet.g_material a1,sajet.g_goods_incoming b1,sajet.sys_part c1,sajet.warehouse d1,sajet.sys_locate e1,sajet.sys_emp f1 '
               +' where  nvl(b1.material_no,b1.pallet_no)=a1.material_no '
               +'      and a1.part_id=c1.part_id '
               +'      and a1.locate_id=e1.locate_id(+) '
               +'      and a1.warehouse_id =c1.warehouse_id((+) '
               +'      and a1.locate_id=d1.locate_id(+) '
               +'      and b1.update_userid=f1.emp_id(+) ';
   if uppercase(cmbType.Text)='WORK ORDER' THEN
      COMMANDTEXT:=COMMANDTEXT+' AND B1.WORK_ORDER='+''''+TRIM(editFilter.Text)+''''
   else if uppercase(cmbType.Text)='PART NO' THEN
      COMMANDTEXT:=COMMANDTEXT+' AND c1.part_no='+''''+trim(editFilter.Text)+''''
   else if uppercase(cmbType.Text)='PALLET' then
      commandtext:=commandtext+' and nvl(b1.material_no,b1.pallet_no)='+''''+trim(editFilter.Text)+'''';
   if  chktime.Checked then
     commandtext:=commandtext+ 'b1.update_time between (to_date('+FormatDateTime('YYYYMMDD',dateFrom.DateTime)+'000000'+',''YYYYMMDDHH24MISS'') and '
                  +' to_date('+FormatDateTime('YYYYMMDD',dateEnd.DateTime)+'235959'+',''YYYYMMDDHH24MISS'') )';
...
var 
  T1:tDate;
begin
  if trim(editFilter.Text)='' then exit;
  t1:=time;
  showstore ;
...
begin
  ...
  begin
    ...
    Params.ParamByName('dll_name').AsString := 'PMCGOODSDLL.DLL';
    memo1.Text:=commandtext;
    Open;
