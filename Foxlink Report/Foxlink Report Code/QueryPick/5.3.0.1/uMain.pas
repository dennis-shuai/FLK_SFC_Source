...
begin
  ...
  begin
   close;
   params.Clear;
   commandtext:=  ' select work_order,model_name,wo_status,part_no,version,request_qty,issue_qty,(issue_qty-scrap_qty)return_qty,ab_issue_qty,ab_return_qty '
               +'          ,(scrap_qty+AB_ISSUE_QTY-request_qty-AB_RETURN_QTY) sub_qty,round(decode(request_qty,''0'',''0'',(scrap_qty+AB_ISSUE_QTY-request_qty-AB_RETURN_QTY)/request_qty),2) sub_percent '
               +' from (    '
               +' select a.work_order,d.part_no model_name,decode(c.wo_status,''0'',''Initial'',''1'',''Prepare'',''2'',''Release'',''3'',''WIP'',''4'',''Hold'',''5'',''Cancel'',''6'',''Complete'',''9'',''Complete-NoCharge'') WO_Status '
               +'          ,b.part_no,a.version,sum(a.request_qty) request_qty,decode(sum(a.scrap_qty),''0'',sum(a.issue_qty),sum(a.scrap_qty)) scrap_qty,sum(a.issue_qty) issue_qty,sum(a.AB_ISSUE_QTY) AB_issue_qty,sum(a.AB_RETURN_QTY) ab_return_qty '
               +'from sajet.g_wo_pick_list a,sajet.sys_part b,sajet.g_wo_base c,sajet.sys_part d '
               +' where a.part_id=b.part_id '
               +'      and a.work_order=c.work_order '
               +'      and c.model_id=d.part_id ';
   if trim(editWO.Text)<>'' then
     commandtext:=commandtext+' and a.work_order='+''''+trim(editwo.Text)+'''';
   commandtext:=commandtext+' group by a.work_order,d.part_no,b.Part_no,a.version,wo_status ';
...
begin
  ...
  begin
    ...
    Params.ParamByName('dll_name').AsString := 'QUERYPICKDLL.DLL';
    memo1.Text:=commandtext;
    Open;
...
begin
   ...
   begin
     ...
     MsExcel.ActiveSheet.range['C'+inttostr(3+row)]:=qrydata.FieldByName('model_name').asstring;
     MsExcel.ActiveSheet.range['D'+inttostr(3+row)]:=qrydata.FieldByName('part_no').asstring;
     MsExcel.ActiveSheet.range['E'+inttostr(3+row)]:=qrydata.FieldByName('version').asstring;
     MsExcel.ActiveSheet.range['F'+inttostr(3+row)]:=qrydata.FieldByName('request_qty').asstring;
     MsExcel.ActiveSheet.range['G'+inttostr(3+row)]:=qrydata.FieldByName('issue_qty').asstring;
     MsExcel.ActiveSheet.range['H'+inttostr(3+row)]:=qrydata.FieldByName('return_qty').asstring;
     MsExcel.ActiveSheet.range['I'+inttostr(3+row)]:=qrydata.FieldByName('ab_issue_qty').asstring;
     MsExcel.ActiveSheet.range['J'+inttostr(3+row)]:=qrydata.FieldByName('ab_return_qty').asstring;
     MsExcel.ActiveSheet.range['K'+inttostr(3+row)]:=qrydata.FieldByName('sub_qty').asstring;
     MsExcel.ActiveSheet.range['L'+inttostr(3+row)]:=floattostr(strtofloat(qrydata.FieldByName('sub_percent').asstring));
     inc(row);
     qrydata.Next;
   end;
end;
...
begin
    Result:=ExtractFilePath(Application.ExeName) + ExtractFileName('QueryPick.xlt')
end;
...
begin
  ...
  begin
    close;
    params.Clear;
    commandtext:=' select a.work_order,b.part_no model_name,decode(wo_status,''2'',''Release'',''3'',''WIP'',''4'',''Hold'',''5'',''Cancel'',''6'',''Complete'',''9'',''Complete-NoCharge'') wo_status '
