...
begin
    ...
    //QueryDefect(QryData, sDStartTime, sDEndTime);
end;
...
begin
    ...
    QryTemp1.CommandText:=' select  a.pdLine_NAME,B.PROCESS_NAME, B.PROCESS_CODE, '+
                         ...
                         ' c.model_ID =d.Part_ID and d.Part_no like ''7690-3368%'' and  a.pdLine_NAME not like ''CM-%'' and '+
...
begin
    ...
    //QryTemp1.Params.CreateParam(ftstring,'FwdDate',ptInput);
    QryTemp1.CommandText:='select Process_NAME , DEFECT_CODE ,Defect_desc,count(C_SN) as Count  ' +
                           ' from '    +
                           ' (  select work_order,serial_number,Process_Name,defect_code,defect_desc,C_SN from ( '    +
                               '  ( select a.work_order,  a.Serial_number,B.PROCESS_NAME , C.defect_Code, c.Defect_desc,1 as C_SN   '+
                               ' from           '+
                               ...
                               ' and a.REC_Time <to_date(:EndTime, ''yyyy/mm/dd hh24:mi:ss'')   and c.defect_Code not like ''SFR%'' and a.ntf_time is null  ' +
                               ' and B.Process_Name=:process and a.WORK_ORDER NOT Like ''RM%''    ' +
                               ' group by  a.work_order,B.PROCESS_NAME , C.defect_Code , c.Defect_desc,a. Serial_number  )  '+
                               ' union           '+
                               ' ( select  a.work_order, a.Serial_number,B.PROCESS_NAME , ''SFR'' as Defect_Code, c.Defect_desc,1 as C_SN   '+
                               ' from                   '+
                               ' sajet.g_SN_defect_first a,sajet.SYS_PROCESS b,sajet.sys_defect c     '+
                               ' where A.DEFECT_ID=C.DEFECT_ID and A.PROCESS_ID=B.PROCESS_ID and a.REC_Time >= to_date(:StartTime, ''yyyy/mm/dd hh24:mi:ss'')  '+
                               ' and a.REC_Time <to_date(:EndTime, ''yyyy/mm/dd hh24:mi:ss'')   and c.defect_Code  like ''SFR%'' and a.ntf_time is null '+
                               ' and a.WORK_Order not like ''RM%'' and B.Process_Name=:process '+
                               ' group by  a.work_order, B.PROCESS_NAME , C.defect_Code, c.Defect_desc ,a. Serial_number  ) '+
                               ' ) )   '+
                               ' group by Process_NAME,DEFECT_CODE ,Defect_desc ' +
                               ' order  by process_name, Count desc ' ;
    QryTemp1.Params.ParamByName('StartTime').AsString := sStartTime;
    //QryTemp1.Params.ParamByName('FwdDate').AsString := sFwdDate;
    QryTemp1.Params.ParamByName('EndTime').AsString := sEndTime;
...
var
    sDStartTime,sNEndTime,SForwardDate :string;
    swStartTime,swEndTime,strTitle,strFPY,strNTF,strPDline,strPDLineFrist,strTitleFirst,strRepass,strTotal,strDefectName,strDefect_QTY,strDefect_Desc :string;
    i,j,LineCount,UsedRow,UsedCount,defect_count,TotalDefect,TotalAOO,TotalFF,TotalFQC:integer;
    ExcelApp: Variant;
begin
    if SaveDialog1.Execute then begin
        ...
        SForwardDate := FormatDateTime('yyyy/mm/dd ',DateTimePicker1.Date-30)+'08:00:00';
        swEndTime    := FormatDateTime('yyyy/mm/dd ',DateTimePicker1.Date+1)+'08:00:00';

        ExcelApp :=CreateOleObject('Excel.Application');
        ExcelApp.Visible :=false;
        ExcelApp.displayAlerts:=false;
        ExcelApp.WorkBooks.Open(ExtractfilePath(ParamStr(0))+'\SFC Yield Report for All.xlt');
        ...
        begin
             ...
             begin
                  ...
                  if  strTitle = 'FQC' then  begin
                     ...
                     ExcelApp.Cells[6,3].Value := StrToInt(strRepass)+ StrToInt(strFPY);
                     ExcelApp.Cells[6,4].Value := (StrToInt(strRepass)+StrToInt(strFPY))/StrToInt(strTotal);
                     ExcelApp.Cells[6,5].Value := 0;
        ...
        begin
             ...
             begin
                  ...
                  strDefect_Desc := QryTemp.FieldByName('Defect_Desc').AsString;
                  strDefect_QTY :=  QryTemp.FieldByName('Count').AsString;
                  ExcelApp.Cells[22+i,3].Value := strDefectName;
                  ExcelApp.Cells[22+i,2].Value := strtoint(strDefect_QTY)/StrtoInt(ExcelApp.Cells[5,2]);
                  ExcelApp.Cells[22+i,1].Value := strDefect_QTY;
                  ExcelApp.Cells[22+i,4].Value := strDefect_Desc;
                  QryTemp.Next;
              end;
        end;
        ...
        begin
             ...
             begin
                  ...
                  strDefect_Desc := QryTemp.FieldByName('Defect_Desc').AsString;
                  strDefect_QTY :=  QryTemp.FieldByName('Count').AsString;
                  ExcelApp.Cells[48+i,3].Value := strDefectName;
                  ExcelApp.Cells[48+i,2].Value := strtoint(strDefect_QTY)/StrtoInt(ExcelApp.Cells[6,2]);
                  ExcelApp.Cells[48+i,1].Value := strDefect_QTY;
                  ExcelApp.Cells[48+i,4].Value := strDefect_Desc;
                  QryTemp.Next;
             end;
        end;
        ...
        for i:= 0 to QryTemp.RecordCount-1 do  begin
          ...
          strTitle :=QryTemp.FieldByName('PROCESS_NAME').AsString ;
          strFPY :=  QryTemp.FieldByName('FPY_QTY').AsString;
          strNTF :=  QryTemp.FieldByName('NTF_QTY').AsString;
          strTotal :=  QryTemp.FieldByName('Total_QTY').AsString;
