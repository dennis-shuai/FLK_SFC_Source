...
//
  ...
  begin
    close;
    params.Clear;
    commandtext:=' select b.part_no,a.material_no,a.material_qty,a.datecode,a.reel_no,a.reel_qty,a.fifocode,d.warehouse_name,c.locate_name,a.release_qty,e.emp_no,a.update_time '
    ...
    if sWarehouse='' then
      commandtext:=commandtext+' and d.warehouse_name is null '
    else
      commandtext:=commandtext+'     and d.warehouse_name= '+''''+sWarehouse+'''';
    if sLocate='' then
      commandtext:=commandtext+' and c.locate_name is null '
    else
      commandtext:=commandtext+'     and c.locate_name= '+''''+sLocate+'''';

    commandtext:=commandtext+' order by a.fifocode asc' ;
    open;
    memo1.Text:=commandtext;
  end;
