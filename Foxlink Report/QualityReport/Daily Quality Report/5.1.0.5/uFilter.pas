...
begin
  ...
  else if F_Str='Defect Code' then
  begin
    DBGrid1.Columns.Items[2].Visible:=False;
    DBGrid1.Columns.Items[1].Width:=370;
    for iTemp:=0 to fMainForm.StrLstDefectName.Count-1 do
    begin
      ChkListData.AddItem(fMainForm.StrLstDefectName.Strings[iTemp],Self);
      ChkListData.Checked[iTemp]:=True;
    end;
  end;
...
begin
  ...
  begin
    sID:=ClientDataSet1.Fieldbyname('DEFECT_ID').AsString;
    sName:=ClientDataSet1.Fieldbyname('DEFECT_Code').AsString;
    cDesc:=ClientDataSet1.Fieldbyname('DEFECT_DESC').AsString;
  end;
  ...
  begin
    ChkListData.AddItem(sName+'  ||  '+cDesc,Self);
    ChkListData.Checked[ChkListData.Items.Count-1]:=True;
  end;
...
begin
  ...
  begin
    ...
    begin
      if F_Str='Defect Code' then
      begin
        i1:=fMainForm.StrLstDefectName.IndexOf(ChkListData.Items.Strings[i]);
        fMainForm.StrLstDefect.Delete(i1);
        fMainForm.StrLstDefectName.Delete(i1);
      end
      else if F_Str='Process Name' then
      begin
