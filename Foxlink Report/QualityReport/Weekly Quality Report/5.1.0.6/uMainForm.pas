...
type
  ...
  public
    ...
    StrLstProcess ,slFactoryID,StrLstProcessName,StrLstStageName  : TStringList;
    stStage,stModel,stProcess,stIndexType : String;
    slStrProcess,processlst :Tstrings;
    G_sSysLang : Integer;
...
Var cost,D1,D2,Dtemp : TDateTime;
    iRow,iCol,iTemp,iTopDefect : Integer;
    Str,sLastWeek : String;
    slStrWeek : TStringList;
begin
  cost:=time();
  Labcnt.Caption:='';
  Labcost.Caption:='';
  ...
  if FormatDateTime('YYYYMM', Date)<>(edtYearW.text+ComboBox1.text) then
    Dtemp:=D2
  else
    Dtemp:=Now;
  if Length(IntToStr(WeekOf(Dtemp)))=1 then
    sLastWeek:=edtYearW.text+'0'+IntToStr(WeekOf(Dtemp))
  else
    sLastWeek:=edtYearW.text+IntToStr(WeekOf(Dtemp));
  ...
  begin
    Close;
    Params.Clear;
    Params.CreateParam(ftString	,'YearM', ptInput);
    Params.CreateParam(ftString	,'YT', ptInput);
    Params.CreateParam(ftString	,'YT2', ptInput);
    ...
    CommandText := ' SELECT * FROM   '+
                   ...
                   '          FROM (  '+
                   '                  SELECT  TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYYWW'') WORK_DATE, A.PROCESS_ID , 0 DEFECT_CNT, '+
                   ...
                   '                       :YearM = SUBSTR(A.WORK_DATE,1,6) AND    '+
                   '                       A.WORK_ORDER=C.WORK_ORDER AND '+
                   '                       C.FACTORY_ID='''+FCID+''' '+
                                           AndCondition('B.MODEL_ID',StrLstModel)+
                                           AndCondition('A.PROCESS_ID',StrLstProcess)+
                   '                  GROUP BY TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYYWW'') ,A.PROCESS_ID '+

                   '                  Union ALL '+

                   '                  SELECT  TO_CHAR(A1.REC_TIME,''YYYYWW'') WORK_DATE , A1.PROCESS_ID,COUNT(A1.DEFECT_ID) DEFECT_CNT, '+
                   ...
                   '          FROM (  '+
                   '                 SELECT  TO_CHAR(A2.REC_TIME,''YYYYWW'') WORK_DATE,G2.PROCESS_NAME,B2.DEFECT_CODE,B2.DEFECT_DESC,D2.REASON_CODE,D2.REASON_DESC,E2.DUTY_CODE,F2.Location,COUNT(*) QTY '+
                   ...
                   '                       :YT2 = TO_CHAR(A2.REC_TIME,''YYYYWW'') AND  '+
                   '                       A2.WORK_ORDER = N2.WORK_ORDER AND '+
                   '                       N2.FACTORY_ID = '''+FCID+''' '+
                                           AndCondition('M2.MODEL_ID',StrLstModel)+
                                           AndCondition('A2.PROCESS_ID',StrLstProcess)+
                   '                 GROUP BY TO_CHAR(A2.REC_TIME,''YYYYWW''),G2.PROCESS_NAME,B2.DEFECT_CODE,B2.DEFECT_DESC,D2.REASON_CODE,D2.REASON_DESC,E2.DUTY_CODE,F2.Location ) ) '+
    ...
    if checkretry.Checked =true then
             CommandText := ' SELECT * FROM   '+
                   ...
                   '                 GROUP BY TO_CHAR(A2.REC_TIME,''YYYYWW''),G2.PROCESS_NAME,B2.DEFECT_CODE,B2.DEFECT_DESC,D2.REASON_CODE,D2.REASON_DESC,E2.DUTY_CODE,F2.Location ) ) '+
                   ' ORDER BY PROCESS_NAME,WORK_DATE ,QTY DESC ';
    Params.ParamByName('YearM').AsString := edtYearW.Text+ComboBox1.Text;
    Params.ParamByName('YT').AsString := edtYearW.Text+ComboBox1.Text;
    Params.ParamByName('YT2').AsString := sLastWeek;
    Open;
    ...
    begin
      ...
      while WeekOf(D1)+iCol<=WeekOf(D2) do
      begin
        if Length(IntToStr(WeekOf(D1)+iCol))=1 then
          Str:='Week0'+IntToStr(WeekOf(D1)+iCol)
        else
          Str:='Week'+IntToStr(WeekOf(D1)+iCol);
      ...
      while not eof do
      begin
        Str:='Week'+Copy(FieldByName('WORK_DATE').asString,5,2);
        ProgressBar1.Progress:=Round(RecNo / RecordCount * 100);
        if slStrWeek.IndexOf(Str)=-1 then
        begin
          slStrWeek.Add(Str);
          sgData.Cells[slStrWeek.Count+1,0]:= 'Week'+Copy(FieldByName('WORK_DATE').asString,5,2);
        end;
        ...
        begin
          ...
          begin
            ...
            Inc(iTemp);
            sgData.Cells[1,iRow]:= 'Week'+Copy(FieldByName('WORK_DATE').asString,5,2);
        ...
        begin
          ...
          if checkretry.Checked =true then
             begin
               sgData.Cells[iCol+1,iRow+1]:= IntToStr(FieldByName('PASS_QTY').AsInteger);     //癘魁pass qty
...
begin
  STRPOINTCOUNT:=InputBox('Input Box', 'Please input defect point count!', '1');
end;
