...
begin
  ...
  My_FileName := ExtractFilePath(Application.ExeName)+'RepairWaitReport.xlt';
...
begin
  ...
  begin
    Close;
    commaNDtext:=' select model_name,defect_date,process_name,defect,count(*) as qty from  '
                 +' (SELECT a.model_name,to_char(C.rec_time,''YYYY/MM/DD'') as defect_date,  '
                 +' f.process_name,e.defect_code||''-''||defect_desc as defect  '
                 +' FROM sajet.SYS_MODEL A,sajet.SYS_PART B ,sajet.G_SN_DEFECT C,  '
                 ...
                 +' AND to_char(C.rec_time,''YYYYMMDDHH24MI'') BETWEEN :starttime AND :endtime  '
                 +' AND C.rp_status is null   '
                 ...
                 +' and (c.work_order,c.serial_number,c.process_id) in  '
                 +' (select work_order, serial_number,process_id from sajet.g_sn_status where current_status=''1'') )   '
                 +' group by model_name,defect_date,process_name,defect  ';
