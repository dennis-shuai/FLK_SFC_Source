    ...
    QryTemp1.CommandText:= 'select Process_NAME ,Process_Code, DEFECT_CODE ,Defect_desc,count(C_SN) as Count  ' +
                           ...
                           ' (    '    +
                                 '  ( select   a.Serial_number,B.PROCESS_NAME ,b.Process_Code, C.defect_Code, c.Defect_desc,1 as C_SN   '+
                                         ...
                                         ' and a.ntf_time is null  and a.WORK_ORDER NOT LIKE ''RM%'' and a.MOdel_ID=d.Part_ID and e.MOdel_ID =d.Model_ID and e.Model_Name =:Model_Name'+
...
begin
    if SaveDialog1.Execute then begin
       ...
       ExcelApp.ActiveSheet.Range['A'+IntToStr(i+3)+':E'+IntToStr(i+3)].Interior.Color :=rgb(55,96,145);
        ...
        begin
              QryDefect.First;
              for  j:=i+4 to defect_count+i+3  do
              begin
...
begin
   ...
   QryTemp1.Close;
   QryTemp1.Params.Clear;
   QryTemp1.CommandText := 'select driver_parameter from SAJET.TGS_GATEWAY_BASE ';//where Server_ID=115 and Gateway_ID=2  ';
   QryTemp1.Open;
   //QryTemp1.Append;
   QryTemp1.First;
   //for  i:=0 to QryTemp1.RecordCount-1 do begin
     TBlobField( QryTemp1.FieldByName('driver_parameter')).SavetoStream(vstream);
     vstream.SaveToFile('D:\123.txt');
    // Qrytemp1.Next;
   //end;
end;
