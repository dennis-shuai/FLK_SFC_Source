                ...
                if  iCount>= 2 then   begin
                     ...
                     if not directoryexists( sFilePath+'\BAK\') then
                      ...
                      if trim(strs.Strings[iCount-1])<>'' then begin
                          ...
                          strResult := strsLast.Strings[18];
                          strResult1 := strsLast.Strings[58];
                          sn := strsLast.Strings[2];
                          //ShowMessage(sn);

                          if (Length(sn) =10)   and ((Copy(sn,1,1) ='P') or  (Copy(sn,1,1) ='M') or (Copy(sn,1,2) ='SN') )
                                  and ( (strResult = 'OK') or (strResult = 'ALERT') ) then begin
