...
var sr: TSearchRec;
sn,sresult,sFilePath,serrCode,strResult,strResult1,Strconn,StrModel,StrStation:string;
i,iCount:integer;
                ...
                if  iCount>= 2 then   begin
                     ...
                     if not directoryexists( sFilePath+'\BAK\') then
                      ...
                      if trim(strs.Strings[iCount-1])<>'' then begin
                          ...
                          strResult := strsLast.Strings[10];
                          strResult1 := strsLast.Strings[59];
                          sn := strsLast.Strings[2];

                          if (Length(sn) =10)   and (Copy(sn,1,2) ='SN')  and ( (strResult = 'OK') or (strResult = 'ALERT') ) then begin

                               if ((strResult = 'OK') or (strResult = 'ALERT')) and (strResult1 ='OK')   then
                                  sresult:='PASS'
                               else
                               begin
                                  sresult:='FAIL';
                      ...
                      //stat1.SimpleText :='Data 上傳OK ';
                      //stat1.Color :=clGreen;
                end;
            Except
               // stat1.SimpleText :='Data 上傳失敗';
               // stat1.Color :=clRed;
            end;
            strs.free;
        end;
