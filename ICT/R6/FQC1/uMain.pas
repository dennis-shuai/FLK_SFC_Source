...
var sr: TSearchRec;
sn,sresult,sFilePath,serrCode,strResult,Strconn,StrModel,StrStation,strDate:string;
i,iCount:integer;
...
begin
     ...
     sFilePath:=CFGIniFile.ReadString('TestLogFile','LogFilePath','');
     CFgIniFile.Free;
     strDate :=FormatDateTime('YYYY_MM_DD',Now());
    if FindFirst( sFilePath+'\*'+strDate+'.csv',faAnyFile,sr)=0 then begin
       repeat
                ...
                begin
                     ...
                     if not directoryexists( sFilePath+'\BAK\') then
                      ...
                      if trim(strs.Strings[iCount-1])<>'' then begin
                          ...
                          strResult := strsLast.Strings[1];
                          sn := strsLast.Strings[2];
                          strResult := trim(strResult);

                          if (Length(sn) =7)   then begin

                               if trim(strResult) <> 'PASS'   then
                               begin
                                  sresult:='FAIL';
                                  serrCode :='123';
                               end else     sresult:='PASS';
