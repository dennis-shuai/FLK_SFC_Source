...
begin
    ...
    try
        ...
        if not Assigned(con1) then con1 := TSocketConnection.Create(nil);
        if not Assigned(smplbjctbrkr1) then  smplbjctbrkr1 := TSimpleObjectBroker.Create(nil);

        if not Assigned(qrySproc) then  qrySproc := TClientDataSet.Create(nil);

        if  (Assigned(con1)) and (not con1.Connected) then   begin
        ...
        AssignFile(slogFile,slogfileName);
        Append(slogFile);
        Writeln(slogFile,FormatDateTime('YYYY/MM/DD HH:MM:SS',Now)+'  Start Connect Fail');
        Writeln(slogFile,FormatDateTime('YYYY/MM/DD HH:MM:SS',Now)+'  Program:'+extractFileName(ParamStr(0)));
        closefile(slogFile);
        Exit;
    end;
...
begin
    ...
    iMsg :='';
    SetLength(iMsg,0);
    if Assigned(con1) and ( not con1.Connected) then
       SajetTransStart;
    if not Assigned(con1) then begin
        iMsg :='NG;have not start connecttion ';
        iLen := Length(iMsg);
        ...
        AssignFile(slogFile,slogfileName);
        Append(slogFile);
        Writeln(slogFile,FormatDateTime('YYYY/MM/DD HH:MM:SS',Now)+'  '+iMsg);
        closefile(slogFile);
        Exit;
    end;
...
begin
   ...
   try
        ...
        if Assigned(con1) then begin
             con1.Connected := False;
             con1.Host:='';
             con1.Address:='';
        end;
        if Assigned(smplbjctbrkr1) then  smplbjctbrkr1.Servers.Clear;
        if Assigned(qrySproc) then
        begin
            qrySproc.Free;
            qrySproc :=nil;
        end;
        ...
        if Assigned(smplbjctbrkr1) then
        begin
            smplbjctbrkr1.Free;
            smplbjctbrkr1 :=nil;
        end;
