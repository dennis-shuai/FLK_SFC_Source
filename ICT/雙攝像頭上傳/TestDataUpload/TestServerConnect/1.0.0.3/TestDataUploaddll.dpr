...
begin
    ...
    if  path ='' then begin
        Result :=  'NG;保存測試數據時,測試Log檔案路徑為空值';
        Exit;
    end;
...
begin
    ...
    try
        slTitle := TStringList.Create;
        if Copy(strTitle,Length(strTitle),1)=';' then
           strTitle := Copy(strTitle,1,Length(strTitle)-1);
        slTitle := SplitString(strTitle,';');
        iTitleCount := slTitle.Count;
        ...
        slData := TStringList.Create;
        if Copy(strData,Length(strData),1)=';' then
           strData := Copy(strData,1,Length(strData)-1);
        slData := SplitString(strData,';');
        iDataCount := slData.Count;

        if iDataCount <> colCount then begin
            Result := 'NG;資料欄位數量'+IntToStr(iDataCount)+'不等於'+IntToStr(colCount);
            if Assigned(slTitle) then slTitle.Free;
            if Assigned(slData) then slData.Free;
            exit;
        end;
...
begin
     ...
     try
         ...
         iColCount :=  StrToIntDef(slData.Strings[2],0);
         sStrTitle :=  slData.Strings[3];
         SN :=  slData.Strings[4];
         sStrData :=  slData.Strings[5];
         slData.Free;
     except
         if Assigned(slData) then slData.Free;
         sResult := 'NG;資料格式錯誤';
