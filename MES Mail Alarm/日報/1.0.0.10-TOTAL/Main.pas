      ...
      IdSMTP1.Connect;
          ...
          begin
              ...
              if FormatDateTime('HH',GetSysDate) <= '12' then
                Subject:='時報表報->'+FormatDateTime('YYYY/MM/DD',GetSysDate-1)+' 白晚班生產報表(廠內數據)'
              else
                Subject:='時報表報->'+FormatDateTime('YYYY/MM/DD',GetSysDate)+' 白班生產報表(廠內數據)';
              ...
              if FormatDateTime('HH',GetSysDate) > '12' then begin
                  ...
                  stemp :='  '+FormatDateTime('YYYY/MM/DD',GetSysDate-1)+'白班生產報表' ;
                  mmo1.Lines.Add(stemp);
              end
              else begin
                  sMaileMessage:='Dear All:';
                  mmo1.Lines.Add(sMaileMessage);
                  stemp :='  '+FormatDateTime('YYYY/MM/DD',GetSysDate)+' 白晚班生產報表';
                  mmo1.Lines.Add(stemp);
              end;
               stemp:= '   SMT生產明細如下:';
               mmo1.Lines.Add(stemp);
            ...
            { IsFind :=0;
             ...
             sMaileMessage:= sMaileMessage+#13;
             sMaileMessage:= sMaileMessage+'   清潔站生產明細如下:'+#13+'      ';
             for  I:=0 to 19 do begin
                if   CLEANReordlist[i].ModelName <>''    then begin
                     sMaileMessage :=  sMaileMessage +format('%-20s',[CLEANReordlist[i].ModelName +'機種'])+
                                          format('%-12s',['待投:'+ CLEANReordlist[i].WIP])+
                                          format('%-12s',['投入:'+ CLEANReordlist[i].Input])+
                                          format('%-12s',['產出:'+ CLEANReordlist[i].Output])+
                                          format('%-18s',['目標直通率:99%'])+
                                          format('%-18s',['實際直通率:'+ CLEANReordlist[i].FPY]) +'    在制工單:'+
                                          CLEANReordlist[i].WIPWO+#13+'      ';
                  IsFind :=1;
                end;
             end ;
             If  IsFind=0  then begin
                 sMaileMessage :=  sMaileMessage + '無投入和產出'+#13;
             end;
             }
              isFind :=0;
