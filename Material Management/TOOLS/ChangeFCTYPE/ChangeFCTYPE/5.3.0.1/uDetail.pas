...
begin
 ...
 // editid.SetFocus;
  ...
  begin
    ...
    Params.ParamByName('dll_name').AsString := 'CHANGEFCTYPEDLL.DLL';
    Open;
  //DateTimePicker1.DateTime:=now();
 // editFifo.Text := GetFIFOCode(now());
  end;
         ...
         with QryMaterial do
           begin
              ...
              CommandText := 'SELECT NVL(B.RT_NO,''N/A'') AS Rt_NO,C.PART_NO,A.DATECODE,A.MATERIAL_NO,A.MATERIAL_QTY,NVL(A.REEL_NO,''N/A'') AS REEL_NO,  '
                ...
                +  'NVL(C.OPTION13,''N'') AS INDATEFLAG ,NVL(C.OPTION14,''0'') AS INDATECODE,A.FACTORY_ID,A.FACTORY_TYPE '
                 ...
                 end
             {
               ...
               lblfctype.Caption :=fieldbyname('FACTORY_TYPE').AsString;
...
begin
if trim(editfctype.Text )='' then
  begin
    MessageDlg('Please select FCTYPE', mtError, [mbOK], 0);
    Exit;
  end;

if lblfctype.Caption = editfctype.Text  then
 begin
    MessageDlg('NOT CHANGE FCTYPE ', mtError, [mbOK], 0);
    Exit;
  end;

if MessageDlg('Change FCTYPE,Are you Sure?', mtCustom, mbOKCancel, 0) = mrOK then
   BEGIN
     with QryMaterial do
          begin
              Close;
              ...
              CommandText := 'update  sajet.g_material '
                    +' set update_userid=:update_userid,UPDATE_TIME=SYSDATE, '
                    +' FACTORY_ID=:FACTORY_ID,FACTORY_TYPE=:FACTORY_TYPE ' ;
