...
type
  ...
  public
    ...
    Function GetFIFOCodeDIFF(FIFO1,FIFO2:string):Integer;
...
Function TfDetail.GetFIFOCodeDIFF(FIFO1,FIFO2:string):Integer;
begin
  with sproc do
    begin
      try
        Close;
        DataRequest('SAJET.SJ_GET_FIFO_DIFF');
        FetchParams;
        Params.ParamByName('TFIFOCODE1').AsString := FIFO1;
        Params.ParamByName('TFIFOCODE2').AsString := FIFO2;
        Execute;
        IF Params.ParamByName('TRES').AsString='OK' THEN
          RESULT:=Params.ParamByName('FIFODIFF').AsInteger
        else
          Showmessage(Params.ParamByName('TRES').AsString);
      finally
        close;
...
begin
 ...
 // editid.SetFocus;
  ...
  begin
    ...
    Params.ParamByName('dll_name').AsString := 'CHANGEFIFODLL.DLL';
    Open;
...
VAR RESULT,sfifo1,sfifo2: STRING;
ififodiff :Integer;
begin
         ...
         with QryMaterial do
           begin
              ...
              CommandText := 'SELECT NVL(B.RT_NO,''N/A'') AS Rt_NO,C.PART_NO,A.DATECODE,A.MATERIAL_NO,A.MATERIAL_QTY,NVL(A.REEL_NO,''N/A'') AS REEL_NO,  '
                +  'A.REEL_QTY,D.WAREHOUSE_NAME,e.locate_name,NVL(A.REMARK,''N/A'') AS REMARK,A.MFGER_NAME,A.MFGER_PART_NO,A.TYPE, to_char(to_date(A.DATECODE,''YYYYMMDD''),''YYIW'') DATEFIFOCODE,A.FIFOCODE,  '
              ...
              if IsEmpty then
                ...
                begin
                  MessageDlg('Not Incomming', mtError, [mbOK], 0);
                  IF sbtconfirm.Enabled =true then
                       sbtconfirm.Enabled :=false;
                  EDITID.SelectAll ;
                  EDITID.SetFocus ;
               ...
               BEGIN
                 ...
                 begin
                     sfifo1 := GetFIFOCode(now());
                     sfifo2 :=fieldbyname('DATEFIFOCODE').AsString;
                     ififodiff :=GetFIFOCodeDIFF(sfifo1,sfifo2);
                     IF ififodiff  >= fieldbyname('INDATECODE').AsInteger then
                     begin
...
begin
if MessageDlg('Change FIFOCODE TO '+editfifo.Text+'  ,Are you Sure?', mtCustom, mbOKCancel, 0) = mrOK then
   BEGIN
     with QryMaterial do
          begin
              Close;
              ...
              CommandText := 'update  sajet.g_material '
                    +' set DateCode =:datecode,update_time=sysdate,update_userid=:update_userid,fifocode=:fifocode ' ;
              ...
              showmessage('FIFOCODE UPDATE OK!');
              sbtconfirm.Enabled:=true;
              editid.SelectAll ;
              editid.SetFocus ;
...
procedure TfDetail.dtp1Change(Sender: TObject);
begin
  EDITDATECODE.Text := FormatDateTime('YYYYMMDD',dtp1.DateTime) ;
end;
