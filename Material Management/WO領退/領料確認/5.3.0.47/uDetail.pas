...
type
  TfDetail = class(TForm)
    ...
    ImageADDseq: TImage;
    Sbtnaddseq: TSpeedButton;
    Label12: TLabel;
    EditORG: TEdit;
    lablmaterialqty: TLabel;
    chkMaterialqty: TCheckBox;
    ...
    procedure own(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure SbtnaddseqClick(Sender: TObject);
    procedure LabTitle1DblClick(Sender: TObject);
    procedure chkMaterialqtyMouseDown(Sender: TObject;
...
begin
  ...
  QryTemp.CommandText := 'select reel_no, material_no, part_no, b.VERSION, b.' + gsOverField + ', option7, b.part_type,a.factory_id '
...
begin
  ...
  begin
    ...
    begin   //琩高畐
         if chkmaterialqty.Checked then
            CommandText := 'Select a.*, b.part_no, b.part_TYPE, b.' + gsOverField + ', c.qty, warehouse_name, locate_name '
             ...
             CommandText := 'Select a.*, b.part_no, b.part_TYPE, b.' + gsOverField + ', ''0'' as qty, warehouse_name, locate_name '
             + ' From SAJET.G_Wo_Pick_List a, sajet.sys_part b, sajet.sys_locate d, sajet.sys_warehouse e '
...
//  LabTitle2.Caption := LabTitle1.Caption;
  ...
  begin
    ...
    lablmaterialqty.Visible :=true;
    chkmaterialqty.Visible :=true;
    chkmaterialqty.Checked :=false;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Params.Clear;
      Params.CreateParam(ftString, 'work_order', ptInput);
      CommandText := 'Select target_qty, default_pdline_id, pdline_name, wo_status,a.factory_id ' +
    ...
    begin
      ...
      G_FCTYPE:='';
      editorg.Clear ;
      G_FCID:=QryTemp.fieldbyname('factory_id').AsString;
      if Getfctype(G_FCID)<>'OK' then
      begin
          MessageDlg('WO '+ edtwo.text+' ORG IS ERROR', mtError, [mbOK], 0);
          editorg.Clear;
          edtWo.SelectAll;
          edtWo.SetFocus;
          exit;
...
procedure TfDetail.own(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfDetail.SbtnaddseqClick(Sender: TObject);
var SINFOSEQ,SLISTSEQ:STRING;
begin
  if trim(edtwo.Text ) = '' then
  begin
    MessageDlg('Please Input WORK_ORDER', mtError, [mbOK], 0);
    Edtwo.SetFocus;
    Exit;
  end;
  if combType.Text = 'ALL' then
...
with QryTemp do
  begin
    ...
    Params.CreateParam(ftString, 'work_order', ptInput);
    Params.CreateParam(ftString, 'part_type', ptInput);
    CommandText := 'select  MAX(sequence) AS sequence from sajet.g_wo_pick_info '
      + 'where work_order = :work_order and sequence like :part_type ';
    Params.ParamByName('work_order').AsString := edtWo.Text;
    Params.ParamByName('part_type').AsString := combType.Text + '%';
    Open;
    SINFOSEQ:='XX';
    if not IsEmpty then
      SINFOSEQ:=FIELDBYNAME('SEQUENCE').AsString ;
    Close;
    Params.Clear;
    Params.CreateParam(ftString, 'work_order', ptInput);
    Params.CreateParam(ftString, 'part_type', ptInput);
    CommandText := 'select  MAX(sequence) AS sequence from sajet.g_pick_LIST '
      + 'where work_order = :work_order and sequence like :part_type ';
    Params.ParamByName('work_order').AsString := edtWo.Text;
    Params.ParamByName('part_type').AsString := combType.Text + '%';
    Open;
    slistseq:='XX' ;
    IF not isempty then
       SLISTSEQ:= FIELDBYNAME('SEQUENCE').AsString ;
    CLOSE;
    IF SINFOSEQ=SLISTSEQ THEN
       BEGIN
          MessageDlg('NOT ADD SEQ!', mtError, [mbOK], 0);
          IF IMAGEADDSEQ.Visible =TRUE THEN
              IMAGEADDSEQ.Visible :=FALSE;
          IF SBTNADDSEQ.Visible =TRUE THEN
              SBTNADDSEQ.Visible :=FALSE;
          exit;
       END;
    IF ((SINFOSEQ='XX') AND (SLISTSEQ<>'XX')) OR ((SLISTSEQ<>'XX') AND (SLISTSEQ>SINFOSEQ)) THEN
       SINFOSEQ:=SLISTSEQ ;
     Close;
     Params.Clear;
     Params.CreateParam(ftString, 'work_order', ptInput);
     Params.CreateParam(ftString, 'pick_qty', ptInput);
     Params.CreateParam(ftString, 'sequence', ptInput);
     CommandText := 'insert into sajet.g_wo_pick_info '
        + '(work_order, pick_qty, sequence, add_userid, add_time) '
        + 'values (:work_order, :pick_qty, :sequence, '''+UpdateUserid+''', sysdate)';
     Params.ParamByName('work_order').AsString := edtWo.Text;
     Params.ParamByName('pick_qty').AsString :='0';
     Params.ParamByName('sequence').AsString :=SINFOSEQ;
     Execute;
     MessageDlg(SINFOSEQ+' ADD OK!', mtInformation, [mbOk], 0);
...
procedure TfDetail.LabTitle1DblClick(Sender: TObject);
begin
     IF IMAGEADDSEQ.Visible =FALSE THEN
        IMAGEADDSEQ.Visible :=TRUE;
     IF SBTNADDSEQ.Visible =FALSE THEN
        SBTNADDSEQ.Visible :=TRUE;
END;

procedure TfDetail.chkMaterialqtyMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
      chkMaterialqty.Checked := not (chkMaterialqty.Checked);
      MessageDlg('Material Qty Change OK', mtError, [mbOK], 0)
end;
