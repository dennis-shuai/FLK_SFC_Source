...
type
   ...
   TfMaterial = class(TForm)
    ...
    edtWOMap: TEdit;
    Label20: TLabel;
    cmbSideMap: TComboBox;
    cmbMachineMap: TComboBox;
    Label21: TLabel;
    cmbPDLineMap: TComboBox;
    Label22: TLabel;
    sgMap: TStringGrid;
    StatusBar7: TStatusBar;
    qrymap: TClientDataSet;
    lbl1: TLabel;
...
Procedure TfMaterial.selectMap;
var iRow,i:integer;
    //T1:tDate;
begin
  //T1:= Time;
  ...
  begin
     ...
     begin
         ...
         commandtext:=  ' select a.wo_sequence,b.pdline_name,c.part_no,d.emp_name,to_char(a.in_time,''yyyy/mm/dd hh24:mi:ss'') in_time '
                                +' ,to_char(a.out_time,''yyyy/mm/dd hh24:mi:ss'') out_time,a.station_no,a.feeder_no,a.reel_no,e.vendor_name,a.Lot_no,a.datecode,a.MFGER_NAME,a.MFGER_PART_NO   '
                       +'  from smt.g_smt_status a,sajet.sys_pdline b,sajet.sys_part c,sajet.sys_emp d,sajet.sys_vendor e '
                       +'  where a.WO_sequence like ''%'+ edtWOMap.Text+''' '
                       +'       and a.pdline_id=b.pdline_id(+) '
                       +'       and a.item_part_id=c.part_id '
                       +'       and a.emp_id = d.emp_id(+) '
                       +'       and a.vendor_id=e.vendor_id(+) '
                       +' union '
                       +' select a.wo_sequence,b.pdline_name,c.part_no,d.emp_name,to_char(a.in_time,''yyyy/mm/dd hh24:mi:ss'') in_time '
                                    +' ,to_char(a.out_time,''yyyy/mm/dd hh24:mi:ss'') out_time,a.station_no,a.feeder_no,a.reel_no,e.vendor_name,a.Lot_no,a.datecode,a.MFGER_NAME,a.MFGER_PART_NO   '
        ...
        begin
           ...
           while not eof do
           begin
             sgMap.RowHeights[iRow]:=16;
             sgMap.Cells[1,iRow]:=fieldbyname('wo_sequence').AsString;
             sgMap.Cells[2,iRow]:=fieldbyname('part_no').AsString;
             sgMap.Cells[3,iRow]:=fieldbyname('pdline_name').AsString;
            // sgMap.Cells[4,iRow]:=fieldbyname('MACHINE_CODE').AsString;
             //sgMap.Cells[5,iRow]:=fieldbyname('side').AsString;
             sgMap.Cells[4,iRow]:=fieldbyname('station_no').AsString;
             sgMap.Cells[5,iRow]:=fieldbyname('feeder_no').AsString;
             sgMap.Cells[6,iRow]:=fieldbyname('reel_no').AsString;
             sgMap.Cells[7,iRow]:=fieldbyname('emp_name').AsString;
             sgMap.Cells[8,iRow]:=fieldbyname('in_time').AsString;
             sgMap.Cells[9,iRow]:=fieldbyname('out_time').AsString;
             sgMap.Cells[10,iRow]:=fieldbyname('vendor_name').AsString;
             sgMap.Cells[11,iRow]:=fieldbyname('Lot_no').AsString;
             sgMap.Cells[12,iRow]:=fieldbyname('datecode').AsString;
           //  sgMap.Cells[11,iRow]:=fieldbyname('qty').AsString;

             sgMap.Cells[13,iRow]:=fieldbyname('MFGER_NAME').AsString;
             sgMap.Cells[14,iRow]:=fieldbyname('MFGER_PART_NO').AsString;
             INC(IROW);
             sgMap.RowCount:=iRow+1;
...
begin
  ...
  begin
    close;
    params.Clear;
    commandtext:=' select SUBSTR(wo_sequence,3,length(wo_sequence)) work_order,g.pdline_name,b.stage_name,c.process_name,d.part_no,a.Lot_no,a.datecode,f.vendor_name,a.qty,to_char(in_time,''YYYY/MM/DD HH24:MI:ss'') in_time '
    ...
    begin
      ...
      begin
        ...
        sgDIP.Cells[6,iRow]:= fieldbyname('Lot_no').AsString;
        sgDIP.Cells[7,iRow]:= fieldbyname('datecode').AsString;
        sgDIP.Cells[8,iRow]:= fieldbyname('vendor_name').AsString;
        sgDIP.Cells[9,iRow]:= fieldbyname('QTY').AsString;
        sgDIP.Cells[10,iRow]:= fieldbyname('in_time').AsString;
        sgDIP.Cells[11,iRow]:= fieldbyname('emp_name').AsString;
        sgDIP.Cells[12,iRow]:= fieldbyname('mfger_name').AsString;
        sgDIP.Cells[13,iRow]:= fieldbyname('mfger_part_no').AsString;
        inc(iRow);
        next;
      end;
...
begin
  ...
  begin
    ...
    BEGIN
     ...
     commandtext:=' select a1.work_order,decode(to_char(a1.side),''0'',''Top'',''1'',''Bottom'') side,b1.station_no,d1.part_no,c1.Lot_no,f1.pdline_name,c1.datecode,TO_CHAR(c1.qty) QTY,g1.emp_name,To_CHAR(c1.in_time,''YYYY/MM/DD HH24:MI:SS'') IN_TIME,TO_CHAR(c1.item_part_id) flag '
                +' ,c1.reel_no,c1.MFGER_NAME,c1.MFGER_PART_NO,H1.MACHINE_CODE,''Y'' IS_sub_part  '
                +' from smt.g_wo_msl a1,( select b3.station_no,b3.wo_sequence,b3.item_part_id,b2.sub_part_id  from  smt.g_wo_msl_detail b3,'
                + ' smt.g_wo_msl_sub b2 where b3.wo_sequence= b2.wo_sequence(+) and b3.item_part_id =b2.item_Part_Id(+) ) b1, '
                +' smt.g_smt_status  c1,sajet.sys_part d1,sajet.sys_pdline f1 ,sajet.sys_emp g1,sajet.sys_machine h1  '
                +' where a1.wo_sequence=b1.wo_sequence '
                +'      and b1.sub_part_id=d1.part_id '
                +'      and c1.pdline_id = f1.pdline_id(+) '
                +'      and c1.emp_id = g1.emp_id(+) '
                +'      and b1.wo_sequence=c1.wo_sequence '
                +'      and  b1.sub_part_id =c1.item_part_id   '
      ...
      commandtext := commandtext
                +' Union '
                +' select a1.work_order,decode(to_char(a1.side),''0'',''Top'',''1'',''Bottom'') side,b1.station_no,d1.part_no,c1.Lot_no,f1.pdline_name,c1.datecode,TO_CHAR(c1.qty) QTY,g1.emp_name,To_CHAR(c1.in_time,''YYYY/MM/DD HH24:MI:SS'') IN_TIME,TO_CHAR(c1.item_part_id) flag '
                +' ,c1.reel_no,c1.MFGER_NAME,c1.MFGER_PART_NO,H1.MACHINE_CODE, ''N'' Is_sub_part  '
                +' from smt.g_wo_msl a1, smt.g_wo_msl_detail b1,'
                +' smt.g_smt_status  c1,sajet.sys_part d1,sajet.sys_pdline f1 ,sajet.sys_emp g1,sajet.sys_machine h1  '
                +' where a1.wo_sequence=b1.wo_sequence '
      ...
      commandtext:=commandtext
                +' union '
                +' select a2.work_order,decode(to_char(a2.side),''0'',''Top'',''1'',''Bottom'') side,b2.station_no,d2.part_no,'''' Lot_No,'''' pdline_name,'''' datecode,'''' qty,'''' emp_name,'''' in_time,'''' flag '
                +'   ,'''' reel_no,'''' MFGER_NAME,'''' MFGER_PART_NO,H2.MACHINE_CODE,''N'' Is_Sub_Part  '
     ...
     begin
      ...
      begin
          ...
          begin
           ...
           sgOnline.Cells[10,iRow]:=fieldbyname('Lot_No').AsString;
           sgOnline.Cells[11,iRow]:=fieldbyname('datecode').AsString;
           sgOnline.Cells[12,iRow]:=fieldbyname('qty').AsString;

           sgOnline.Cells[13,iRow]:=fieldbyname('MFGER_NAME').AsString;
           sgOnline.Cells[14,iRow]:=fieldbyname('MFGER_PART_NO').AsString;
           sgOnline.Cells[15,iRow]:=fieldbyname('Is_sub_part').AsString;
           INC(IROW);
    ...
    ELSE BEGIN
     ...
     commandtext:=' select a1.work_order,b1.station_no,decode(to_char(a1.side),''0'',''Top'',''1'',''Bottom'') side,d1.part_no,c1.Lot_no,f1.pdline_name,c1.datecode,TO_CHAR(c1.qty) QTY,g1.emp_name,To_CHAR(c1.in_time,''YYYY/MM/DD HH24:MI:SS'') IN_TIME,TO_CHAR(c1.item_part_id) flag '
...
begin
  ...
  begin
    ...
    if cmbPart.Items.Count=0 then begin
       ...
       begin
         ...
         params.Clear;
         params.CreateParam(ftString	,'iWO', ptInput);
         commandtext:='select * FROM ( select distinct c.Part_no from smt.g_wo_msl a,smt.g_wo_msl_detail b,sajet.sys_part c  '
                 +' where a.wo_sequence=b.wo_sequence '
                 +'       and a.work_order =:iWO '
                 +'       and b.item_part_id=c.part_id '
                 +' union '
                 +' select distinct c.Part_no from smt.g_wo_msl a,smt.g_wo_msl_detail b,sajet.sys_part c ,smt.g_wo_msl_sub d'
                 +' where a.wo_sequence=b.wo_sequence '
                 +'       and b.wo_sequence =d.wo_sequence '
                 +'       and d.item_part_id = b.item_part_id '
                 +'       and a.work_order =:iWO '
                 +'       and d.sub_part_id= c.part_id )'
                 +' order by part_no ';
  ...
  begin
    selectBetch;
  end else if PageControl1.ActivePage=TabSHEET7 then
    SelectMap;
end;
...
begin
  ...
  sgOnline.Cells[10,0]:='Lot No';
  sgOnline.Cells[11,0]:='DATECODE';
  sgOnline.Cells[12,0]:='QTY';

  sgOnline.Cells[13,0]:='MFGER NAME';
  sgOnline.Cells[14,0]:='MFGER PART NO';
  ...
  sgDIP.Cells[6,0]:='Lot Code' ;
  sgDIP.Cells[7,0]:='DATECODE';
  sgDIP.Cells[8,0]:='Vendor Name';
  sgDIP.Cells[9,0]:='QTY';
  sgDIP.Cells[10,0]:='In Time';
  sgDIP.Cells[11,0]:='Emp Name';
  sgDIP.Cells[12,0]:='MFGER Name';
  sgDIP.Cells[13,0]:='MFGER Part NO';
  ...
  sgMap.Cells[1,0]:='WORK ORDER';
  sgMap.Cells[2,0]:='PART NO';
  sgMap.Cells[3,0]:='PDLINE';
  sgMap.Cells[4,0]:='MACHINE';
  sgMap.Cells[5,0]:='STATION';

  sgMap.Cells[6,0]:='REEL NO';
  sgMap.Cells[7,0]:='EMP NAME';
  sgMap.Cells[8,0]:='IN TIME';
  sgMap.Cells[9,0]:='OUT TIME';

  sgMap.Cells[10,0]:='vendor_name';
  sgMap.Cells[11,0]:='Lot_No';
  sgMap.Cells[12,0]:='datecode';

  sgMap.Cells[13,0]:='MFGER NAME';
  sgMap.Cells[14,0]:='MFGER PART NO';
...
begin
  ...
  if ARow=0 then exit;
  if (sgOnline.Cells[14,Arow]='Y') then
  begin
     sgOnline.Canvas.Brush.Color:=clYellow;
     sgOnline.Canvas.TextRect(Rect, Rect.Left+5 , Rect.Top+2, sgOnline.Cells[ACol, ARow]);
  end
  else if (sgOnline.Cells[9,Arow]='') and (Arow<>dateRow) then
  begin
...
procedure TfMaterial.sgMapDrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);
Var K : Integer;
begin
if (ACol<=0) then exit;
  ...
  if (ARow=0) and (ACol<k) then exit ;
  if ARow=0 then exit;
  if (sgMap.Cells[3,Arow]='') and (sgMap.Cells[9,Arow]='') and (Arow<>dateRow) then
  begin
    sgMap.Canvas.Brush.Color:=clWindow;
    sgMap.Canvas.TextRect(Rect, Rect.Left+5 , Rect.Top+2, sgMap.Cells[ACol, ARow]);
  end
  else if  Arow=dateRow then
  begin
    sgMap.Canvas.Brush.Color:=$ED9564;
    sgMap.Canvas.TextRect(Rect, Rect.Left+5 , Rect.Top+2, sgMap.Cells[ACol, ARow]);
  end
  else begin
    sgMap.Canvas.Brush.Color:=$578B2E;//$FFF8F0;//clgreen; ED9564
    sgMap.Canvas.TextRect(Rect, Rect.Left+5 , Rect.Top+2, sgMap.Cells[ACol, ARow]);
  end;
end;
