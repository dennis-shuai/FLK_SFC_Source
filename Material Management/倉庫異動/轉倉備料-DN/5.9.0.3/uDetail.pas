...
function TfDetail.CheckPallet: Boolean;
//var sPartNo: string;
begin
  Result := False;
  with QryTemp do
  begin
    ...
    Params.ParamByName('pallet_no').AsString := edtmaterial.Text;
    Open;
    if IsEmpty then
    begin
      MessageDlg('Pallet: ' + edtmaterial.Text + ' not found!', mtError, [mbOK], 0);
      Close;
      Exit;
    end
    else if FieldByName('close_flag').AsString = 'N' then
    begin
      MessageDlg('Pallet: ' + edtmaterial.Text + ' Unfinished!', mtError, [mbOK], 0);
      Close;
      Exit;
    end;
    //sPartNo := FieldByName('part_no').AsString;
    ...
    begin
      ...
      begin
        MessageDlg('Pallet: ' + edtmaterial.Text + ' is Empty!', mtError, [mbOK], 0);
        Close;
        Exit;
      end;
      sedtQty.Value := 0;
...
BEGIN
 if Ord(Key) = vk_Return then
  begin
    ...
    //如果是F開頭的就先合批    Add by Xiaobo_Yuan 20110705
    if Copy(edtmaterial.Text,1,1)='P'  then
    begin
      with QryTemp do
      begin
        try
          ...
          params.Clear;
          Params.CreateParam(ftString, 'MATERIAL_NO', ptInput);
          commandtext:=' select MATERIAL_NO from SAJET.G_MATERIAL  WHERE    '
                      +' MATERIAL_NO in (select distinct carton_no from '
                      +'    sajet.g_sn_status where pallet_no =:MATERIAL_NO)' ;
          Params.ParamByName('MATERIAL_NO').AsString := edtmaterial.Text ;
          Open ;
          if isempty then
          begin
      ...
      begin
        try
          ...
          commandtext:=' update sajet.g_material '
                      ...
                      +'    ,update_time=sysDate '
                      +'    ,remark=:material_no '
                      +'  WHERE MATERIAL_NO IN(SELECT CARTON_NO FROM SAJET.G_SN_STATUS '
                      +'  WHERE PALLET_NO=:material_no) ';
          Params.ParamByName('MATERIAL_NO').AsString := edtmaterial.Text ;
          Params.ParamByName('userid').AsString :=UpdateUserID;
          Execute;
          ...
          Params.CreateParam(ftString, 'material_no', ptInput);
          CommandText := 'insert into sajet.g_ht_material '
            + 'select distinct a.* from sajet.g_material a,sajet.g_sn_status b '
            + 'where A.MATERIAL_NO =b.carton_no and B.PALLET_NO =:material_no ';
          Params.ParamByName('material_no').AsString := edtmaterial.Text ;
          Execute;
          ...
          params.Clear;
          Params.CreateParam(ftString, 'MATERIAL_NO', ptInput);
          Params.CreateParam(ftString, 'sedtQty', ptInput);
          Params.CreateParam(ftString, 'userid', ptInput);
          commandtext:=' insert into sajet.g_material   '
                      +' SELECT A.RT_ID,A.PART_ID,A.DATECODE,:MATERIAL_NO,:sedtQty,REEL_NO, '
                      +'    REEL_QTY,STATUS,LOCATE_ID,WAREHOUSE_ID,:userid,SYSDATE,A.REMARK,RELEASE_QTY, '
                      +'     A.VERSION,MFGER_NAME,MFGER_PART_NO,RT_SEQ,''M'',FIFOCODE,FACTORY_ID,FACTORY_TYPE  '
                      +'      FROM SAJET.G_MATERIAL A,SAJET.G_SN_STATUS B '
                      +'       WHERE B.PALLET_NO=:MATERIAL_NO AND A.MATERIAL_NO=B.CARTON_NO AND ROWNUM=1 ';
          Params.ParamByName('MATERIAL_NO').AsString := edtmaterial.Text ;
          Params.ParamByName('sedtQty').AsString := sedtQty.Text ;
          Params.ParamByName('userid').AsString :=UpdateUserID;
          Execute;
          ...
          CommandText := 'delete from sajet.g_material '
            ...
            + 'SELECT distinct CARTON_NO FROM SAJET.G_SN_STATUS  WHERE PALLET_NO=:material_no)' ;
          Params.ParamByName('material_no').AsString := edtmaterial.Text;
          Execute;
        except
           MessageDlg('Execut SQL Fail!', mtError, [mbOK], 0);
           edtmaterial.SelectAll ;
           edtmaterial.SetFocus ;
           exit;
        end;
