      ...
      begin
          ...
          begin
              ...
              begin
                ...
                Params.CreateParam(ftString, 'material_no', ptInput);
                CommandText := 'select distinct material_no, material_qty, a.datecode, part_no, '
                  + 'nvl(b.label_type, c.' + gsLabelField + ') label_type, release_qty, '
                  + ' a.locate_id,a.part_id,warehouse_id,a.FIFOCode,a.type '
  ...
  begin
    ...
    begin
      ...
      CommandText := 'insert into sajet.g_material '
        + ' (rt_id, part_id, datecode, material_no, material_qty, reel_no, reel_qty,'
        + ' update_userid, locate_id, warehouse_id, status,REMARK, MFGER_NAME, MFGER_PART_NO,'
        + '  version,rt_seq,FIFOCode,factory_id,factory_type ) '
        + ' select rt_id, part_id,:DC,  material_no, material_qty, :reel_no, :reel_qty, :update_userid, locate_id, '
        + ' warehouse_id, status,REMARK, MFGER_NAME, MFGER_PART_NO, version,rt_seq,:FIFO ,'
        + ' factory_id,factory_type from ' + sTable + ' where material_no = :material_no and rownum = 1 ';
      Params.ParamByName('reel_no').AsString := sReel;
      Params.ParamByName('reel_qty').AsInteger := iTemp;
      Params.ParamByName('update_userid').AsString := UpdateUserid;
      Params.ParamByName('material_no').AsString := QryMaterial.FieldByName('material_no').AsString;
