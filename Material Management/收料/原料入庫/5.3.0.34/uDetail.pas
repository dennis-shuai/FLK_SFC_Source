...
begin
  ...
  begin
    ...
    begin
     ...
     commandtext:= ' select material_no,material_qty,sum(reel_qty) reel_qty, part_no, status ,a.part_id ,option7,a.rt_seq,a.fifocode,a.factory_id  '
                  +' from sajet.g_material a,sajet.sys_part b '
                  + 'where a.rt_id = :rt_id and material_no = :material_no and a.part_id = b.part_id '
                  + 'group by material_no, material_qty, part_no, status, a.part_id, option7,rt_seq,fifocode,factory_id ';
      ...
      begin
        ...
        begin
          ...
          if combStock.ItemIndex <> -1 then
            StringGrid1.Cells[7, iRow] := slStockId[combStock.ItemIndex];
...
begin
  ...
  begin
    cmbStock.ItemIndex := slStockId.IndexOf(StringGrid1.Cells[7, giRow]);
    cmbStockChange(Self);
...
begin
  ...
  begin
    ...
    CommandText := 'select locate_id, locate_name from sajet.sys_locate a '
      + 'where a.warehouse_id = :warehouse_id and enabled = ''Y'' ';
...
begin
  ...
  for i := 1 to StringGrid1.RowCount - 1 do
    ...
    begin
      ...
      if combStock.ItemIndex <> -1 then
        StringGrid1.Cells[7, i] := slStockId[combStock.ItemIndex];
      if combLocate.ItemIndex <> -1 then
