  ...
  begin
    ...
    begin
      ...
      begin
          ...
          Params.Clear;
          Params.CreateParam(ftString, 'rt_id', ptInput);
          CommandText := ' SELECT distinct nvl(warehouse_id,0) wh_id FROM SAJET.G_MATERIAL  where RT_ID =:rt_id  ';
          Params.ParamByName('rt_id').AsString := gsRTID;
          Open;
          if IsEmpty then begin
             MessageDlg('沒有該單號,IQC是否有檢驗？',mtError,[mbOK],0);
             Exit;
          end;
          {
          sType := FieldByName('Type').AsString ;
          if sType ='S'  then begin
             MessageDlg('裡面物料已經被拆分',mtError,[mbOK],0);
             Exit;
          end;

          if sType ='M'  then begin
             MessageDlg('裡面物料已經被合併',mtError,[mbOK],0);
             Exit;
          end;

          if sType ='O'  then begin
             MessageDlg('裡面物料已經發料',mtError,[mbOK],0);
             Exit;
          end;
          }
          swhId := FieldByName('wh_id').AsString ;

          if  swhId='0' then begin
             MessageDlg('沒有設置默認倉別',mtError,[mbOK],0);
             Exit;
          end;
