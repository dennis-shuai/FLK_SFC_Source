  ...
  for i := 1 to sedtBoxQty.Value do
    ...
    begin
      ...
      CommandText := 'insert into sajet.g_material '
        + '(rt_id, part_id, datecode, material_no, material_qty, update_userid, MFGER_NAME, MFGER_PART_NO, version,RT_seq,type) '
        + 'values (:rt_id, :part_id, :datecode, :material_no, :material_qty, :update_userid, :MFGER_NAME, :MFGER_PART_NO, :version,:rt_seq,''P'')';
      Params.ParamByName('rt_id').AsString := QryDetail.FieldByName('rt_id').AsString;
      Params.ParamByName('PART_ID').AsString := QryDetail.FieldByName('Part_ID').AsString;
