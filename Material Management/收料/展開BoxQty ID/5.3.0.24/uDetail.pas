...
begin
  sSQL := 'Select A.ROWID, A.RT_ID, A.PART_ID, B.PART_NO, A.PART_VERSION, A.DATECODE, A.INCOMING_QTY, LOT_NO,A.RT_SEQ, ' +
    '       A.VENDOR_LOTNO, A.VENDOR_PARTNO, A.INCOMING_TIME, LABEL_TYPE, PRINT_QTY, a.MFGER_NAME, a.MFGER_PART_NO,C.factory_id  ' +
  ...
  for i := 1 to sedtBoxQty.Value do
    ...
    begin
      ...
      CommandText := 'insert into sajet.g_material '
        + '(rt_id, part_id, datecode, material_no, material_qty, update_userid, MFGER_NAME, MFGER_PART_NO, version,RT_seq,type,FIFOCODE,factory_ID,factory_type) '
        + 'values (:rt_id, :part_id, :datecode, :material_no, :material_qty, :update_userid, :MFGER_NAME, :MFGER_PART_NO, :version,:rt_seq,''P'',:FIFOCODE,:factory_id,:factory_type)';
      Params.ParamByName('rt_id').AsString := QryDetail.FieldByName('rt_id').AsString;
      Params.ParamByName('PART_ID').AsString := QryDetail.FieldByName('Part_ID').AsString;
