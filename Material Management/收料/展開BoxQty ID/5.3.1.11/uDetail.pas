...
begin
  ...
  DateTimePicker1.DateTime:=GetSysDate;
  edtFifo.Text:=getfifocode(GetSysDate);
end;
...
begin
  ...
  begin
   ...
   { if lablType.Caption = '' then
    ...
    if StrToIntDef(FieldByName(gsBoxField).AsString, 0) = 0 then
    begin
       MessageDlg('該料號沒設置默認每箱數量',mterror,[mbok],0);
       Exit;
    end;
 ...
 // edtDateCode.Text:= QryDetail.FieldByName('DATECODE').AsString;
  try
     DateTimePicker1.Date := QryDetail.FieldByName('DATECODE').AsDateTime;
  except
      DateTimePicker1.Date :=GetSysDate;
 ...
 { if trim(edtDateCode.Text)='' then
  ...
  for i := 1 to sedtBoxQty.Value do
    ...
    begin
      ...
      Close;
      Params.Clear;
      CommandText := 'select * from sajet.SYS_WAREHOUSE where WAREHOUSE_NAME ='''+sPrefixWH+'''';
      Open;

      if IsEmpty then begin
         MessageDlg('採購沒有設置預設倉別',mtError,[mbOK],0);
         exit;
      end;

      sPrefixWHID := FieldByName('WAREHOUSE_ID').AsString;;
      ...
      {if edtDateCode.Text = '' then
        ...
        Params.ParamByName('datecode').AsString := QryDetail.FieldByName('datecode').AsString;}
      Params.ParamByName('datecode').AsString :=FormatDateTime('YYYYMMDD',DateTimePicker1.Date);
      ...
      //add wahouse 預設
      Params.ParamByName('Warehouse').AsString := sPrefixWHID;
