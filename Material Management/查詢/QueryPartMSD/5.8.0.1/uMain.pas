...
BEGIN
   ...
   stringgrid1.Cells[7,0]:='Floor life(M)';
...
begin
   ...
   strsql:=' select e.work_order,d.pdline_name,f.machine_code,b.part_no,a.reel_no,   '
          +' a.in_time,c.floor_life*60 as floor_life, '
          +' c.floor_life*60 - nvl(substr((sysdate - a.in_time)*60*60,0,(instr((sysdate - a.in_time)*60*60,''.'' )-1)),1) as open_time,  '
   ...
   begin
     try
     if strtoint(editopentime.Text)>0  then
        strsql:=strsql+' and  c.floor_life*60 - nvl(substr((sysdate - a.in_time)*60*60,0,(instr((sysdate - a.in_time)*60*60,''.'' )-1)),1)  <= '+''''+trim(editopentime.Text)+'''';
     except
