...
begin
  ...
  begin
   ...
   commandtext:=' select part_no,update_time,type,locator,material_no,material_qty,reel_no,reel_qty,emp_name from ( ';
   commandtext:= commandtext
               +' select b1.part_no,to_char(a1.update_time,''YYYY/MM/DD HH24:Mi:SS'') update_time,decode(a1.type,''I'',''INPUT'',''O'',''OUTPUT'',''S'',''SPLIT'',''M'',''MERGE'',''P'',''PRINT'',''UNKNOW'') type '
               +'      ,c1.warehouse_name||''-''||d1.locate_name locator,a1.material_no,a1.material_qty,a1.reel_no,a1.reel_qty,e1.emp_name '
   ...
   commandtext:=commandtext
               +' union '
               +' select b2.part_no,to_char(a2.update_time,''YYYY/MM/DD HH24:Mi:SS'') update_time,decode(a2.type,''I'',''INPUT'',''O'',''OUTPUT'',''S'',''SPLIT'',''M'',''MERGE'',''P'',''PRINT'',''UNKNOW'') type '
               +'      ,c2.warehouse_name||''-''||d2.locate_name locator,a2.material_no,a2.material_qty,a2.reel_no,a2.reel_qty,e2.emp_name '
   ...
   commandtext:=commandtext+ 'and to_char(a2.update_time,''YYYYMMDD'') between '+FormatDateTime('YYYYMMDD',DTStart.Date)+'  and '
                          +FormatDateTime('YYYYMMDD',DTEnd.Date);
   commandtext:=commandtext+' )  group by material_no,reel_no,type,part_no,material_qty,reel_qty,update_time,locator,emp_name '
