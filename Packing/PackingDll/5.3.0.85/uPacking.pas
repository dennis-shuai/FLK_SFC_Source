...
type
  ...
  public
    { Public declarations }
    mWO, mPartID, mPartNo, gSNWo, gSN, gsCheckSum, mLabelFile, mCustPartNo,m_boxqty: string;
    mCarry: string;
    mDateCode: string;
    ...
    g_bAdditional,g_bLoadAdditional,g_bCheckBOxFull,g_bPackByWo:Boolean;
    ...
    function GetPackingByWO:Boolean;
    procedure CreateBox;
    ...
    // 2010.10.21 add by xiaobo_yuan 新增cheetah掃描50 begin
    function ApartSN(Tobj :TEdit;Aobject:TListBox): Boolean ;
    // 2010.10.21 add by xiaobo_yuan 新增cheetah掃描50 end
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if Fieldbyname('PARAME_ITEM').AsString = 'Check Box Full' then
        g_bCheckBoxFull := (Fieldbyname('PARAME_VALUE').AsString = 'Y')
...
begin
  ...
  begin
    ...
    begin
      ...
      sbtnCloseBox.Visible := False;
      Image2.Visible := False;
      if not g_bCheckBoxFull then  begin
          labBCap.Visible := False;
          LabBoxQty.Visible := False;
          LabBoxCap.Visible := False;
          labBoxDiv.Visible := False;
...
begin
  ...
  GetVersion(ExtractFileDir(Application.ExeName) + '\PackingDll.Dll');
...
function TfPacking.ApartSN(Tobj :TEdit;Aobject:TListBox  ): Boolean ;
var
  str:string;
  i:Integer ;
begin
    str:=UpperCase(Trim(tobj.Text));
    Aobject.Clear ;
    while str <> '' do
    begin
      ...
      begin
        if (Copy(str,1,i-1)<>'') and (Length(Copy(str,1,i-1))=13) then
            Aobject.AddItem(Copy(str,1,i-1),Aobject);
      end;
...
function TfPacking.GetPackingByWO:Boolean;
begin
    result :=False;
    with qrytemp do begin
        ...
        Params.Clear;
        Params.CreateParam(ftString, 'WORK_ORDER', ptInput);
        CommandText := 'SELECT IsPackByWo from sajet.g_wo_base_ext where work_order= :work_order ';
        Params.ParamByName('WORK_ORDER').AsString :=mWO;
        Open;
        if IsEmpty then begin
            Exit;
...
begin
  ...
  begin
    ...
    if (PackingBase = 'Work Order') or g_bPackByWo then
        CommandText := CommandText + 'WHERE  WORK_ORDER = :WORK_ORDER '
    else  begin
        CommandText := CommandText + 'WHERE  MODEL_ID = :WORK_ORDER and '+
                       ' work_order not in (select a.work_order from sajet.g_wo_base_ext a,'+
                       ' sajet.g_wo_base b where a.work_order=b.work_order and a.IsPackbyWO=''Y'' and b.wo_status in (3,4)) ';
    end;
...
//    if (gsCarton = 'N/A') or (gsPallet = 'N/A') then
    ...
    begin
       ...
       begin
          Close;
          DataRequest('SAJET.ccm_chk_packbyWO');
          FetchParams;
          Params.ParamByName('TWO').AsString := editMO.Text;
       ...
       if sRes <> 'OK' then begin
          ShowMsg('Work Order is Different!!' + #13#10 + sRes, 'ERROR');
          Close;
          Exit;
       end;
    ...
    begin
        BoxNo := editBox.Text;
        if not g_bCheckBoxFull then LabBoxQty.Caption := IntToStr(RecordCount);
    end;
    Close;
  end;
  ...
  begin
      ...
      begin
          with QryTemp do begin
              ...
              Params.Clear;
              Params.CreateParam(ftString, 'BOX_NO', ptInput);
              CommandText := 'SELECT  A.SERIAL_NUMBER , A.BOX_NO '
                + 'FROM  SAJET.G_SN_STATUS A  '
                + 'WHERE A.BOX_NO = :BOX_NO ';
              Params.ParamByName('BOX_NO').AsString := editBox.Text;
              Open;
              m_boxqty := IntToStr(RecordCount);

              if  m_boxqty <>LabBoxCap.Caption then begin
                  ShowMsg('Box Qty not Match(Box數量:'+m_boxqty+'不等於'+LabBoxCap.Caption+') ' , 'ERROR');
                  Exit;
              end;
          end;
      end;
...
begin
  ...
  begin
    ...
    begin
        MessageBox(Handle ,'SN條碼不能為空,請重新掃描!','ERROR',MB_ICONERROR );
        editSN.SetFocus ;
        editSN.SelectAll ;
        Abort ;
    end;
    ...
    begin
      ...
      Params.Clear;
      Params.CreateParam(ftString, 'WORK_ORDER', ptInput);
      CommandText := 'SELECT WORK_ORDER,WO_RULE from SAJET.G_WO_BASE '
        + 'WHERE WORK_ORDER = :WORK_ORDER and rownum = 1 '; // rownum 2006.11.13 add
      Params.ParamByName('WORK_ORDER').AsString := Trim(editMO.Text);
      Open;
      m_nRule :=Fieldbyname('WO_RULE').AsString ;
    end;

    if Pos('Cheetah',m_nRule)>0 then
    begin
        ApartSN(editSN ,lstSNList );

        if lstSNList.Count <=0 then
        begin
            MessageBox(Handle ,'掃描的條碼不合法','ERROR',MB_ICONERROR );
            editSN.SetFocus ;
            editSN.SelectAll ;
            Abort ;
        end;
        for i:=0 to lstSNList.Count do
        begin
...
var sRes,Start_Process_ID,Route_ID,stempsn: string;
dtSysdate:TDateTime;
IsFirstRepair,IsRepair :Boolean;
iHourOfOutTime :Double;
begin
   Result := False;

   //begin-----------------------Bell_Shuai-----------------20100723---------------
    with  QryTemp do     //檢查是否需要設定包裝為投入站
    begin
        Close;
        Params.Clear;
        Params.CreateParam(ftString,'Serial_NUMBER',ptInput);
        CommandText := 'select * from sajet.G_SN_STATUS Where Serial_NUmber =:Serial_number or customer_SN=:Serial_number';
    ...
    begin
        ...
        // Check  Out of Time
        if (sProcess = 'Mylar') or (sProcess = 'CM-VI') then
        begin
            with SProc do
            begin
                try
                    Close;
                    DataRequest('SAJET.CCM_GET_INTERVAL_TIME');
                    FetchParams;
                  ...
                  on  E: Exception do
                  begin
                       sRes := 'CCM_GET_INTERVAL_TIME Exception' + #13#10 + E.Message;
                  end;
                end;
                Close;
            end;
            ...
            if sRes <> 'OK' then
            begin
                ShowMsg('Get Interval Time Error: ' + sRes, 'ERROR');
                Exit;
            end;

            if iHourOfOutTime=0 then
            begin

                QryTemp.Close;
                QryTemp.Params.Clear;
                QryTemp.CommandText :='select nvl(param_value,0) param_value  from sajet.sys_Base where param_name =''Mylar Out of Time'' ';
                QryTemp.Open;

                if (not QryTemp.IsEmpty)  and ( QryTemp.FieldByName('param_value').AsInteger <> 0 )then
                begin
                    iHourOfOutTime :=QryTemp.FieldByName('param_value').AsFloat;
                    if SProc.Params.ParamByName('THOURS').AsFloat  >= iHourOfOutTime then
                    begin
                        with QryTemp do
                        begin
                            try
                        ...
                        begin
                            try
                                ...
                                Params.ParamByName('TTERMINALID').AsString := TerminalID;
                                Params.ParamByName('TSN').AsString := SNNO;
                                Params.ParamByName('tdefect').AsString := 'OOT01';
                            ...
                            on  E: Exception do
                            begin
                                sRes := 'SJ_NOGO Exception' + #13#10 + E.Message;
                            end;
                            end;
                            Close;
                        end;
                        ...
                        if sRes <> 'OK' then
                        begin
                            ShowMsg('SJ_NOGO Error: ' + sRes, 'ERROR');
                            Exit;
                        end
                        else begin
                            ShowMsg('超時未處理', 'ERROR');
            ...
            begin
                try
                    ...
                    Params.ParamByName('PALLET').AsString := editPallet.Text;
                    Open;

                    if not IsEmpty then
                    begin
                        stempsn := fieldbyName('serial_number').AsString ;
                        with SProc do begin
                            close;
                            DataRequest('sajet.ccm_check_repair') ;
                            FetchParams;
                            Params.ParamByName('TSN').AsString :=stempsn;
                            Execute;

                            if Params.ParamByName('tflag').AsString='N' then
                                IsFirstRepair :=false
                            else
                                IsFirstRepair :=True;
                        ...
                        begin
                            if IsFirstRepair then
                            begin
                                MessageDlg('正常品不能和維修品混裝',mtError,[mbOK],0);
                                Exit;
                            end;
                        end else begin
                            if not IsFirstRepair then
                            begin
                                MessageDlg('維修品不能和正常品混裝',mtError,[mbOK],0);
                                Exit;
                            end;
                        end;
                    end;
...
begin
  ...
  begin
       ...
       begin
           try
                Close;
                DataRequest('SAJET.CCM_SET_REPAIR_NEXTPROCESS');
                FetchParams;
                Params.ParamByName('TTERMINALID').AsString := TerminalID;
                Params.ParamByName('TSN').AsString := listSN.items.strings[0]; //SNNO;
                Params.ParamByName('TWO').AsString := editMO.text;
              ...
              on E: Exception do
              begin
                  sRes := 'Call CCM_SET_REPAIR_NEXTPROCESS Exception-' + E.Message;
              end;
           end;
           Close;
       end;
...
procedure TfPacking.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
  if Pos('Error reading from socket',e.Message)>0 then
  begin
    Exit ;
  end;
end;
