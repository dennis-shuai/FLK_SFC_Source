...
function G_getASSYKPSN(f_SocketConnection: TSocketConnection; f_sProvideName,sFieldName,sFieldValue: string;m_tsSN:TStringList; var f_tsParam, f_tsData: tstrings): boolean;
...
function G_getBoxNoData(f_SocketConnection: TSocketConnection; f_sProvideName, f_sBoxNO: string; var f_tsParam, f_tsData: tstrings): boolean;
var iCount: integer; sTable, sParam, sData: string; m_tsCSN,m_tsSN: TStringList;
begin
  result := false;
  try
    ...
    begin
      try
        ...
        CommandText := ' select to_char(create_time, nvl(b.param_value,''yyyy/mm/dd'')) create_date, '
          + ' to_char(create_time, ''yyyymmdd'') datecode , '
          + ' to_char(create_time, nvl(c.param_value,''yyyy/mm/dd hh24:mi:ss'')) create_datetime ,'
          + ' to_char(create_time, ''yyiw'') create_week '
          + ' from sajet.g_pack_box a, sajet.sys_base b, sajet.sys_base c  '
          + ' where a.box_no = :box_no and b.param_name(+) = ''CREATE_DATE'' '
          + ' and c.param_name(+) = ''CREATE_DATETIME'' ';
        ...
        f_tsParam.Add('datecode');
        f_tsData.Add(FieldByName('datecode').AsString);
        f_tsParam.Add('CREATE_WEEK');
        f_tsData.Add(FieldByName('CREATE_WEEK').AsString);
        ...
        //此BOX中的MAC==2007/7/24==========
        if not G_getASSYKPSN(f_SocketConnection,f_sProvideName,'BOX_NO',f_sBoxNO,m_tsSN,f_tsParam, f_tsData) then
          exit;
      finally
        m_tsCSN.Free;
        m_tsSN.Free;
...
function G_getCartonNoData(f_SocketConnection: TSocketConnection; f_sProvideName, f_sCartonNO: string; var f_tsParam, f_tsData: tstrings): boolean;
var iCount: integer; m_tsCSN, m_tsBox,m_tsSN: TStringList;
    ...
    begin
      try
        ...
        CommandText := ' select to_char(create_time, nvl(b.param_value,''yyyy/mm/dd'')) create_date, '
          ...
          + ' where a.Carton_NO = :carton_no and b.param_name(+) = ''CREATE_DATE'' '
          + ' and c.param_name(+) = ''CREATE_DATETIME''  ';
        ...
        Params.Clear;
        params.CreateParam(ftString, 'carton_no', ptinput);
        CommandText := ' select * from sajet.g_sn_status a,sajet.g_wo_base b ' +
          ' where a.carton_no=:carton_no  and a.work_order=b.work_order ' +
          ' and rownum=1 ';
        Params.ParamByName('carton_no').asstring := f_sCartonNO;
        open;

        f_tsParam.Add('PO');
        f_tsData.Add(FieldByName('PO_NO').AsString);


        Close;
        Params.Clear;
        params.CreateParam(ftString, 'carton_no', ptinput);
        CommandText := ' SELECT * FROM ( SELECT CARTON_NO,CREATE_TIME,ROWNUM CARTON_SEQ FROM ( SELECT  B.CARTON_NO ,B.CREATE_TIME  '+
                       ' FROM SAJET.G_SN_STATUS A,SAJET.G_PACK_CARTON B WHERE A.WORK_ORDER = ( '+
                       ' SELECT WORK_ORDER FROM SAJET.G_SN_STATUS WHERE CARTON_NO=:carton_no AND ROWNUM=1) '+
                       ' AND A.CARTON_NO=B.CARTON_NO GROUP BY B.CARTON_NO ,B.CREATE_TIME   ORDER BY B.CREATE_TIME '+
                       ' )) WHERE CARTON_NO =:carton_no ';

        Params.ParamByName('carton_no').asstring := f_sCartonNO;
        open;

        f_tsParam.Add('Carton_Seq');
        f_tsData.Add(FieldByName('Carton_Seq').AsString);
        ...
        //此carton中的MAC==2007/7/24==========
        if not G_getASSYKPSN(f_SocketConnection,f_sProvideName,'CARTON_NO',f_sCartonNO,m_tsSN,f_tsParam, f_tsData) then
          exit;
      finally
        m_tsBox.Free;
        m_tsCSN.Free;
  ...
  try
    ...
    begin
      try
        ...
        CommandText := ' select to_char(create_time, nvl(b.param_value,''yyyy/mm/dd'')) create_date, '
          + 'to_char(create_time, nvl(c.param_value,''yyyy/mm/dd hh24:mi:ss'')) create_datetime ,'
          + 'to_char(create_time, ''yyiw'') create_week, to_char(create_time, ''yyyymmdd'') datecode '
          + 'from sajet.g_pack_pallet a, sajet.sys_base b, sajet.sys_base c   '
          + 'where a.pallet_no = :pallet_no and b.param_name(+) = ''CREATE_DATE'' '
          + ' and c.param_name(+) = ''CREATE_DATETIME''  ';
        ...
        Params.Clear;
        params.CreateParam(ftString, 'PALLET_NO', ptinput);
        CommandText := ' SELECT * FROM ( SELECT PALLET_NO,CREATE_TIME,ROWNUM PALLET_SEQ FROM ( SELECT  B.PALLET_NO ,B.CREATE_TIME  '+
                       ' FROM SAJET.G_SN_STATUS A,SAJET.G_PACK_PALLET B WHERE A.WORK_ORDER = ( '+
                       ' SELECT WORK_ORDER FROM SAJET.G_SN_STATUS WHERE PALLET_NO=:PALLET_NO AND ROWNUM=1) '+
                       ' AND A.PALLET_NO=B.PALLET_NO GROUP BY B.PALLET_NO ,B.CREATE_TIME   ORDER BY B.CREATE_TIME '+
                       ' )) WHERE PALLET_NO =:PALLET_NO ';

        Params.ParamByName('PALLET_NO').asstring := f_sPalletNO;
        open;

        f_tsParam.Add('Pallet_Seq');
        f_tsData.Add(FieldByName('Pallet_Seq').AsString);
...
{        if m_tsSN.Count > 0 then
        ...
        //此Pallet中的MAC==2007/7/24==========
        if not G_getASSYKPSN(f_SocketConnection,f_sProvideName,'PALLET_NO',f_sPalletNO,m_tsSN,f_tsParam, f_tsData) then
          exit;
      finally
        m_tsSN.free;
        m_tsCSN.free;
...
function G_getASSYKPSN(f_SocketConnection: TSocketConnection; f_sProvideName,sFieldName,sFieldValue: string;m_tsSN:TStringList; var f_tsParam, f_tsData: tstrings): boolean;
var sParamName,sKPSNPartNo,sKPSNPartID:string;//,sLastSN,s:string;
  i,j,iIndex,iCount:integer;
  f_tsKPSN,f_tsSN:TStringList;
begin
  result := false;
  try
  f_tsKPSN:= TStringList.create;
  f_tsSN:= Tstringlist.Create;
   with TClientDataSet.Create(Application) do
    begin
      try
        RemoteServer := f_SocketConnection;
        ...
        //此carton中的MAC==2007/7/24==========
        //SYS_BASE中paramValue=參數名字@料號(Ex:MAC@MAC ID代表G_SN_KEYPARTS中料號為MAC ID的表示MAC)
        Close;
        Params.Clear;
        CommandText := 'select * from sajet.sys_base '
                     + 'where param_name = ''LabelKPSN'' ';
        open;
        if not IsEmpty then
        begin
          while not Eof do
          begin
            sParamName:=UpperCase(Copy(FieldByName('param_value').asstring,1,Pos('@',FieldByName('param_value').asstring)-1));
            sKPSNPartNo:=UpperCase(Copy(FieldByName('param_value').asstring,Pos('@',FieldByName('param_value').asstring)+1,length(FieldByName('param_value').asstring)));
            f_tsKPSN.Add(sParamName);
            f_tsKPSN.Add(sKPSNPartNo);
            Next;
          end;

          for i:=0 to (f_tsKPSN.count div 2)-1 do
          begin
            sParamName:=f_tsKPSN.strings[i*2];
            sKPSNPartNo:=f_tsKPSN.strings[i*2+1];
            ...
            Params.ParamByName('part_no').asstring := sKPSNPartNo;
            open;
            if IsEmpty then
              continue;
            sKPSNPartID:=FieldByName('part_id').asstring;

            {Close;
            Params.Clear;
            params.CreateParam(ftString, 'part_id', ptinput);
            params.CreateParam(ftString, 'Data', ptinput);
            CommandText := 'select * from sajet.g_sn_keyparts '
                         + 'where item_part_id = :part_id '
                         + 'and serial_number in ( '
                         + ' select serial_number from sajet.g_sn_status '
                         + ' where '+sFieldName+'=:Data) '
                         + ' order by serial_number,item_part_sn ';
            Params.ParamByName('part_id').asstring := sKPSNPartID;
            Params.ParamByName('Data').asstring := sFieldValue;
            open;   }
            ...
            while not eof do
            begin
              f_tsSN.Add(fieldbyname('serial_number').AsString);
              next;
            end;

            for J:= 0 to f_tsSN.Count-1 do
            begin
              close;
              params.Clear;
              ...
              commandtext:=' select * from sajet.g_sn_keyparts '
                          ...
                          +'   and item_part_id=:Part_id ';
              Params.ParamByName('SN').asstring := f_tsSN.Strings[j];
              Params.ParamByName('Part_id').asstring := sKPSNPartID;
              open;
              iCount := 1; //sLastSN:='';
              iIndex:= m_tsSN.IndexOf(f_tsSN.Strings[j])+1;
              while not eof do
              begin
                 //iIndex:= m_tsSN.IndexOf(f_tsSN.Strings[j])+1;
                 //if sLastSN<>FieldByName('Serial_NUmber').AsString then
                 //iCount:=1;
                 f_tsParam.Add(sParamName+inttostr(iIndex)+'_'+inttostr(iCount));
                 f_tsData.Add(fieldbyname('item_part_sn').AsString);
                 //sLastSN:=FieldByName('Serial_NUmber').AsString;
                 inc(iCount);
                 next;
              end;
            end;
