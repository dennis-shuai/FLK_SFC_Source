...
uses sysutils,classes,dialogs,forms;
...
//=====================================================================================================================
  ...
  procedure G_convertTstringToTranserFile(f_sFileName:string;f_tsData : tstrings);
  procedure G_convertTranserFileToTstring(f_sFileName:string;var f_tsData : tstrings);
//=====================================================================================================================
...
begin
  try
    result:='';
    for i:=1 to f_tsData.Count do begin
      if i<>1 then result:=result+f_sSpare;
      result:=Result+f_tsData[i-1];
    end;
  except
...
procedure G_convertTstringToTranserFile(f_sFileName:string;f_tsData : tstrings);
var sData,sTemp  : string;
    aData : array [0..4095] of byte;
begin
  if not FileExists(ExtractFilePath(Application.ExeName)+f_sFileName) then begin
    with TFileStream.Create(ExtractFilePath(Application.ExeName)+f_sFileName,fmCreate)  do begin
      try
      finally
        free;
      end;
  ...
  with TFileStream.Create(ExtractFilePath(Application.ExeName)+f_sFileName,fmOpenReadWrite)  do begin
    try
      Seek(0,soFromEnd);
      sData:= G_convertTstringsToTransferData(f_tsData);

      while sData<>'' do begin
        sTemp:=copy(sData,1,length(aData));
        delete(sData,1,length(sTemp));
        G_convertStrToBuffer(sTemp,@aData);
        WriteBuffer(aData,length(sTemp));
      end;
    finally
      free;
    end;
...
begin
  ...
  if not FileExists(ExtractFilePath(Application.ExeName)+f_sFileName) then exit;

  with TFileStream.Create(ExtractFilePath(Application.ExeName)+f_sFileName,fmOpenRead) do begin
    try
      Seek(0,soFromBeginning);
      sData:='';
      iCount:=Read(aData,length(aData));
      while iCount<>0 do begin
        sData:=sData+G_convertBufferToStr(@aData,iCount);
        iCount:=Read(aData,length(aData));
      end;
    finally
      free;
  ...
  DeleteFile(ExtractFilePath(Application.ExeName)+f_sFileName);

  tsData:=TStringList.create;
  try
    while G_convertTransferDataToTstrings(sData,tsData) do begin
      f_tsData.Add(G_convertTstringsToEncodeString(tsData));
    end;
  finally
    tsData.free;
  end;
