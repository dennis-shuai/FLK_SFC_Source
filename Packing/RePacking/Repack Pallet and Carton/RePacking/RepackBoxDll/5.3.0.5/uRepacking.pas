...
type
  ...
  private
    mPalletOfCarton,mCartonOfBox: String;
    TerminalID: String;
...
begin
   ...
   begin
      ...
      CommandText := 'SELECT A.SERIAL_NUMBER,A.CUSTOMER_SN, A.CURRENT_STATUS, A.WORK_FLAG, A.WORK_ORDER, A.MODEL_ID, A.PROCESS_ID, A.QC_NO, A.QC_RESULT,SHIPPING_ID, '
                   + ' A.Carton_No,A.Pallet_No,      B.PART_NO, DECODE(A.QC_RESULT,''N/A'',''NULL'',''0'',''PASS'',''1'',''REJECT'') RESULT '
...
begin
   ...
   begin
      ...
      CommandText := 'UPDATE SAJET.G_SN_STATUS '
                   + '   SET BOX_NO = ''' + editReBoxNo.Text + ''' '
                   + '     , Carton_NO  = ''' + mCartonOfBox+''''
   ...
   if ((lablfifocode.Caption<>'0' ) or ((lablfifocode.Caption='0' )and  QryBoxData.IsEmpty ))
     and (mCartonofBox<>soldcarton ) then
      with qrymaterial do
        begin
           ...
           commandtext:='select material_no from sajet.g_material '
                      +' where material_no= ''' +  mCartonOfBox + ''' and rownum=1 ';
           open;
           ...
           begin
                 ...
                 commandtext:='insert into sajet.g_ht_material '
                        +' (select * from sajet.g_material where material_no= ''' + mCartonOfbox + ''' and rownum=1 )';
                 execute;
           end ;
...
begin
   ...
   begin
      ...
      sOldPallet:=FieldByName('Pallet_No').asstring;
     ...
     { if QryBoxData.RecordCount <> 0 then
      begin
         if QryBoxData.FieldByName('Model_ID').AsString <> FieldByName('Model_ID').AsString then
         begin
            MessageDlg('Customer SN Model not Match !!',mtError,[mbCancel],0);
            Close;
            Exit;
         end
         else if (QryBoxData.FieldByName('QC_NO').AsString = 'N/A') and
         ...
         else if QryBoxData.FieldByName('QC_Result').AsString <> FieldByName('QC_Result').AsString then
         begin
     ...
     if QryBoxData.FieldByName('Out_PDLINE_TIME').AsString <> FieldByName('Out_PDLINE_TIME').AsString then
         begin
            MessageDlg('Out Process Status not Match !!',mtError,[mbCancel],0);
            Close;
            Exit;
      end;

      // Check Process
      if QryBoxData.FieldByName('WIP_PROCESS').AsString <> FieldByName('WIP_PROCESS').AsString then
         begin
            MessageDlg('Process  Differrent !!',mtError,[mbCancel],0);
            Close;
            Exit;
      end;

     if sOLDCarton <> 'N/A' then begin
         if QryBoxData.IsEmpty then begin
               mCartonofBox :=soldCarton ;
               mPalletOfCarton := sOldPallet;
         end else begin
              if soldCarton <> mCartonOfBox then begin
                  ...
                  begin
                          ...
                          if (mCartonOfbox <> '')  and  (mCartonOfbox <> 'N/A') and (mCartonofBox <> sOldCarton)  then
                 ...
                 //新加入一個carton
                  ...
                  begin
                      ...
                      commandtext:='select a.factory_id from sajet.g_wo_base a,sajet.g_sn_status b '
                                  +' where b.BOX_no='''+editReBoxNo.Text +''' and a.work_order=b.work_order and rownum=1 ';
                      open;
                      if not isempty then
                      begin
...
begin
   ...
   begin
      Close;
      Params.Clear;
      CommandText := 'SELECT A.CUSTOMER_SN, A.WORK_ORDER, B.PART_NO, C.PROCESS_NAME, A.MODEL_ID, A.PROCESS_ID,A.wip_process, '
