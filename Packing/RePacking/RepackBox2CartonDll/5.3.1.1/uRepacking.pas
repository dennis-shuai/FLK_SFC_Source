...
type
  ...
  private
    mCartonModelID,soldcarton, mProcessOfCarton, sOldProcess: String;
    sfifo,sOldfifo:string;
    TerminalID: String;
...
begin
         ...
         begin
               ...
               begin
                   ...
                   commandtext:=' insert into sajet.g_material  '
                              +'(part_id,dateCode,material_no,material_qty,status,locate_id,warehouse_id,update_userid,update_time,version,type,fifocode,factory_id,factory_type) '
                              +' SELECT part_id,DateCode,''' +  editReCartonNo.Text + ''','+IntTOStr(BOX_COUNT)+',status,locate_id,warehouse_id,'''+UpdateUserID+''',SYSDATE,version,type,fifocode,factory_id,factory_type '
                              +' FROM SAJET.G_MATERIAL WHERE MATERIAL_NO=''' + soldcarton + ''' AND ROWNUM=1 ';
                   execute;
                   close;
                   params.Clear ;
   ...
   begin
       close;
       params.Clear ;
       commandtext:='UPDATE SAJET.G_SN_STATUS SET CARTON_NO ='''+editRecartonNo.Text +''',PALLET_NO =''N/A'' '+
...
begin
   ...
   begin
      Close;
      Params.Clear;
      CommandText := 'SELECT A.SERIAL_NUMBER,A.CUSTOMER_SN, A.CURRENT_STATUS, A.WORK_FLAG, A.WORK_ORDER, A.MODEL_ID,A.CARTON_NO, A.PROCESS_ID, A.QC_NO, A.QC_RESULT,A.SHIPPING_ID, '
      ...
      begin
          ...
          begin  //Model不同
             ...
             begin
                if  mProcessOfCarton <> sOldProcess then
                begin
                   MessageDlg('Customer SN - ' + FieldByName('SERIAL_NUMBER').AsString + #13#10+
            ...
            begin
               if Qryfifo.IsEmpty then
                  sOldfifo := '0'
                   ...
                   sOldfifo := Qryfifo.fieldbyname('FIFOCODE').AsString ;

                if  sfifo > sOldfifo then
                begin
                     lablfifocode.Caption :=  sOldfifo;
                end else if sfifo ='0' then begin
                    lablfifocode.Caption :=  sOldfifo;
                end;
            end;
