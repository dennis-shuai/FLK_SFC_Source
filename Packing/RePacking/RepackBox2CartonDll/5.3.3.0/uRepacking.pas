...
type
  ...
  public
    { Public declarations }
    UpdateUserID : String;
    Authoritys,AuthorityRole,sIsCartonPackbyWo,sIsBoxPackbyWo : String;
...
begin
   ...
   begin
      ...
      CommandText := ' SELECT A.SERIAL_NUMBER  '
                   ...
                   + ' WHERE A.BOX_NO = ''' + editReCartonNo.Text + ''' ';
      Open;

      if not IsEmpty then begin
          MessageDlg( 'Box_No 不能當Carton_NO',mtError,[mbCancel],0);
          Close;
          Exit;
...
begin
   ...
   begin
      Close;
      Params.Clear;
      CommandText := 'SELECT WORK_ORDER, MODEL_ID,PROCESS_ID, decode(NEXT_PROCESS,0,WIP_PROCESS,NEXT_PROCESS) WIP_PROCESS,QC_NO, QC_RESULT, Out_PDLine_Time '
...
begin
   Result := False;
   sIsBoxPackbyWo := GetPackingByWO('Box_no',editBoxNo.Text);
   if (sIsBoxPackbyWo <> 'Y') and  (sIsBoxPackbyWo <>'N') then begin
        MessageDlg( sIsBoxPackbyWo,mtError,[mbCancel],0);
        Exit;
   end;
   ...
   begin
      ...
      begin
          ...
          begin
            ...
            //Model不同
             ...
             mProcessOfCarton  :=   QryCartonData.FieldByName('WIP_PROCESS').AsString ;
             ...
             begin
              ...
              mWoOfBox:=FieldByName('Work_order').AsString ;
             if  sIsBoxPackbyWo <> sIsCartonPackbyWo then begin
                 MessageDlg( 'box 包裝方式不同',mtError,[mbCancel],0);
                 Exit;

             end;
             if (sIsBoxPackbyWo ='Y')  and  ( mWoOfCarton <> mWoOfBox ) then begin
           ...
           {
            if Copy(mWoOfCarton,Length(mWoOfCarton)-1,2) ='-F' then begin
                   if Copy(mWoOfBox,Length(mWoOfBox)-1,2) <> '-F' then begin
                       MessageDlg('不能刷入非試產工令',mtError,[mbCancel],0);
                       Close;
                       Exit;
                   end;
            end else if Copy(mWoOfCarton,Length(mWoOfCarton)-1,2) ='-C' then begin
                   if Copy(mWoOfBox,Length(mWoOfBox)-1,2) <> '-C' then begin
                       MessageDlg('不能刷入非試驗工令',mtError,[mbCancel],0);
                       Close;
                       Exit;
                   end;
            end else if mWoOfCarton <>'' then begin
                   if (Copy(mWoOfBox,Length(mWoOfBox)-1,2) ='-F') or (Copy(mWoOfBox,Length(mWoOfBox)-1,2) = '-C') then begin
    ...
    begin
         if IsInRepackTemp or IsboxInStock then begin
              if not CheckMaterialInfo then Exit;
         end
         else begin
              MessageDlg('sn not in  Warehouse',mtError,[MBOK],0);
              Exit;
         end;
    end else begin
       if not (IsInRepackTemp or IsboxInStock) then
        if bCheckWO then begin
             if not QryCartonData.IsEmpty then
             if mWoOfCarton <> mWoOfBox  then
             begin
                 MessageDlg('Work Order Different !!'+#13#10
                       + mWoOfBox,mterror,[mbCancel],0);
                  Close;
                  Exit;
             end;
        end;
