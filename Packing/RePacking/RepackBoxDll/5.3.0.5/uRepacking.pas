   ...
   with qryfifo do
     begin
         ...
         commandtext:='select nvl(warehouse_id,''0'') as warehouse_id,nvl(locate_id,''0'') as locate_id,fifocode '
                    +' from sajet.g_material where material_no=  ''' + editReBoxNo.Text + ''' and rownum=1 ' ;
         open;
...
begin
   ...
   begin
     ...
     if sOLDCarton <> 'N/A' then begin
            if qrycartondata.IsEmpty then begin
               mCartonofBox :=soldCarton ;
               mPalletOfCarton := Qrytemp.fieldbyname('Pallet_NO').AsString;
            end;
