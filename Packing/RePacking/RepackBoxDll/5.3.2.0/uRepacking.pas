...
begin
     ...
     begin
          ...
          Params.ParamByName('factoryid').AsString :=QryMaterial.fieldbyName('FACTORY_ID').AsString;
          Params.ParamByName('factorytype').AsString :=QryMaterial.fieldbyName('FACTORY_TYPE').AsString;
          Params.ParamByName('warehouse').AsString := QryMaterial.fieldbyName('warehouse_id').AsString;
          Params.ParamByName('locate').AsString := QryMaterial.fieldbyName('locate_id').AsString;
          Params.ParamByName('status').AsString := QryMaterial.fieldbyName('status').AsString;
          Params.ParamByName('version').AsString := QryMaterial.fieldbyName('version').AsString;
          Params.ParamByName('DateCode').AsString := QryMaterial.fieldbyName('DateCode').AsString;
          execute;
     end;
end;
...
begin
          ...
          if IsCSNInStock then begin
              ...
              if IsCSNInRepackTemp then begin
                  ...
                  begin
                      ...
                      params.Clear ;
                      Params.CreateParam(ftString,'SN',ptInput);
                      commandtext:=' update sajet.g_sn_repack set BOX_NO='''+editReBoxNo.Text+ ''' where serial_number =:SN ';
                      Params.ParamByName('SN').AsString :=g_sn;
                      Execute;
                  end;
              end;
   ...
   begin
       ...
       begin
           if IsCSNInRepackTemp then begin
                ...
                begin
                     ...
                     close;
                     params.Clear ;
                     commandtext:='delete  from sajet.g_sn_repack where serial_number ='''+g_sn+'''';
                     execute;
                end
           end else begin
       ...
       begin
            if mCartonOfBox <> mCartonOfCSN then
            begin
                 ReduceMaterialQty(mCartonOfCSN);
                 AddMaterialQty(mCartonOfbOX);
            end;
   ...
   begin
      ...
      Params.ParamByName('Carton').AsString := mCartonOfBox;
      Params.ParamByName('Pallet').AsString := mPalletOfBox;
      Execute;
      Close;
   end;
...
begin
   ...
   end;
   {
    ...
    with QryBoxRepack do begin
         ...
         if IsEmpty then begin
             ...
             IsBoxInRepackTemp := True;
              //sWarehouseIdOfBox :=fieldbyName('warehouse_id').AsString;
         end;
    end;
    ...
    begin
          ...
          begin
                if qryfifo.FieldByName('warehouse_id').AsString <>qrymaterial.fieldbyname('warehouse_id').AsString then
                begin
                ...
                //check locate
                if qryfifo.FieldByName('locate_id').AsString <>qrymaterial.fieldbyname('locate_id').AsString then
                begin
                ...
                //check fifocode
                if  qrymaterial.fieldbyname('fifocode').AsString < lablfifocode.Caption then
                    lablfifocode.Caption:= qrymaterial.fieldbyname('fifocode').AsString ;
          end else begin

                if not IsCSNInRepackTemp then begin
    ...
    begin
         if IsCSNInstock then lablfifocode.Caption:= qrymaterial.fieldbyname('fifocode').AsString ;

         if (not IsCSNInStock) and  (not IsCSNInRepackTemp) then begin
         ...
         if IsBoxInRepackTemp    then begin
             if IsCSNInStock  then begin
                    if QryBoxRepack.FieldByName('WareHouse_ID').AsString <> qrymaterial.FieldByName('WareHouse_ID').AsString  then
                    begin
                         MessageDlg('Warehouse is different!',mtError,[mbCancel],0);
                         Exit;
                    end;

                    if QryBoxRepack.FieldByName('Locate_ID').AsString <> qrymaterial.FieldByName('Locate_ID').AsString  then
                    begin
                         MessageDlg('Locate  is different!',mtError,[mbCancel],0);
                         Exit;
                    end;
                    ...
                    if QryBoxRepack.FieldByName('WareHouse_ID').AsString <> QryTemp.FieldByName('WareHouse_ID').AsString  then
                    begin
                         MessageDlg('Warehouse is different!',mtError,[mbCancel],0);
                         Exit;
                    end;

                    if QryBoxRepack.FieldByName('Locate_ID').AsString <> QryTemp.FieldByName('Locate_ID').AsString  then
                    begin
                         MessageDlg('Locate  is different!',mtError,[mbCancel],0);
                         Exit;
                    end;
