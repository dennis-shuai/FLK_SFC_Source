...
type
  ...
  public
    { Public declarations }
    UpdateUserID : String;
    G_PARTNO,sPartID,mWoOfBox,mWoOfCSN,sIsBoxPackByWo,sIsSNPackByWo:String;
...
begin
   ...
   sIsBoxPackByWo := GetPackingByWO('Box_No',editReBoxNo.Text);
...
begin
   ...
   if not QryBoxData.IsEmpty then  begin
     if sIsSNPackByWo <>sIsBoxPackByWo then begin
         MessageDlg('包裝方式不同',mtError,[mbCancel],0);
         Exit;
     end;
     if (sIsBoxPackbyWo ='Y')  and  ( mWoOfBox <> mWoOfCSN ) then begin
         MessageDlg( mWoOfCSN,mtError,[mbCancel],0);
         Exit;
     end;
   end;
