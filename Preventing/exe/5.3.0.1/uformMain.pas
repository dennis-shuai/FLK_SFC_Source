...
uses
  ...
  ObjBrkr, ToolWin, ComCtrls, ImgList,IniFiles;
...
type
  ...
  private
    ...
    procedure LoadSajetParamDll(f_sDllName: string; WOStatus: string);
  public
    { Public declarations }
    gsProgram: string;
...
procedure TformMain.LoadSajetParamDll(f_sDllName: string; WOStatus: string);
begin
  try
    ...
    m_initialSajetParamDll(self, PanelParent, Application, LoginUserID, SocketConnection1, '', '', WOStatus);
  except
    on E: Exception do raise Exception.create('(' + ClassName + '.LoadSajetParamDll)' + E.Message);
  end;
end;
...
var pIni: TIniFile; sImage, sFileName: string;
begin
  ...
  LabNO.Caption := '';
  LabName.Caption := '';
  sFileName := Copy(ExtractFileName(Application.ExeName), 1, Length(ExtractFileName(Application.ExeName)) - Length(ExtractFileExt(Application.ExeName)));
  with csFTemp do
  begin
    Close;
    Params.Clear;
    ...
    CommandText := 'Select PROGRAM ' +
      ...
      'Where EXE_FILENAME = :EXE_FILENAME ';
    Params.ParamByName('EXE_FILENAME').AsString := sFileName;
    Open;
...
begin
  ...
  begin
    if G_tsDllParam.Strings[Tag - 1] = '' then
      ...
      loadSajetParamDll(G_tsDllName.Strings[Tag - 1], G_tsDllParam.Strings[Tag - 1]);
//    loadSajetDll(G_tsDllName.Strings[Tag-1]);
  end;
end;
