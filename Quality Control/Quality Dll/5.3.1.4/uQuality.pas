...
type
   TfQuality = class(TForm)
    ...
    CHBPASSCLEAR: TCheckBox;
    ChBRejectClear: TCheckBox;
   ...
   public
    { Public declarations }
      ...
      G_sCartonNO: string;
      G_sRouteID: string;
      G_QCBase,G_InputType:string;
      slPallet: TStringList;
      G_bPassClear,G_bRejectClear,G_bPassClearonlylotno ,bRejectStatus:Boolean;
      G_bCheckPart,G_bCheckLot:Boolean;
      G_PartProcess:string;
      G_tsSampleLevel:TStrings;
      function GetWorkStaionOption(var bPassClear,bRejectClear,bPassClearonlylotno:Boolean):Boolean;
...
begin
  ...
  begin
    ...
    // 已檢驗的 Pallet
      Params.Clear;
      Params.CreateParam(ftString, 'QC_NO', ptInput);
      CommandText := 'Select PALLET_NO,CARTON_NO,BOX_NO,Count(SERIAL_NUMBER) QTY '
                    +'  From SAJET.G_SN_STATUS '
                    +' Where QC_NO = :QC_NO '
                    +' Group By Pallet_No,CARTON_NO,Box_No ';
      Params.ParamByName('QC_NO').AsString := cmbQCLotNo.Text;
      Open;
      I := 0;
      while not Eof do
      begin
         ...
         begin
            ...
            Subitems.Add(Fieldbyname('BOX_NO').AsString);
  ...
  // Check SN & 檢查工單是否一致
   try
     //
       with QryTemp do
       begin
           if (G_InputType ='SN') OR (G_InputType='')  then
           begin
               Close;
               Params.Clear;
               Params.CreateParam(ftString, 'SERIAL_NUMBER', ptInput);
               ...
               begin
                  ...
                  begin
                     MessageBeep(17);
                     MessageDlg('Input Data error!! No SN', mtError, [mbCancel], 0);
                     Close;
                     Exit;
                  end;
               end;
           end else if  G_InputType ='Box' then
           begin
                Close;
                Params.Clear;
                ...
                Open;
                 ...
                 begin
                     MessageBeep(17);
                     MessageDlg('Input Data error!! No BOX', mtError, [mbCancel], 0);
                     Close;
                     Exit;
                 end;

           end else if G_InputType ='Carton' then
           begin
                Close;
                Params.Clear;
                ...
                begin
                    MessageBeep(17);
                    MessageDlg('Input Data error!! No CARTON', mtError, [mbCancel], 0);
                    Close;
                    Exit;
                end;
           end;
           ...
           begin
              MessageBeep(17);
              MessageDlg('Serial Number Hold', mtError, [mbCancel], 0);
              Exit;
           end;


           if G_QCBase ='Work Order' then
           begin
   ...
   end;
  {
   ...
   begin
      try
        ...
        //Params.ParamByName('TINPUTTYPE').AsString:=  G_InputType;
...
begin
  ...
  case screen.width of
   ...
   GetWorkStaionOption(G_bPassClear,G_bRejectClear,G_bPassClearonlylotno);//取得QC站別的設定; 是否要清除QCLOTNO,PALLET,CARTON
   GetQCLotNo;
   GetAllDefect;
   sbtnPass.Enabled:=False;
   sbtnReject.Enabled := False;
...
function TfQuality.GetWorkStaionOption(var bPassClear,bRejectClear,bPassClearonlylotno:Boolean):boolean;
begin
  result := False;
  bPassClear := False;
  bRejectClear := False;
  bPassClearonlylotno:=false;
  G_bCheckPart := False;
  G_PartProcess := '';
  G_QCBase :='';
  G_InputType :='';
    ...
    try
       ...
       begin
          ...
          If Fieldbyname('PARAME_ITEM').AsString = 'Clear QC Data Only LOT NO' Then
             bPassClearonlylotno := (Fieldbyname('PARAME_VALUE').AsString = 'Y');
          If Fieldbyname('PARAME_ITEM').AsString = 'QC Base' Then
             G_QCBase :=Fieldbyname('PARAME_VALUE').AsString ;
          If Fieldbyname('PARAME_ITEM').AsString = 'Input Type' Then
             G_InputType :=Fieldbyname('PARAME_VALUE').AsString ;
          //是否檢查附件
  ...
  CHBPassClear.Enabled :=FALSE;
  CHBRejectClear.Enabled :=FALSE;
  CHBPassClearonlylotno.Enabled :=FALSE;
  CHBPassClear.Checked  :=bPassClear;
  CHBRejectClear.Checked  := bRejectClear;
  CHBPassClearonlylotno.Checked :=bPassClearonlylotno;
end;
...
//========================================開始異動資料庫===============================================
     ...
     begin
        try
           begin
              Close;
              DataRequest('SAJET.SJ_QC_TRANSFER2');
              FetchParams;
              ...
              if G_bPassClearonlylotno then
                Params.ParamByName('TPASSCLEARonlylotno').AsString := '1'
              else
                Params.ParamByName('TPASSCLEARonlylotno').AsString := '0';
           ...
           MessageDlg('SAJET.SJ_QC_TRANSFER2 Exception',mtError,[mbCancel],0);
           Close;
           exit;
        end;
        Close;
