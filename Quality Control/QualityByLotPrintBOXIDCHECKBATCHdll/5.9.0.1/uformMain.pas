...
type
  ...
  TformMain = class(TForm)
    ...
    procedure EditBOXIDKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure EditMachineKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
    procedure clearData;
  ...
  public
    ...
    function checkboxid(SBOXID:string):boolean;
    function checkbatch(SWO:string):boolean;
  end;
...
function TformMain.checkBOXID(sBOXID:STRING): Boolean;
begin
   Result := False;
   with QryTemp do
   begin
       //check box id 長度是否為11碼,並且以字母 B 開頭.
       if length(trim(sboxid))<>11 then
       begin
           MessageDlg('BOX ID length is not 11 bit !!', mtError, [mbOK], 0);
           editboxid.SelectAll ;
           editboxid.SetFocus ;
           Exit;
       end;
       if copy(sboxid,1,1)<>'B' Then
       begin
           MessageDlg('BOX ID not like BYYWWXXXXXX', mtError, [mbOK], 0);
           editboxid.SelectAll ;
           editboxid.SetFocus ;
           Exit;
       end;
      ...
      //check g_qc_lot
       Close;
       Params.Clear;
       CommandText := 'Select qc_type from sajet.G_QC_LOT '
                + ' WHERE QC_TYPE = '''+SBOXID+''' ';
       Open;
       if not isempty then
       begin
            MessageDlg('BOX ID DUB (G_QC_LOT) !!', mtError, [mbOK], 0);
            editboxid.SelectAll ;
            editboxid.SetFocus ;
            Exit;
       end;
       ...
       CommandText := 'Select material_no from sajet.G_material '
                + ' WHERE material_no = '''+SBOXID+''' ';
       Open;
       if not isempty then
       begin
            MessageDlg('BOX ID DUB (G_material) !!', mtError, [mbOK], 0);
            editboxid.SelectAll ;
            editboxid.SetFocus ;
            Exit;
       end;
       ...
       close;
       Params.Clear;
       CommandText := 'Select material_no from sajet.G_ht_material '
                + ' WHERE material_no = '''+SBOXID+''' ';
       Open;
       if not isempty then
       begin
            MessageDlg('BOX ID DUB (G_ht_material) !!', mtError, [mbOK], 0);
            editboxid.SelectAll ;
            editboxid.SetFocus ;
            Exit;
       end;
...
function TformMain.checkBATCH(sWO:STRING): Boolean;
begin
   Result := False;
   swo:='00'+SWO;
   with QryTemp do
   begin
       Close;
       Params.Clear;
       CommandText := 'select * from smt.g_smt_status where wo_sequence = '''+SWO+''' ';
       Open;
       if  isempty then
       begin
            CLOSE;
            Params.Clear;
            CommandText := 'select * from smt.g_smt_travel where wo_sequence = '''+SWO+''' ';
            Open;
            if isempty then
            begin
                   MessageDlg('NOT DONE BATCH MATERIAL !!', mtError, [mbOK], 0);
                   editboxid.SelectAll ;
                   editboxid.SetFocus ;
                   Exit;
            end;
...
begin
  ...
  //檢查BOX ID
  editboxid.Text :=trim(editboxid.Text);
  if editboxid.Text='' then
  begin
     MessageDlg('Please Input BOX ID!', mtError, [mbOK], 0);
     exit;
  end;
  //check box id 是否DUB
  if not checkboxid(editboxid.Text) then exit;

  //check batch 批次上料
  if not checkbatch(QC.sWorkOrder) then exit;

  //檢查是否有KEY IN Machine 機台號碼.
  editmachine.Text:=trim(editmachine.Text);
  if editmachine.Text='' then
  begin
     MessageDlg('Please Input Machine Code!', mtError, [mbOK], 0);
     editmachine.SelectAll;
     editmachine.SetFocus ;
     exit;
  end;
    ...
    begin
      try
        ...
        begin
          ...
          QryTemp.Params.CreateParam(ftString, 'machine_code', ptInput);
          QryTemp.CommandText := 'update sajet.g_qc_lot '
            + 'set qc_type = :qc_type, machine_code = :machine_code '
            + 'where qc_lotno = :qc_lotno and rownum = 1';
         // QryTemp.Params.ParamByName('qc_type').AsString := sMaterial;
          QryTemp.Params.ParamByName('qc_type').AsString := editboxid.Text;
          QryTemp.Params.ParamByName('qc_lotno').AsString := editLotNo.Text;
          QryTemp.Params.ParamByName('machine_code').AsString := editmachine.Text;
          QryTemp.Execute;
          QryTemp.Close;
          {
        ...
        lablMsg.Caption := 'BOX ID: ' + editboxid.Text + ' add OK.';
      except
        on e: Exception do
...
begin
  ...
  begin
    try
      ...
      Params.Clear;
      Params.CreateParam(ftString, 'WORK_ORDER', ptInput);
      CommandText := '  SELECT SUM(NVL(LOT_SIZE,0)) QTY '
        + '    FROM SAJET.G_QC_LOT '
...
procedure TformMain.EditBOXIDKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if Key = 13 then
   begin
...
procedure TformMain.EditMachineKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 13 then
  begin
