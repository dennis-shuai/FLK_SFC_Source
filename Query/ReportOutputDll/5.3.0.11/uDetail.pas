...
var
  fDetail: TfDetail;
  FcID: string;
  ChkDate, ChkLine, ChkWO, ChkPart, chkModel: Boolean;
  DateStyle: string;
  ChartMain, ChartSecond: string;
  slFactoryID, StrLstPdLine, StrLstPart, StrLstSort: TStringList;
  sQueryCon: string;
...
begin
  ...
  begin
    Result := ' and ((WORK_DATE = ''' + sStartDate + ''' and WORK_TIME >= ''' + cmbWSStart.Text + ''') '
      + '  and (WORK_DATE = ''' + sStartDate + ''' and WORK_TIME < ''' + cmbWSEnd.Text + ''')) ';
  end
  else
  begin
    Result := ' and ((WORK_DATE = ''' + sStartDate + ''' and WORK_TIME >= ''' + cmbWSStart.Text + ''') '
      + '   or (WORK_DATE > ''' + sStartDate + ''' and WORK_DATE < ''' + sEndDate + ''') '
      + '   or (WORK_DATE = ''' + sEndDate + ''' and WORK_TIME < ''' + cmbWSEnd.Text + ''')) ';
  end;
end;
...
var i, J, mTotal, mC: Integer;
  ...
  begin
    if cmbChart.Text = 'Bar' then
      Result := TBarSeries.Create(Chart1)
    else if cmbChart.Text = 'Line' then
      Result := TLineSeries.Create(Chart1)
    else if cmbChart.Text = 'Pie' then
      Result := TPieSeries.Create(Chart1);
  end;
begin
...
begin
  ...
  begin
    ...
    if DateStyle = 'Date' then
    begin
      sField := sField + sDot + 'TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYY/MM/DD'') "Date" ';
      sGroupby := sGroupby + sDot + 'TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYY/MM/DD'')';
    end;
    if DateStyle = 'Week' then
    begin
      sField := sField + sDot + 'TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYY-WW'') "Date" ';
      sGroupby := sGroupby + sDot + 'TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYY-WW'')';
    end;
    if DateStyle = 'Month' then
    begin
      sField := sField + sDot + 'TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYY/MM'') "Date" ';
      sGroupby := sGroupby + sDot + 'TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYY/MM'')';
    end;
    if DateStyle = 'Year' then
    begin
      sField := sField + sDot + 'TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYY'') "Date" ';
      sGroupby := sGroupby + sDot + 'TO_CHAR(TO_DATE(A.WORK_DATE,''YYYYMMDD''),''YYYY'')';
    end;
  end;
