    ...
    begin
      ...
      begin
        ...
        if Fieldbyname('PARAM_NAME').AsString = 'Defect Info.' then
          ChkDefect.Checked := (Fieldbyname('PARAM_VALUE').AsString = 'Y');
        if Fieldbyname('PARAM_NAME').AsString = 'Reason Info.' then
          ChkReason.Checked := (Fieldbyname('PARAM_VALUE').AsString = 'Y');
        if Fieldbyname('PARAM_NAME').AsString = 'Duty' then
          ChkDuty.Checked := (Fieldbyname('PARAM_VALUE').AsString = 'Y');
        if Fieldbyname('PARAM_NAME').AsString = 'Location' then
          ChkLocation.Checked := (Fieldbyname('PARAM_VALUE').AsString = 'Y');
      end;
...
begin
  ...
  SaveReportParams(RpID, 'Display Information', 'Defect Process', S);
  if S = 'N' then
    if ListSort.Items.IndexOf('Defect Process') <> -1 then begin
      ListSort.Items.Delete(ListSort.Items.IndexOf('Defect Process'));
      ListSort.Items.Delete(ListSort.Items.IndexOf('Process Code'));
    end;

  S := 'N';
  if ChkDefect.Checked then S := 'Y';
  SaveReportParams(RpID, 'Display Information', 'Defect Info.', S);
  if S = 'N' then
    if ListSort.Items.IndexOf('Defect Info.') <> -1 then
      ListSort.Items.Delete(ListSort.Items.IndexOf('Defect Info.'));

  S := 'N';
  if ChkReason.Checked then S := 'Y';
  SaveReportParams(RpID, 'Display Information', 'Reason Info.', S);
  if S = 'N' then
    if ListSort.Items.IndexOf('Reason Info.') <> -1 then
      ListSort.Items.Delete(ListSort.Items.IndexOf('Reason Info.'));

  S := 'N';
  if ChkDuty.Checked then S := 'Y';
  SaveReportParams(RpID, 'Display Information', 'Duty', S);
  if S = 'N' then
    if ListSort.Items.IndexOf('Duty') <> -1 then
      ListSort.Items.Delete(ListSort.Items.IndexOf('Duty'));

  S := 'N';
  if ChkLocation.Checked then S := 'Y';
  SaveReportParams(RpID, 'Display Information', 'Location', S);
  if S = 'N' then
    if ListSort.Items.IndexOf('Location') <> -1 then
      ListSort.Items.Delete(ListSort.Items.IndexOf('Location'));
...
begin
  ...
  begin
    ...
    if ChkProcess.Checked then
      with LvItems.Items.Add do Caption := 'Defect Process';
    if ChkProcess.Checked then
      with LvItems.Items.Add do Caption := 'Process Code';
    if ChkDefect.Checked then
      with LvItems.Items.Add do Caption := 'Defect Info.';
    if ChkReason.Checked then
      with LvItems.Items.Add do Caption := 'Reason Info.';
    if ChkDuty.Checked then
      with LvItems.Items.Add do Caption := 'Duty';
    if ChkLocation.Checked then
      with LvItems.Items.Add do Caption := 'Location';
...
begin
  ...
  if ChkProcess.Checked then
    cmbMain.Items.Add('Defect Process');
  if ChkDefect.Checked then
    cmbMain.Items.Add('Defect Info.');
  if ChkReason.Checked then
    cmbMain.Items.Add('Reason Info.');
  if ChkDuty.Checked then
    cmbMain.Items.Add('Duty');
  if ChkLocation.Checked then
    cmbMain.Items.Add('Location');
...
begin
  ...
  else if (Sender as TCheckBox).Name = 'ChkDefect' then
  begin
    sStr := 'Defect Info.';
  end
  else if (Sender as TCheckBox).Name = 'ChkReason' then
  begin
    sStr := 'Reason Info.';
  end
  else if (Sender as TCheckBox).Name = 'ChkDuty' then
  begin
    sStr := 'Duty';
  end
  else if (Sender as TCheckBox).Name = 'ChkLocation' then
  begin
    sStr := 'Location';
  end;
...
begin
  ...
  if ChkProcess.Checked then
    cmbSecond.Items.Add('Defect Process');
  if ChkDefect.Checked then
    cmbSecond.Items.Add('Defect Info.');
  if ChkReason.Checked then
    cmbSecond.Items.Add('Reason Info.');
  if ChkDuty.Checked then
    cmbSecond.Items.Add('Duty');
  if ChkLocation.Checked then
    cmbSecond.Items.Add('Location');
