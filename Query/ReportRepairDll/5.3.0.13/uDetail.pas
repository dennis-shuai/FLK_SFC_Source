...
uses
  ...
  MConnect, SConnect, ObjBrkr, Menus, Clrgrid, ComObj, ComCtrls,
...
type
  ...
  public
    { Public declarations }
    RpID, UpdateUserID, gsSpec, gsPath: string;
    gbOnlyDefect, gbLocation: Boolean;
...
var
  fDetail: TfDetail;
  FcID: string;
  ChkDate, ChkLine, ChkWO, ChkModel, ChkPart, ChkProcess, ChkDefect, ChkReason, ChkDuty, ChkLocation: Boolean;
  DateStyle: string;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if Fieldbyname('PARAM_NAME').AsString = 'Production Line' then
          ChkLine := (Fieldbyname('PARAM_VALUE').AsString = 'Y')
        else if Fieldbyname('PARAM_NAME').AsString = 'Work Order' then
        ...
        else if Fieldbyname('PARAM_NAME').AsString = 'Defect Process' then
          ChkProcess := (Fieldbyname('PARAM_VALUE').AsString = 'Y')
        else if Fieldbyname('PARAM_NAME').AsString = 'Defect Info.' then
          ChkDefect := (Fieldbyname('PARAM_VALUE').AsString = 'Y')
        else if Fieldbyname('PARAM_NAME').AsString = 'Reason Info.' then
        begin
          ChkReason := (Fieldbyname('PARAM_VALUE').AsString = 'Y');
          if ChkReason then gbOnlyDefect := False;
        end
        else if Fieldbyname('PARAM_NAME').AsString = 'Duty' then
        begin
          ChkDuty := (Fieldbyname('PARAM_VALUE').AsString = 'Y');
          if ChkDuty then gbOnlyDefect := False;
        end
        else if Fieldbyname('PARAM_NAME').AsString = 'Location' then
        begin
          ChkLocation := (Fieldbyname('PARAM_VALUE').AsString = 'Y');
          if ChkLocation then gbOnlyDefect := False;
          gbLocation := ChkLocation;
        end;
    ...
    Close;
    if not ChkDefect then gbOnlyDefect := False;
...
begin
  ...
  if Str = 'Date' then
  begin
    Result := 'TO_CHAR(REC_TIME,''YYYY/MM/DD'')';
    if DateStyle = 'Date' then
      Result := 'TO_CHAR(REC_TIME,''YYYY/MM/DD'')';
    if DateStyle = 'Week' then
      Result := 'TO_CHAR(REC_TIME,''YYYY-WW'')';
    if DateStyle = 'Month' then
      Result := 'TO_CHAR(REC_TIME,''YYYY/MM'')';
    if DateStyle = 'Year' then
      Result := 'TO_CHAR(REC_TIME,''YYYY'')';
  end;
  ...
  if Str = 'Location' then Result := 'NVL(Location,''N/A'')';
end;
...
begin
  ...
  begin
    ...
    for i := 0 to StrPlato.RowCount - 1 do //計算全部數量
...
var sSQL, sField, sGroupby, sDot: string;
  ...
  begin
    ...
    if Str = 'Production Line' then Result := 'PDLINE_NAME';
    if Str = 'Part No' then Result := 'PART_NO';
    if Str = 'Model Name' then Result := 'MODEL_NAME';
    if Str = 'Work Order' then Result := 'A.WORK_ORDER';
    if Str = 'Defect Process' then Result := 'PROCESS_NAME';
    if Str = 'Defect Info.' then
    begin
    ...
    if Str = 'Location' then Result := 'K.Location';
  end;

  function GetClickData: string;
  var sStr: String;
  begin
    sStr := Series.XLabel[ValueIndex];
    if sStr <> '' then
      Result := ' and ' + SelectField1(cmbMain.Text, 'Where') + ' = ''' + sStr + ''' '
    else
      Result := ' and ' + SelectField1(cmbMain.Text, 'Where') + ' is null ';
  end;
begin
  if cmbChart.Text = 'Pareto' then begin
    if (Series.LastValueIndex = ValueIndex) and (StrPlato.RowCount - 1 <> Series.LastValueIndex) then
      Exit;
  end;
  ...
  begin
    ...
    if DateStyle = 'Date' then
    begin
      sField := sField + sDot + 'TO_CHAR(REC_TIME,''YYYY/MM/DD'') "Date" ';
      sGroupby := sGroupby + sDot + 'TO_CHAR(REC_TIME,''YYYY/MM/DD'')';
    end;
    if DateStyle = 'Week' then
    begin
      sField := sField + sDot + 'TO_CHAR(REC_TIME,''YYYY-WW'') "Date" ';
      sGroupby := sGroupby + sDot + 'TO_CHAR(REC_TIME,''YYYY-WW'')';
    end;
    if DateStyle = 'Month' then
    begin
      sField := sField + sDot + 'TO_CHAR(REC_TIME,''YYYY/MM'') "Date" ';
      sGroupby := sGroupby + sDot + 'TO_CHAR(REC_TIME,''YYYY/MM'')';
    end;
    if DateStyle = 'Year' then
    begin
      sField := sField + sDot + 'TO_CHAR(REC_TIME,''YYYY'') "Date" ';
      sGroupby := sGroupby + sDot + 'TO_CHAR(REC_TIME,''YYYY'')';
    end;
  end;
  ...
  begin
    if sField <> '' then sDot := ',';
    sField := sField + sDot + 'NVL(K.Location,''N/A'') "Location"';
    sGroupby := sGroupby + sDot + 'K.Location';
  end;
  ...
  sSQL := 'Select ' + sField + ',sum(A.DEFECT_QTY) QTY ' + //',Count(A.SERIAL_NUMBER) QTY '+
    'From SAJET.G_SN_DEFECT A ' +
    ' ,SAJET.G_SN_REPAIR B ';
  if gbLocation then
    sSQL := sSQL + ', SAJET.G_SN_REPAIR_LOCATION K ';
  sSQL := sSQL + ' ,SAJET.SYS_PDLINE C ' +
    ' ,SAJET.SYS_DEFECT F ' +
    ' ,SAJET.SYS_REASON G ' +
    ' ,SAJET.SYS_DUTY H ';
  if (ChkProcess) or (cmbMain.Text = 'Defect Process') or (cmbSecond.Text = 'Defect Process') then
  ...
  sSQL := sSQL + 'Where ' + SelectDate + GetClickData + '  and C.FACTORY_ID = ' + FcID +
...
var sSQL, sField, sGroupby, sDot: string; i: Integer;
  ...
  begin
    ...
    begin
      ...
      if (StrLstSort.Strings[I] = 'Defect Info.') and chkDefect then
      begin
        if Result = '' then
          Result := '"Defect Code"'
        else
      ...
      if (StrLstSort.Strings[I] = 'Reason Info.') and ChkReason then
      begin
        if Result = '' then
          Result := '"Reason Code"'
        else
      ...
      if (StrLstSort.Strings[I] = 'Duty') and ChkDuty then
      begin
        if Result = '' then
          Result := '"Duty Code"'
        else
      ...
      if (StrLstSort.Strings[I] = 'Location') and ChkLocation then
      begin
        if Result = '' then
          Result := '"Location"'
        else
      ...
      if (StrLstSort.Strings[I] = 'Defect Process') and chkProcess then
      begin
        if Result = '' then
      ...
      if (StrLstSort.Strings[I] = 'QTY') then
      begin
        if Result = '' then
          Result := 'QTY desc'
        else
...
begin
  ...
  sSQL := 'Select ' + sField + ', sum(A.DEFECT_QTY) QTY '
    + 'From SAJET.G_SN_DEFECT A ';
  if not gbOnlyDefect then
    sSQL := sSQL + ' ,SAJET.G_SN_REPAIR B ';
  if gbLocation then
    sSQL := sSQL + ' ,SAJET.G_SN_REPAIR_LOCATION K ';
  sSQL := sSQL + ' ,SAJET.SYS_PDLINE C ';
  if ChkDefect then
  ...
  if ChkModel or (Trim(editModel.Text) <> '') then
    sSQL := sSQL + ' ,SAJET.SYS_MODEL J ';
  sSQL := sSQL + 'Where ' + SelectDate + '  and C.FACTORY_ID = ' + FcID
    + '  and A.PDLINE_ID = C.PDLINE_ID ';
  if ChkProcess then
    sSQL := sSQL + '  and A.PROCESS_ID = D.PROCESS_ID ';
  if ChkPart or (Trim(editPart.Text) <> '') or ChkModel or (Trim(editModel.Text) <> '') then
...
var sSQL: string;
  ...
  begin
    ...
    begin
      if DateStyle = 'Date' then
        Result := Result + ' and TO_CHAR(REC_TIME,''YYYY/MM/DD'') = ''' + QryData.Fieldbyname('Date').AsString + ''' ';
      if DateStyle = 'Week' then
        Result := Result + ' and TO_CHAR(REC_TIME,''YYYY-WW'') = ''' + QryData.Fieldbyname('Date').AsString + ''' ';
      if DateStyle = 'Month' then
        Result := Result + ' and TO_CHAR(REC_TIME,''YYYY/MM'') = ''' + QryData.Fieldbyname('Date').AsString + ''' ';
      if DateStyle = 'Year' then
        Result := Result + ' and TO_CHAR(REC_TIME,''YYYY'') = ''' + QryData.Fieldbyname('Date').AsString + ''' ';
    end;
    ...
    if chkModel then
      if QryData.Fieldbyname('Model Name').AsString <> '' then
        Result := Result + ' and J.Model_Name = ''' + QryData.Fieldbyname('Model Name').AsString + ''' '
      else
    ...
    if ChkLocation then
      Result := Result + ' and NVL(K.Location,''N/A'') = ''' + QryData.Fieldbyname('Location').AsString + ''' ';
  end;
begin
  if not QryData.Active then Exit;
  if QryData.Eof then Exit;
  sSQL := 'Select A.WORK_ORDER "Work Order"' +
    ',E.PART_NO "Part No"' + gsSpec +
  ...
  if not gbOnlyDefect then
    sSQL := sSQL + ',B.REMARK ';
  sSQL := sSQL + 'From SAJET.G_SN_DEFECT A ';
  if not gbOnlyDefect then
    sSQL := sSQL + ' ,SAJET.G_SN_REPAIR B ';
  if gbLocation then
    sSQL := sSQL + ' ,SAJET.G_SN_REPAIR_LOCATION K ';
  ...
  if gbLocation then
    sSQL := sSQL + '  and A.RECID = K.RECID(+) ';
  sSQL := sSQL + '  and A.PDLINE_ID = C.PDLINE_ID ' +
    '  and A.PROCESS_ID = D.PROCESS_ID ' +
    '  and A.MODEL_ID = E.PART_ID ' +
  ...
  if Trim(editPart.Text) <> '' then
    if Pos('%', editPart.Text) <> 0 then
      ...
      sSQL := sSQL + ' and E.PART_NO = ''' + Trim(editPart.Text) + ''' ';
  if Trim(editModel.Text) <> '' then begin
    if Pos('%', editModel.Text) <> 0 then
      sSQL := sSQL + ' and J.MODEL_NAME Like ''' + Trim(editModel.Text) + ''' '
    else
      sSQL := sSQL + ' and J.MODEL_NAME = ''' + Trim(editModel.Text) + ''' ';
  ...
  begin
    ...
    begin
      ...
      CommandText := 'select work_order "Work Order", target_qty "Target Qty" '
        + ',part_no "Part No"'
        + ', part_type "Part Type", spec1 "Spec1", spec2 "Spec2" '
