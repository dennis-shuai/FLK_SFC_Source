...
uses
  ...
  MConnect, SConnect, ObjBrkr, Menus, ComObj, ComCtrls, TeEngine,
  ImgList, DBGrid1, TeeProcs, Chart, Series, GradPanel;
...
var
  fDetail: TfDetail;
  FcID: string;
  ChkLine, ChkProcess, ChkTerminal, ChkWO, ChkPart, ChkModel: Boolean;
...
begin
  ...
  for i := 0 to DBGrid11.Columns.Count - 1 do
    MsExcel.Worksheets['Data'].Range[Chr(i + 65) + '1'].Value := DBGrid11.Columns[i].Title.Caption;
...
function TfDetail.SelectField(Str: string): string;
begin
  Result := 'Model_ID';
  ...
  if Str = 'Work Order' then Result := 'B.WORK_ORDER';
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (cmbMain.Text = 'Process Name') and (StrLstSort.IndexOf('Process Code') > -1) then
      ...
      begin
        if (cmbSecond.Text = 'Process Name') and (StrLstSort.IndexOf('Process Code') > -1) then
...
begin
  ...
  begin
    ...
    if ((cmbSecond.Text = '') or (cmbSecond.Text = 'N/A')) then
    begin
...
var sSQL, sField, sGroupby, sDot: string;
  ...
  begin
    Result := ' and ' + SelectField(cmbMain.Text) + ' = ''' + Series.XLabel[ValueIndex] + ''' '
  end;
begin
  ...
  begin
    if sField <> '' then sDot := ',';
    sField := sField + sDot + 'E.Part_No "Part No"';
    sGroupby := sGroupby + sDot + 'E.Part_No';
  end;
  ...
  if ChkProcess then
    sSQL := sSQL + '  and A.WIP_PROCESS = D.PROCESS_ID ';
  if ChkPart or (Trim(editPart.Text) <> '') or ChkModel or (Trim(editModel.Text) <> '')  then
  ...
  sSQL := sSQL + GetClickData
    + AddCondition('A.PDLINE_ID', StrLstPdLine)
    + AddCondition('A.WIP_PROCESS', StrLstProcess)
    + ' Group By ' + sGroupby;
...
begin
  ...
  sSQL := 'Select ' + sField + ',SUM(A.WIP_QTY) QTY ' + //',Count(A.SERIAL_NUMBER) QTY '+
    'From SAJET.G_SN_STATUS A,' +
    ' SAJET.G_WO_BASE B ';
  sSQL := sSQL + ' ,SAJET.SYS_PDLINE C ';
  if ChkProcess then
  ...
  if Trim(combLine.Text) <> '' then sSQL := sSQL + ' and C.PDLINE_NAME = ''' + Trim(combLine.Text) + ''' ';
  sSQL := sSQL + AddCondition('A.WIP_PROCESS', StrLstProcess)
    + ' Group By ' + sGroupby;
  ...
  sQueryCon := Trim(sQueryCon);
  DBGrid11.DataSource := DataSource1;
  for i := 0 to DBGrid11.Columns.Count - 1 do
    if DBGrid11.Columns[i].Width > 130 then
      DBGrid11.Columns[i].Width := 130;
end;
...
procedure TfDetail.DBGrid11DblClick(Sender: TObject);
var sSQL: string;
  ...
  begin
    ...
    if chkWO then
      Result := Result + ' and b.Work_Order = ''' + QryData.Fieldbyname('Work Order').AsString + ''' ';
    if chkPart then
...
begin
  ...
  sSQL := sSQL + 'From SAJET.G_SN_STATUS A,' +
    ' SAJET.G_WO_BASE B, ' +
    ' SAJET.SYS_PDLINE C, ' +
    ' SAJET.SYS_PROCESS D, ' +
    ' SAJET.SYS_EMP F ';
  ...
  sSQL := sSQL + ' and A.EMP_ID = F.EMP_ID(+) ' +
    GetCondition + 'Order by "Serial Number" ';
