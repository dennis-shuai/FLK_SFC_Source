...
begin
     ...
     begin
        ...
        if IsEmpty then begin
            MessageDlg('找不到不良歷史,請檢查產出時間和不良時間',mtError,[mbYes],0);
            exit;
        end;
    ...
    //檢查是否有相同的東西
     ...
     begin
          ...
          Params.Clear;
          Params.CreateParam(ftString,'sn', ptInput);
          CommandText := ' select a.defect_Code,a.defect_desc,B.REC_TIME ,d.duty_desc,e.reason_desc  '+
                         ' from sajet.sys_defect a,sajet.g_sn_defect b,sajet.g_sn_repair c  '+
                         ' ,sajet.sys_duty d,sajet.sys_reason e '+
                         ' where A.DEFECT_ID =B.DEFECT_ID and b.recid=c.recid and C.SERIAL_NUMBER =:SN '+
                         ' and C.RP_PROCESS_ID=100271  and C.REASON_ID =E.REASON_ID and C.DUTY_ID =D.DUTY_ID';
     ...
     begin
         pnl1.Caption := '不良現象->'+QryTemp2.Fieldbyname('defect_Code').aSString+'/'+QryTemp2.Fieldbyname('defect_desc').aSString;
         lblTime.Caption := '不良時間->'+QryTemp2.Fieldbyname('REC_TIME').aSString;
         lblReason.caption := '不良原因->'+QryTemp2.Fieldbyname('reason_desc').aSString;
         lblDuty.caption :=   '維修動作->'+QryTemp2.Fieldbyname('Duty_desc').aSString;
...
var sRes, sProcessID,sStageID: string;
...
begin
  ...
  begin
    ...
    CommandText := 'Select A.PROCESS_ID,a.serial_number,A.WORK_ORDER,A.WORK_FLAG,A.STAGE_ID,A.MODEL_ID,B.PART_NO,A.VERSION, A.OUT_PROCESS_TIME, a.route_id ' //, c.step '
   ...
   // Check Route
    ...
    QryTemp.CommandText :='select * from SAJET.G_WO_REPAIR_TIMES   '+
                        ' where  WORK_ORDER=:WO ';
    QryTemp.Params.ParamByName('WO').AsString := sWO;
    QryTemp.Open;
    ...
    if QryTemp.IsEmpty then begin
        ...
        MAX_REPAIR_TIMES := QryTemp.fieldByName('MAX_REPAIR_TIMES').AsInteger;
    end;
    ...
    //QryTemp.Params.CreateParam(ftString,'WO',ptInput);
    QryTemp.CommandText :=' select Count(*) REPAIR_TIMES from SAJET.G_SN_REPAIR a,SAJET.G_SN_REPAIR_Location b,sajet.g_sn_defect C '+
                          ' where A.SERIAL_NUMBER = :SN  and a.RECID =b.RECID and ' +
                          '  C.Stage_id = :stage and a.recid=c.recid  and c.defect_id <> 10000744 ';
    QryTemp.Params.ParamByName('SN').AsString := gbSN;
    QryTemp.Params.ParamByName('STAGE').AsString := sStageID;
   // QryTemp.Params.ParamByName('WO').AsString := sWO;
    QryTemp.Open;
    ...
    begin
       MessageDlg('維修次數不能大於 '+IntToStr(MAX_REPAIR_TIMES)+'次', mtError, [mbCancel], 0);
       Exit;
    end;
...
var i,j: Integer; B: Boolean; sRes, sProcessID, sEmpNo,sRlTerminal: string;
