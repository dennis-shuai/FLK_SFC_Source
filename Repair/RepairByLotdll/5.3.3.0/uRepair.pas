...
begin
     ...
     begin
        ...
        if IsEmpty then begin
            MessageDlg('找不到不良歷史，或者沒有刷入維修',mtError,[mbYes],0);
            exit;
        end;
  ...
  begin
...

   {
    ...
    QryTemp.CommandText :=' select Count(*) REPAIR_TIMES from SAJET.G_SN_REPAIR a,SAJET.G_SN_REPAIR_Location b,sajet.g_sn_defect C '+
                          ...
                          ' a.work_order=:WO and  C.Stage_id = :stage and a.recid=c.recid and c.defect_id <> 10000744 ';
...
var i,j: Integer; B: Boolean; sRes, sProcessID, sEmpNo: string;
...
begin
  ...
  for i:=1 to StringGrid1.RowCount-2 do begin
      ...
      begin
          try
              Close;
              DataRequest('SAJET.CCM_REPAIR_GO');
              FetchParams;
              Params.ParamByName('TTERMINALID').AsString := TerminalID;
              Params.ParamByName('TSN').AsString := StringGrid1.Cells[1,i];;
              Params.ParamByName('TEMPID').AsString := UpdateUserID;
              ...
              sRes := Params.ParamByName('TRES').AsString;
                ...
                sRes := 'CCM_REPAIR_GO Exception:' + E.Message;
          end;
          Close;
      end;
      if sRes <> 'OK' then
