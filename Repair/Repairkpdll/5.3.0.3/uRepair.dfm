object fRepair: TfRepair
  Left = 114
  Top = 79
  BorderStyle = bsNone
  Caption = 'qa'
  ClientHeight = 556
  ClientWidth = 792
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 792
    Height = 556
    Align = alClient
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 0
    object ImageAll: TImage
      Left = 0
      Top = 0
      Width = 792
      Height = 556
      Align = alClient
      ParentShowHint = False
      ShowHint = False
      Transparent = True
    end
    object LabelPacking: TLabel
      Left = 13
      Top = 7
      Width = 150
      Height = 16
      AutoSize = False
      Caption = 'Repair Key Parts'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Label1: TLabel
      Left = 11
      Top = 6
      Width = 150
      Height = 16
      AutoSize = False
      Caption = 'Repair Key Parts'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clYellow
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Image3: TImage
      Left = 400
      Top = 353
      Width = 97
      Height = 20
      Picture.Data = {
        
        B5B3B8B5B3B8B5B3B8B5B3B8B5B3B8B5B300}
      Stretch = True
      Transparent = True
    end
    object Label25: TLabel
      Left = 536
      Top = 489
      Width = 105
      Height = 18
      Caption = 'Detect Station'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object Label3: TLabel
      Left = 8
      Top = 103
      Width = 149
      Height = 18
      Caption = 'Defect Information'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -15
      Font.Name = 'Bookman Old Style'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label4: TLabel
      Left = 536
      Top = 513
      Width = 111
      Height = 18
      Caption = 'Detect Process'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object Label6: TLabel
      Left = 536
      Top = 463
      Width = 84
      Height = 18
      Caption = 'Detect Line'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object Label5: TLabel
      Left = 8
      Top = 236
      Width = 113
      Height = 18
      Caption = 'Defect Reason'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -15
      Font.Name = 'Bookman Old Style'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Image1: TImage
      Left = 8
      Top = 219
      Width = 65
      Height = 18
      Picture.Data = {
        
        B5B3B8B5B3B8B5B3B8B5B3B8B5B3B8B5B300}
      Stretch = True
      Transparent = True
    end
    object Image4: TImage
      Left = 80
      Top = 219
      Width = 65
      Height = 18
      Picture.Data = {
        
        B5B3B8B5B3B8B5B3B8B5B3B8B5B3B8B5B300}
      Stretch = True
      Transparent = True
    end
    object Image5: TImage
      Left = 152
      Top = 219
      Width = 65
      Height = 18
      Picture.Data = {
        
        B5B3B8B5B3B8B5B3B8B5B3B8B5B3B8B5B300}
      Stretch = True
      Transparent = True
    end
    object LabPN: TLabel
      Left = 117
      Top = 87
      Width = 47
      Height = 16
      Caption = 'LabPN'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clMaroon
      Font.Height = -13
      Font.Name = 'Microsoft Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object LabLine: TLabel
      Left = 656
      Top = 463
      Width = 78
      Height = 16
      Caption = 'Detect Line'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clMaroon
      Font.Height = -13
      Font.Name = 'Microsoft Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object LabTerminal: TLabel
      Left = 656
      Top = 489
      Width = 97
      Height = 16
      Caption = 'Detect Station'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clMaroon
      Font.Height = -13
      Font.Name = 'Microsoft Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object LabProcess: TLabel
      Left = 656
      Top = 513
      Width = 106
      Height = 16
      Caption = 'Detect Process'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clMaroon
      Font.Height = -13
      Font.Name = 'Microsoft Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
      Visible = False
    end
    object sbtnFailSN: TSpeedButton
      Left = 395
      Top = 58
      Width = 23
      Height = 22
      Flat = True
      Glyph.Data = {
        
        00FFD6B6A5D6B6A5CEA68CE7BEA5DEB69C8C8E8C0000FF0000FF}
      OnClick = sbtnFailSNClick
    end
    object Image6: TImage
      Left = 432
      Top = 60
      Width = 65
      Height = 18
      Picture.Data = {
        
        B5B3B8B5B3B8B5B3B8B5B3B8B5B3B8B5B300}
      Stretch = True
      Transparent = True
    end
    object sbtnScrap: TSpeedButton
      Left = 432
      Top = 60
      Width = 65
      Height = 18
      Caption = 'Scrap'
      Enabled = False
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = sbtnScrapClick
    end
    object sbtnAdd: TSpeedButton
      Left = 8
      Top = 219
      Width = 65
      Height = 18
      Caption = 'Add'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = sbtnAddClick
    end
    object sbtnDelete: TSpeedButton
      Left = 80
      Top = 219
      Width = 65
      Height = 18
      Caption = 'Delete'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = sbtnDeleteClick
    end
    object sbtnRepair: TSpeedButton
      Left = 152
      Top = 219
      Width = 65
      Height = 18
      Caption = 'Repair'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = sbtnRepairClick
    end
    object sbtnFinish: TSpeedButton
      Left = 401
      Top = 352
      Width = 95
      Height = 21
      Caption = 'Finish'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = sbtnFinishClick
    end
    object Label7: TLabel
      Left = 8
      Top = 358
      Width = 113
      Height = 18
      Caption = 'Repair History'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -15
      Font.Name = 'Bookman Old Style'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object sbtnRepairer: TSpeedButton
      Left = 218
      Top = 31
      Width = 28
      Height = 24
      Flat = True
      Glyph.Data = {
        
        00FFD6B6A5D6B6A5CEA68CE7BEA5DEB69C8C8E8C0000FF0000FF}
      OnClick = sbtnRepairerClick
    end
    object labRPName: TLabel
      Left = 253
      Top = 34
      Width = 77
      Height = 18
      Caption = 'Emp Name'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clMaroon
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label8: TLabel
      Left = 21
      Top = 35
      Width = 90
      Height = 18
      Caption = 'Repairer No.'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label11: TLabel
      Left = 20
      Top = 33
      Width = 90
      Height = 18
      Caption = 'Repairer No.'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label9: TLabel
      Left = 7
      Top = 61
      Width = 103
      Height = 18
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Key Part S/N'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label2: TLabel
      Left = 6
      Top = 59
      Width = 103
      Height = 21
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Key Part S/N'
      Font.Charset = ANSI_CHARSET
      Font.Color = -1
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label10: TLabel
      Left = 9
      Top = 87
      Width = 100
      Height = 18
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Part No'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label16: TLabel
      Left = 8
      Top = 85
      Width = 100
      Height = 18
      Alignment = taRightJustify
      AutoSize = False
      Caption = 'Part No'
      Color = clTeal
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial Black'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object cmbSN: TComboBox
      Left = 115
      Top = 58
      Width = 273
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ItemHeight = 16
      ParentFont = False
      TabOrder = 0
      OnCloseUp = cmbSNCloseUp
      OnKeyPress = cmbSNKeyPress
    end
    object LVDefect: TListView
      Left = 8
      Top = 122
      Width = 489
      Height = 94
      Columns = <
        item
          Caption = 'Defect Code'
          Width = 100
        end
        item
          Caption = 'Description'
          Width = 160
        end>
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Items.Data = {1E0000000100000000000000FFFFFFFFFFFFFFFF00000000000000000131}
      LargeImages = ImageList1
      ReadOnly = True
      RowSelect = True
      ParentFont = False
      PopupMenu = PopupMenu1
      SmallImages = ImageList1
      TabOrder = 1
      ViewStyle = vsReport
      OnClick = LVDefectClick
    end
    object LVReason: TListView
      Left = 7
      Top = 254
      Width = 489
      Height = 97
      Columns = <
        item
          Caption = 'Reason Code'
          Width = 100
        end
        item
          Caption = 'Description'
          Width = 160
        end>
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ReadOnly = True
      RowSelect = True
      ParentFont = False
      TabOrder = 2
      TabStop = False
      ViewStyle = vsReport
    end
    object Panel2: TPanel
      Left = 392
      Top = 426
      Width = 489
      Height = 133
      BevelOuter = bvNone
      Color = clWhite
      TabOrder = 3
    end
    object DBGrid1: TDBGrid
      Left = 9
      Top = 377
      Width = 489
      Height = 78
      DataSource = dsrcDefect
      TabOrder = 4
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'PROCESS_NAME'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Times New Roman'
          Font.Style = []
          Title.Caption = 'Process Name'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 89
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DEFECT_CODE'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Times New Roman'
          Font.Style = []
          Title.Caption = 'Defect Code'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 84
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DEFECT_DESC'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Times New Roman'
          Font.Style = []
          Title.Caption = 'Defect Desc'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 109
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REC_TIME'
          Title.Caption = 'Rec Time'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 121
          Visible = True
        end>
    end
    object DBGrid2: TDBGrid
      Left = 8
      Top = 458
      Width = 489
      Height = 88
      DataSource = dsrcReason
      TabOrder = 5
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'repairer'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Times New Roman'
          Font.Style = []
          Title.Caption = 'Repairer'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clBlack
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 88
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REASON_CODE'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Times New Roman'
          Font.Style = []
          Title.Caption = 'Reason Code'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 88
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REASON_DESC'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Times New Roman'
          Font.Style = []
          Title.Caption = 'Reason Desc'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 101
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Repair_Time'
          Title.Caption = 'Repair Time'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 131
          Visible = True
        end>
    end
    object editRepairer: TEdit
      Left = 116
      Top = 31
      Width = 97
      Height = 24
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Bookman Old Style'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
      Text = 'editRepairer'
      OnChange = editRepairerChange
      OnKeyPress = editRepairerKeyPress
    end
  end
  object QryData: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspQryData'
    Left = 528
    Top = 440
  end
  object QryTemp: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspQryTemp1'
    Left = 432
    Top = 296
  end
  object SProc: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspStoreproc'
    Left = 368
    Top = 296
  end
  object ImageList1: TImageList
    Left = 416
    Top = 392
    Bitmap = {
      
      000000000000}
  end
  object PopupMenu1: TPopupMenu
    Left = 472
    Top = 400
    object AddDefect1: TMenuItem
      Caption = 'Add Defect'
      OnClick = sbtnAddClick
    end
    object DeleteDfect1: TMenuItem
      Caption = 'Delete Defect'
      OnClick = sbtnDeleteClick
    end
    object RepairRecord1: TMenuItem
      Caption = 'Repair Record'
      OnClick = sbtnRepairClick
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object Scrap1: TMenuItem
      Caption = 'Scrap'
      OnClick = sbtnScrapClick
    end
    object Finish1: TMenuItem
      Caption = 'Repair Complete'
      OnClick = sbtnFinishClick
    end
  end
  object dsrcDefect: TDataSource
    DataSet = qryData1
    Left = 448
    Top = 480
  end
  object qryData1: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspQryData'
    AfterScroll = qryData1AfterScroll
    Left = 600
    Top = 360
  end
  object qryDetail: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DspQryData'
    Left = 576
    Top = 304
  end
  object dsrcReason: TDataSource
    DataSet = qryDetail
    Left = 392
    Top = 432
  end
end
