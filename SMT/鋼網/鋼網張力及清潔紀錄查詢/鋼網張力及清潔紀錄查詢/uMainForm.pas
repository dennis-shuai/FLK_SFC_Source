...
begin
   ...
   begin
      ...
      SQLStr :='SELECT A.TOOLING_SN,C.WORK_ORDER,E.PDLINE_NAME,G.PROCESS_NAME,B.CUST_ASSET_NO||'' N/キよ络μ'' VALUE,'
               ...
               +' SAJET.SYS_EMP D,SAJET.SYS_PDLINE E,SAJET.SYS_PDLINE_PROCESS_RATE F,SAJET.SYS_PROCESS G'
               +' WHERE  A.TOOLING_SN_ID=B.TOOLING_SN_ID AND A.TOOLING_SN_ID=C.TOOLING_SN_ID AND D.EMP_ID=B.UPDATE_USERID '
               +' AND C.PROCESS_ID=F.PROCESS_ID AND E.PDLINE_ID=F.PDLINE_ID AND C.PROCESS_ID=G.PROCESS_ID';
      if EditSN.Text <> '' then
      begin
