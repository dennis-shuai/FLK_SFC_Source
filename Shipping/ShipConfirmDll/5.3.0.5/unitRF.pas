...
begin
  try
    if f_iMsgType=cMsgReceiveData then begin
      addLog(cLogTest,'Receive GID: SubID: Data -- '+G_convertBufferToStr(f_pData,f_iLen));
      m_DriverThread.addData(f_iGID,f_iSubID,f_iPort,G_convertBufferToStr(f_pData,f_iLen));
    end
    else raise Exception.Create('Unknow Receive Data Type');
  except
    on E:Exception do addLog(cLogError,'('+ClassName+'.onRsvDataFromDriver)'+E.Message);
  end;
end;
...
begin
  try
    ...
    if not m_bInitialOK then raise Exception.Create('Driver Unit Not Initialize OK');
    addLog(cLogNormal,'('+ClassName+'.setStarted)'+'Driver Unit Active Start');
    ...
    if f_bStart then addLog(cLogNormal,'('+ClassName+'.setStarted)'+'Driver Unit Active OK');
    m_bStart:=f_bStart;
...
begin
  result:=false;
  try
    addLog(cLogNormal,'('+ClassName+'.Setup)'+'Setup Driver Unit Start');
    ...
    addLog(cLogNormal,'('+ClassName+'.Setup)'+'Setup Driver Unit OK');
    result:=true;
  except
    on E:Exception do begin
...
begin
  ...
  try
    ...
    addLog(cLogNormal,'Driver Unit Create Start');
    ...
    addLog(cLogNormal,'Driver Unit Create OK');
...
begin
  try
    addLog(cLogNormal,'('+Classname+'.Destroy)'+'Driver Unit Destroy Start');
    m_DriverThread.free;
    m_DriverLink.free;
    addLog(cLogNormal,'('+Classname+'.Destroy)'+'Driver Unit Destroy OK');
    m_Log.free;
  except
