unit uAuthorityLogin;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, Buttons, StdCtrls, ExtCtrls, BmpRgn, DB, clsDataSet ;

type
  TAuthorityLogin = class(TForm)
    Imagemain: TImage;
    Bevel1: TBevel;
    Label2: TLabel;
    Label3: TLabel;
...
var
  fAuthorityLogin: TAuthorityLogin;
...
procedure TAuthorityLogin.SetTheRegion;
var HR: HRGN;
begin
...
procedure TAuthorityLogin.WMEraseBkGnd(var Msg: TWMEraseBkGnd);
var Brush: TBrush;
begin
  Brush := TBrush.Create;
  Brush.Color := Color;
...
procedure TAuthorityLogin.WMNCHitTest(var msg: TWMNCHitTest);
var
  i: integer;
  p: TPoint;
  AControl: TControl;
...
procedure TAuthorityLogin.FormCreate(Sender: TObject);
begin
  //Imagemain.Picture.LoadFromFile(ExtractFilePath(Application.ExeName) + 'sLogin.bmp');
  SetTheRegion;
end;

procedure TAuthorityLogin.SpeedButton1Click(Sender: TObject);
begin
  Close;
end;

function TAuthorityLogin.SetStatusbyAuthority: Boolean;
var sAuth : string;
begin
  Result := False;
  ...
  begin
    ...
    begin
      ...
      Params.ParamByName('PRG').AsString := PRG ;//'Shipper Label Print';
      Params.ParamByName('FUN').AsString := FUN ;//'Config';
      Open;
      while not Eof do
      begin
...
procedure TAuthorityLogin.sbtnLogInClick(Sender: TObject);
begin
  ModalResult := mrNone;
...
procedure TAuthorityLogin.editUserNOKeyPress(Sender: TObject; var Key: Char);
begin
  if Ord(Key) = vk_Return then
    editPwd.SetFocus;
end;

procedure TAuthorityLogin.editPwdKeyPress(Sender: TObject; var Key: Char);
begin
  if Ord(Key) = vk_Return then
    sbtnLogInClick(Self);
end;
