...
type
  ...
  public
    ...
    function showTOOLING(tooling_sn:string):string;
...
function TfMain.showTOOLING(tooling_sn:string):string;
begin
   with qrydata do
     begin
         close ;
...
BEGIN
    ...
    try
       editaddcount.Text:=floattostr(strtofloat(editaddcount.Text));
    except
        begin
             showmessage('Count Value input error!') ;
             editaddcount.SelectAll ;
             editaddcount.SetFocus ;
             EXIT;
        end;
    end;

    if strtofloat(editaddcount.Text )<=0 then
    begin
       showmessage('Count Value input error!') ;
       editaddcount.SelectAll ;
       editaddcount.SetFocus ;
    ...
    with qrydata do
     begin
         ...
         if fieldbyname('limit_used_count').AsInteger  <=strtofloat(editCOUNT.Text) then
            begin
               showmessage('THE Tooling SN LIMIT_COUNT:'+ fieldbyname('limit_used_count').AsString +'<= USED_COUNT '+ editCOUNT.Text) ;
               //cleardata;
               editsn.SetFocus ;
               EXIT;
            end
...
begin
  ...
  begin
    ...
    Params.ParamByName('dll_name').AsString := 'TOOLINGINPUTDLL.DLL';
    Open;
  end;
...
begin
   if trim(editsn.Text )<>'' then
     if key=13 then
        begin
           strtoolingsn:=showTOOLING(editsn.Text);
