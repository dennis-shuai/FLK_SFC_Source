...
type
  ...
  public
    { Public declarations }
    OrderIdx: string;
    UpdateUserID,sKeeperUserID,sOKeeperUserID,sKeeperName,sKeeperNo,gsParam: string;
...
Begin
   ...
   begin
       ...
       commandtext:='SELECT A.TOOLING_no,A.TOOLING_NAME,A.TOOLING_TYPE,B.TOOLING_SN_ID,B.TOOLING_SN,C.MACHINE_NO,'
                   +' C.ASSET_NO,C.MACHINE_TYPE,d.WAREHOUSE_Name,e.LOCATE_Name,C.keeper_UserID,  '
                   +' A.ENABLED AS TOOLING_ENABLED, B.ENABLED AS TOOLING_SN_ENABLED,c.monitor_dept, '
                   +' c.machine_memo,C.CUST_ASSET_NO,c.Provider,f.emp_no,f.emp_name,c.backup '
                   +' FROM SAJET.SYS_TOOLING A, SAJET.sys_tooling_sn B, SAJET.G_TOOLING_MATERIAL C,'
                   +' sajet.SYS_WareHouse d,sajet.sys_Locate e,sajet.sys_emp f  '
                   +' WHERE A.TOOLING_ID=B.TOOLING_ID  and f.emp_id=c.keeper_userid   '
       ...
       if (UPPERCASE(fieldbyname('MACHINE_TYPE').AsString) ='I') or
        (UPPERCASE(fieldbyname('MACHINE_TYPE').AsString) ='FI')  then
       begin
           lblstatus.Caption :='The Tooling_sn Of '+Trim(editsn.Text )+' Had In Stock!';
           lblstatus.Font.Color :=clRed;
           exit;
       end;
       ...
       begin
        ...
        sOKeeperUserID :=fieldbyname('Keeper_userID').AsString ;
...
BEGIN
  ...
  if getlocateid(trim(cmbboxwarehouse.Text),trim(cmbboxlocate.Text))='XXX' then
   ...
   if sKeeperUserID='' then
      sKeeperUserID :=  sOKeeperUserID
   else begin
      edtEmpNo.Text :=sKeeperNo ;
      edtEmpName.Text :=sKeeperName;
   end;
   with qrydata do
     begin
         close;
         ...
         commandtext:='Update SAJET.G_TOOLING_MATERIAL SET warehouse_id=:warehouse_id , '
                     +' locate_id=:locate_id,machine_type=''I'',machine_used=''N'' ,Keeper_USERID =:Keeper,'
        ...
        params.ParamByName('Keeper').AsString := sKeeperUserID ;
        params.ParamByName('Cust_Asset_NO').AsString:=edt1.Text ;
        params.ParamByName('Provider').AsString := edt2.Text ;
        ...
        lblstatus.Caption :=editsn.Text +' In Stock OK!';
...
begin
   if trim(editsn.Text )<>'' then
     if key=13 then begin
         Checktoolingsn ;
         edtEmpNo.Text := Qrytemp.fieldbyname('emp_no').AsString ;
         edtEmpName.Text := Qrytemp.fieldbyname('emp_Name').AsString ;
...
begin
    ...
    begin
        ...
        CommandText :='Select emp_id,Emp_NO,emp_name from sajet.sys_emp where emp_no =:empno';
        Params.ParamByName('empNO').AsString :=edtEmpNo.Text;
        Open;

        if not IsEmpty then begin
            edtEmpName.Text := fieldByName('emp_name').AsString;
            sKeeperName := edtEmpName.Text;
            sKeeperNo :=   edtEmpNo.Text;
            sKeeperUserID := fieldByName('emp_id').AsString;
        end else  begin
             edtEmpName.Text :='';
             sKeeperNo :='';
             sKeeperName :='';
