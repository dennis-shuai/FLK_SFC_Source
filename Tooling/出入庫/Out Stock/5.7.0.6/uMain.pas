...
type
  TfMain = class(TForm)
   ...
   // procedure cmbboxwarehouseChange(Sender: TObject);
    ...
    procedure cmbboxwarehouseSelect(Sender: TObject);
    procedure edtEmpNoChange(Sender: TObject);
    procedure cmbboxlocateSelect(Sender: TObject);
  ...
  public
    { Public declarations }
    OrderIdx: string;
    UpdateUserID,gsParam,sborrowUserID: string;
...
begin
   ...
   begin
       ...
       commandtext:=' SELECT a.warehouse_NAME from SAJET.SYS_warehouse  a,sajet.sys_emp_warehouse b '+
                    ' WHERE a.ENABLED=''Y'' and a.warehouse_id=b.warehouse_id and a.warehouse_desc =''OUT'' '+
...
Begin
   ...
   begin
       ...
       commandtext:='SELECT A.TOOLING_no,A.TOOLING_NAME,A.TOOLING_TYPE,B.TOOLING_SN_ID,B.TOOLING_SN,C.MACHINE_NO,'
                   +' C.ASSET_NO,C.MACHINE_TYPE,d.WAREHOUSE_Name,e.LOCATE_Name,  '
                   +' A.ENABLED AS TOOLING_ENABLED, B.ENABLED AS TOOLING_SN_ENABLED,c.monitor_dept, c.machine_memo,c.backup,c.Cust_Asset_NO,c.Provider '
                   +' FROM SAJET.SYS_TOOLING A, SAJET.sys_tooling_sn B, SAJET.G_TOOLING_MATERIAL C,sajet.SYS_WareHouse d,sajet.sys_Locate e  '
                   +' WHERE A.TOOLING_ID=B.TOOLING_ID    '
                   +' AND B.TOOLING_SN_ID=C.TOOLING_SN_ID  AND B.TOOLING_SN=:tooling_sn '
                   +' and c.warehouse_ID =d.warehouse_ID and c.locate_ID = e.Locate_ID and rownum=1 ';
       ...
       if IsEmpty then
         ...
         begin
             ...
             lblwarehouse.Caption:= fieldbyname('warehouse_Name').AsString+
                           '--'+fieldbyname('locate_Name').AsString;
             memomachine.Text :=fieldbyname('machine_memo').AsString ;
             edt1.Text :=  fieldbyname('Cust_Asset_NO').AsString ;
             edt2.Text :=  fieldbyname('Provider').AsString ;
             mmo1.Text :=  fieldbyname('Backup').AsString ;
            //cmbboxwarehouse.Text := get

         end;
      if UPPERCASE(fieldbyname('MACHINE_TYPE').AsString) ='O'  then
        begin
           lblstatus.Caption :='The Tooling_sn Of '+Trim(editsn.Text )+' Had Out Stock!';
           lblstatus.Font.Color :=clRed;
           exit;
        end;
...
BEGIN
    ...
    if getlocateid(trim(cmbboxwarehouse.Text),trim(cmbboxlocate.Text))='XXX' then
     ...
     begin
         lblstatus.Caption :='Please Input Emp No!';
         lblstatus.Font.Color :=clRed;
         exit;
     end ;
     ...
     begin
         ...
         params.CreateParam(ftstring,'borrow_userid',ptinput);
         commandtext:=' Update SAJET.G_TOOLING_MATERIAL SET warehouse_id=:warehouse_id , '
                     +' locate_id=:locate_id,machine_type=''O'',machine_used=''Y'' , '
                     +' Update_userid=:update_userid ,update_time=sysdate,borrow_userId=:borrow_userid '
        ...
        params.ParamByName('borrow_userid').AsString := sborrowUserID ;
        execute;
        ...
        lblstatus.Caption :=editsn.Text +' Out Stock OK!';
...
begin
  ...
  begin
    ...
    Params.ParamByName('dll_name').AsString := 'ToolingInStockDLL.dll';
    Open;
  end;
  ...
  editsn.SetFocus ;
  editsn.SelectAll ;
  //cmbboxwarehouse.Style :=csDropDownList;
  //cmbboxlocate.Style := csDropDownList;
  cleardata;
  getwarehouse;
  edt1.Clear;
  edt2.Clear;
...
procedure TfMain.cmbboxwarehouseSelect(Sender: TObject);
begin
   getlocate;    
end;

procedure TfMain.edtEmpNoChange(Sender: TObject);
begin
    with QryTemp do
    begin
        Close;
        ...
        CommandText :='Select emp_id,emp_name from sajet.sys_emp where emp_no =:empno';
        Params.ParamByName('empNO').AsString :=edtEmpNo.Text;
        Open;

        if not IsEmpty then begin
            edtEmpName.Text := fieldByName('emp_name').AsString;
            sborrowUserID := fieldByName('emp_id').AsString;
        end else  begin
             edtEmpName.Text :='';
             sborrowUserID := '';
        end;
...
procedure TfMain.cmbboxlocateSelect(Sender: TObject);
begin
   edtEmpNo.SetFocus;
end;
