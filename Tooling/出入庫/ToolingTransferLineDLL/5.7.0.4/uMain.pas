...
Begin
   ...
   begin
      ...
      if (UPPERCASE(fieldbyname('MACHINE_TYPE').AsString)='FI') OR (UPPERCASE(fieldbyname('MACHINE_TYPE').AsString)='I') OR (UPPERCASE(fieldbyname('MACHINE_TYPE').AsString)='TS') then
        begin
           lblstatus.Caption :='The Tooling_sn Of '+Trim(editsn.Text )+' Is Not Out Stock!';
           exit;
        end;
   ...
   with qrydata do
     begin
         ...
         commandtext:='Update SAJET.G_TOOLING_MATERIAL SET warehouse_id=:warehouse_id , '
                     +' locate_id=:locate_id,machine_type=''TL'' ,'
        ...
        lblstatus.Caption :=editsn.Text +' Transfer Line OK!';
...
begin
  ...
  begin
    ...
    Params.ParamByName('dll_name').AsString := 'ToolingTransferLineDLL.dll';
    Open;
  end;
