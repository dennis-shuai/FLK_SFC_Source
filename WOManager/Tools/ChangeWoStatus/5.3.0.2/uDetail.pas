...
procedure TfDetail.ConfirmClick(Sender: TObject);
begin
 if (strtoint(edtnewinputqty.Text)>=0) and ( MessageDlg('Change input_qty,Do you now?',
    mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
    begin
      with qrytemp do
      begin
          ...
          commandtext:='update sajet.g_wo_base set input_qty=:input_qty ,update_userid=:update_userid,update_time=sysdate '
                   +' where work_order=:work_order and rownum=1 ';
          params.ParamByName('input_qty').AsInteger :=strtoint(edtnewinputqty.Text);
          params.ParamByName('work_order').AsString :=g_wo;
          params.ParamByName('update_userid').AsString :=UpdateUserID;
          execute;
