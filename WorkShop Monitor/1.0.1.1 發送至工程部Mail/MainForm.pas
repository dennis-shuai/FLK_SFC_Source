...
uses
  ...
  SConnect,IniFiles,IdComponent, IdTCPConnection,Comobj,
...
begin
   ...
   if StrLstPDLineID.Count<=0 then
    MessageDlg('Please Choose PDLine ',mtInformation, [mbOK],0)
     ...
     Addresslist.Add('Duke_Chang@foxlink.com');
...
begin
      ...
      DataTemp.CommandText :=  ' select  MODEL_NAME,PDLINE_NAME,DATETIME,WORK_TIME,UPH,Cycle_Time,Remark,   '+
                               '  ROUND(exp(sum(ln(decode(SPY_YIELD,0,0.000001,SPY_YIELD))))*100,2)  SPY_YIELD, '+
                               '         round(exp(sum(ln(decode(FPY_YIELD,0,0.000001,FPY_YIELD))))*100,2) FPY_YIELD  '+
                               '  from (  ' +
                               ...
                               '       from (  '+
                               '                  select  g.MODEL_NAME,A.PDLINE_ID,C.PDLINE_NAME ,b.PROCESS_NAME,b.PROCESS_CODE,to_char(A.WORK_DATE)||TRIM(to_CHAR(A.WORK_TIME,''00''))  DATETIME,'+
...
begin
     ...
     DataTemp.Params.CreateParam(ftstring,'StartTime',ptInput);
     DataTemp.Params.CreateParam(ftstring,'EndTime',ptInput);
     DataTemp.CommandText :=   ' select  PDLINE_NAME,ROUND(exp(sum(ln(decode(SPY_YIELD,0,0.000001,SPY_YIELD))))*100,2) as SPY_YIELD, '+
...
var   ExcelApp:variant;
      ...
      sFristPDLine, sPDLineName,sModel,sSpy,sTargetYield,sWorkTime,sRemark,sRPDLINE:string;
begin
    ...
    ExcelApp.Range['A3:Q3'].Interior.Color :=clBlue;
    ExcelApp.Rows[3].Font.Color :=clWhite;
    ExcelApp.Range['A1:Q2'].Merge;
            ...
            if ExcelApp.Cells[j+4 ,2].Value <> '' then begin
                ...
                if  sPDLineName  = sRPDLINE then begin
                    ...
                    begin
                        ...
                        sFailPdline := sFailPdline+''''+sRPDLINE +''',';
    ...
    ExcelApp.ActiveSheet.Range['A1:Q'+IntToStr(PDLINECount+3)].HorizontalAlignment :=3;
    ExcelApp.ActiveSheet.Range['A1:Q'+IntToStr(PDLINECount+3)].VerticalAlignment :=2;
    ExcelApp.ActiveSheet.Range['A3:Q'+IntToStr(PDLINECount+3)].Font.Size :=11;
    ExcelApp.ActiveSheet.Range['A3:Q'+IntToStr(PDLINECount+3)].Font.Name :='Arial';
    ExcelApp.ActiveSheet.Range['A3:Q'+IntToStr(PDLINECount+3)].Font.Bold :=True;
    ExcelApp.ActiveSheet.Range['B4:Q'+IntToStr(PDLINECount+3)].Font.Color :=clBlue;
