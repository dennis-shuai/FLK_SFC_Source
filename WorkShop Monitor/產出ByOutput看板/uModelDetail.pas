...
uses
  ...
  Dialogs, ExtCtrls, TeeProcs, TeEngine, Chart,DB, DBClient,Series,DateUtils,
  OleCtrls, TeeChart_TLB, DbChart;
...
begin
    ...
    QryTemp.Params.CreateParam(ftstring,'EndTime',ptInput);
    QryTemp.Params.CreateParam(ftstring,'Pdline',ptInput);
    QryTemp.CommandText := ' select * from (select DATETIME,DEFECT_DESC ,SUM(DEFECT_QTY) QTY , row_number()'+
                           ...
                           ' select c.pdline_NAME, a.Serial_number ,B.DEFECT_CODE ,B.DEFECT_DESC,to_CHAR(a.REC_TIME,''YYYYMMDDHH24'' ) as "DATETIME", ' +
                           ...
                           ' group by  DATETIME  ,DEFECT_DESC )where ran<=3 ';
