...
begin
  if Key <>#13 then exit;
   ...
   if Copy(edtSN.Text,1,3) <> 'SN6' then
   begin
   ...
   begin
       ...
       if Isempty then begin
           ...
           if  Copy(edtSN.Text,1,4) <> 'SN6A' then
           begin
              close;
              params.clear;
              params.CreateParam(ftstring,'SN',ptInput);
              commandtext :='Update sajet.g_sn_status set customer_sn = SUBSTR(customer_sn,1,3)||''X''|| SUBSTR(customer_sn,5,6)   where   customer_sn =:sn and work_flag=0';
              params.ParamByName('SN').AsString := edtSN.Text;
              Execute;
