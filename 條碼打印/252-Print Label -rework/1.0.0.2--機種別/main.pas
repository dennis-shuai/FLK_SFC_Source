...
var sSN,sCSN,sNewCSN,sPartNo:string;
begin
  if Key <>#13 then exit;
   ...
   if Copy(edtSN.Text,1,2) <> 'SN' then
   begin
   ...
   begin
       ...
       commandtext :='select a.serial_number,a.customer_sn,b.part_no from sajet.g_sn_status a,sajet.sys_part b '+
                     ' where (a.serial_number = :sn or a.customer_sn =:sn )and a.work_flag=0 and a.model_id=b.part_id ';
       ...
       if Isempty then begin
           ...
           if sPartNo ='7691-1252-0080' then
               if  Copy(sCSN,1,5) = 'SN6X0' then
               begin
                  sNewCSN := Copy(sCSN,1,3)+'Y1'+Copy(sCSN,6,5);
                  close;
                  params.clear;
                  params.CreateParam(ftstring,'SN',ptInput);
                  params.CreateParam(ftstring,'CSN',ptInput);
                  commandtext :='Update sajet.g_sn_status set customer_sn =:CSN   where   serial_number =:sn and work_flag=0';
                  params.ParamByName('SN').AsString := sSN;
                  params.ParamByName('CSN').AsString := sNewCSN;
                  Execute;

                  close;
                  params.clear;
                  params.CreateParam(ftstring,'CSN',ptInput);
                  commandtext :='select * from sajet.g_sn_status where CUSTOMER_SN = :csn or customer_sn =:sn and work_flag=0';
                  params.ParamByName('CSN').AsString :=sNewCSN ;
                  Open;
                  if not IsEmpty then
                     edtsn.Text :=sNewCSN
