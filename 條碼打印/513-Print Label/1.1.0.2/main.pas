...
begin
  if Key <>#13 then exit;
   if LENGTH(edtSN.Text) <> 11 then begin
      MessageDlg('ªø«×¿ù»~',mterror,[mbok],0);
      edtsn.SelectAll;
      exit;
   end;
   if Copy(edtSN.Text,1,2) <> 'C1' then
   begin
   ...
   with qrydata do begin
       ...
       commandtext :='select WORK_ORDER,BOX_NO from sajet.g_sn_status where (serial_number = :sn or customer_sn =:sn) and work_flag=0';
       params.ParamByName('SN').AsString := edtSN.Text;
       Open;

       BOX :=fieldbyname('BOX_NO').AsString;
