...
begin
  ...
  begin
    ...
    begin
      try
        Close;
        DataRequest('SAJET.SJ_SMT_CKRT_PANEL');
        FetchParams;
        Params.ParamByName('TERMINALID').AsString := TerminalID;
        Params.ParamByName('TREV').AsString := EditSN.Text;
        Execute;
    ...
    if copy(sRes,1,2)='OK' then sRes :='OK';
    ...
    begin
      ...
      begin
        ...
        params.Clear;
        params.CreateParam(ftString, 'SN', ptInput);
        CommandText := ' Select A.SERIAL_NUMBER,A.WORK_ORDER,B.PART_NO ' +
          ' From SAJET.G_SN_STATUS A,SAJET.SYS_PART B  ' +
          ' where  A.BOX_NO=:SN  AND A.MODEL_ID=B.PART_ID  ' +
      ...
      begin
        try
          Close;
          DataRequest('SAJET.SJ_SMT_BOX_GO');
          FetchParams;
          Params.ParamByName('TTERMINALID').AsString := TerminalID;
