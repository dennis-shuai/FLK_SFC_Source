...
begin
   ...
   with Qrytemp do begin
      ...
      CommandText :='SELECT C.PART_NO FROM SAJET.G_SN_STATUS A,SAJET.G_SN_KEYPARTS B,SAJET.SYS_PART C '
                     +' WHERE B.SERIAL_NUMBER=A.SERIAL_NUMBER AND B.ITEM_PART_SN=:SN AND B.ENABLED=''Y'' '
                     +'  AND A.MODEL_ID=C.PART_ID';

      Params.ParamByName('SN').AsString :=edtGuitar.Text;
      Open;

      rPartno:=fieldbyname('PART_NO').Asstring;
      ...
      if IsEmpty then begin
           ...
           IsOpen :=false;
           PrintFile:= GetCurrentDir+'\\'+'C_'+rPartno+'.LAB' ;
           If not FileExists( PrintFile) then
           begin
                 MessageDlg( 'Label ÀÉ®×¤£¦s¦b',mterror,[mbOK],0);
                 IsOpen :=false;
           ...
           if (IsStart) and (IsOpen) then begin
            BarDoc.Variables.Item('D_SN').Value :=  CSN;
            BarDoc.Variables.Item('G_SN').Value :=  GCSN;
            BarDoc.Variables.Item('PART_NO').Value :=  rPartNO;
            Bardoc.PrintLabel(1);
            Bardoc.FormFeed;
           end;
...
var wo,part,color,sRes,DCsn:string;
begin
  if Key <>#13 then exit;
  ...
  with sproc do begin
      ...
      if sRes <>'OK' then begin
          ...
          if MessageDlg('¬O§_­«·s¦C¦L±ø½X?',mtInformation,[mbno,mbOK],0)=mrok then begin
                  with qrytemp do begin
                    close;
                    params.clear;
                    params.CreateParam(ftstring,'SN',ptInput);
                    commandtext :='SELECT A.SERIAL_NUMBER,A.CUSTOMER_SN FROM SAJET.G_SN_STATUS A,SAJET.G_SN_KEYPARTS B '
                                   +' WHERE A.SERIAL_NUMBER=B.SERIAL_NUMBER  '
                                   +' AND B.ITEM_PART_SN=:SN AND B.ENABLED=''Y''';
                    params.ParamByName('SN').AsString := edtGuitar.Text ;
                    Open;
                    gCSN :=Trim(edtGuitar.Text);
                    if not IsEmpty then begin

                      //­«·s¥´¦L
                      sn :=fieldbyname('serial_number').Asstring;
                      DCsn :=fieldbyname('CUSTOMER_SN').Asstring;

                      if DCsn <> Trim(edtDrum.Text) then begin

                           msgpanel.Caption := '±½´y±ø½X»P¸j©w²£«~¤£¤@­P¡A½ÐÀË¬d²£«~±ø½X';
                           msgpanel.Font.Color :=clRed;
                           msgpanel.Color := clYellow;
  ...
  begin
    Close;
    Params.Clear;
    Params.CreateParam(ftString,'GSN',ptInput);
    CommandText := 'SELECT SERIAL_NUMBER FROM SAJET.G_SN_KEYPARTS WHERE ITEM_PART_SN=:GSN AND ENABLED=''Y''';
    Params.ParamByName('GSN').AsString := gCSN;
    Open;

     if not IsEmpty then begin
          msgpanel.Caption := '¦N¥L±ø½X¤w¸g¸j©w¡A½Ð½T»{';
          msgpanel.Font.Color :=clYellow;
  ...
  begin
       ...
       params.clear;
       params.CreateParam(ftstring,'SN',ptInput);
       commandtext :='select SUBSTR(b.part_no,11,4) PART,a.work_order  '+
                      'from sajet.g_sn_status a,sajet.sys_part b where  '+
       ...
       //ÀË¬d¹ª±ø½X¬O§_©M¥]¸Ë¿é¤J¤u³æ¤@¼Ë¡C

       if WO<>edtWO.Text then
       begin
            msgpanel.Caption := CSN+'¬°'+WO+',½ÐÀË¬d¤u³æ¬O§_¥¿½T ';
            msgpanel.Font.Color :=clRed;
            msgpanel.Color := clYellow;
            edtDrum.Clear;
            edtGuitar.Clear;
...
begin
   ...
   begin
      ...
      Params.Clear;
      Params.CreateParam(ftString, 'PALLET_NO', ptInput);
      CommandText := 'SELECT COUNT(DISTINCT SERIAL_NUMBER) COUNT  FROM SAJET.G_SN_STATUS'
                      +' WHERE PALLET_NO=:PALLET_NO';
      Params.ParamByName('PALLET_NO').AsString := edtPallet.Text;
      Open;
      QTY:=fieldbyname('COUNT').AsString;
   ...
   if (IsStart) and (IsOpen) then begin
      ...
      BarDoc.Variables.Item('PART_NO').Value :=  mPartNo;
      BarDoc.Variables.Item('WORK_ORDER').Value :=  sWO;
